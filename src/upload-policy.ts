...
export function uploadValidationDetails(error: UploadValidationError, locale: Locale) {
  const messages: Record<UploadValidationError, { en: string; el: string; status: number }> = {
    ...
    file_too_large: { en: "Each file must be no larger than 20 MB.", el: "Κάθε αρχείο πρέπει να είναι έως 20 MB.", status: 413 },
    total_too_large: { en: "The total selection must be no larger than 80 MB.", el: "Η συνολική επιλογή πρέπει να είναι έως 80 MB.", status: 413 },
  };
