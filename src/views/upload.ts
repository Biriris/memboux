...
export function freeUploadGuardScript(locale: Locale) {
  const copy: Record<Locale, { status: string; full: string; closed: string; exceeded: string }> = {
    en: { status: "{plan} · {used}/{limit} photo or video slots used.", full: "This event has reached its {limit}-media package limit. Existing memories remain available.", closed: "The upload period for this event has ended. Existing memories remain available.", exceeded: "The selected files exceed the remaining upload slots." },
    el: { status: "{plan} · χρησιμοποιήθηκαν {used}/{limit} θέσεις φωτογραφιών ή βίντεο.", full: "Το event έφτασε το όριο των {limit} αρχείων του πακέτου. Οι υπάρχουσες αναμνήσεις παραμένουν διαθέσιμες.", closed: "Η περίοδος uploads αυτού του event ολοκληρώθηκε. Οι υπάρχουσες αναμνήσεις παραμένουν διαθέσιμες.", exceeded: "Τα επιλεγμένα αρχεία ξεπερνούν τις διαθέσιμες θέσεις upload." },
    fr: { status: "{plan} · {used}/{limit} emplacements utilisés.", full: "L’événement a atteint la limite de {limit} fichiers de son forfait.", closed: "La période d’ajout de cet événement est terminée.", exceeded: "Les fichiers sélectionnés dépassent les emplacements restants." },
    de: { status: "{plan} · {used}/{limit} Plätze belegt.", full: "Das Event hat sein Paketlimit von {limit} Dateien erreicht.", closed: "Der Upload-Zeitraum für dieses Event ist beendet.", exceeded: "Die ausgewählten Dateien überschreiten die verbleibenden Plätze." },
    es: { status: "{plan} · {used}/{limit} espacios usados.", full: "El evento alcanzó el límite de {limit} archivos de su paquete.", closed: "El periodo de subidas de este evento ha terminado.", exceeded: "Los archivos seleccionados superan los espacios restantes." },
    it: { status: "{plan} · {used}/{limit} spazi usati.", full: "L’evento ha raggiunto il limite di {limit} file del pacchetto.", closed: "Il periodo di upload di questo evento è terminato.", exceeded: "I file selezionati superano gli spazi rimanenti." },
  };
  return `<script>(()=>{const copy=${JSON.stringify(copy[locale])};document.querySelectorAll('form[data-multi-upload]').forEach(async form=>{const match=new URL(form.action,location.href).pathname.match(/^\/api\/upload\/([^/]+)$/);if(!match)return;try{const response=await fetch('/api/upload/'+encodeURIComponent(match[1])+'/capacity',{credentials:'include',headers:{Accept:'application/json'},cache:'no-store'});if(!response.ok)return;const state=await response.json();if(!state.limited)return;const remaining=Number(state.remaining),closed=Boolean(state.uploadWindowClosed),plan=state.planKey==='event_free'?'Memboux Free':state.planKey==='event_plus'?'Celebration':'Moments',input=form.querySelector('input[type="file"]'),button=form.querySelector('button[type="submit"],button:not([type])'),message=(closed?copy.closed:remaining<=0?copy.full:copy.status).replace('{plan}',plan).replace('{used}',state.used).replace('{limit}',state.limit),notice=document.createElement('div');form.dataset.freeUploadRemaining=String(remaining);notice.setAttribute('role','status');notice.className='rounded-xl border p-3 text-sm font-semibold leading-6 '+(closed||remaining<=0?'border-amber-300 bg-amber-50 text-amber-950':'border-violet-200 bg-violet-50 text-violet-950');notice.textContent=message;form.prepend(notice);if(closed||remaining<=0){if(input)input.disabled=true;if(button)button.disabled=true;button?.classList.add('cursor-not-allowed','opacity-50')}form.addEventListener('submit',event=>{const selected=input?.files?.length||0;if(!closed&&selected<=remaining)return;event.preventDefault();event.stopImmediatePropagation();window.alert(closed?copy.closed:copy.exceeded+' '+remaining);},{capture:true})}catch{}})})()<\/script>`;
}
