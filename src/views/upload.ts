...
export function uploadQueueScript(locale: Locale) {
  const copy: Record<Locale, { queue: string; waiting: string; uploading: string; complete: string; failed: string; refreshing: string; file: string; files: string }> = {
    ...
    el: { queue: "Σειρά μεταφόρτωσης", waiting: "Σε αναμονή", uploading: "Μεταφορτώνεται", complete: "Ολοκληρώθηκε", failed: "Χρειάζεται έλεγχο", refreshing: "Η μεταφόρτωση ολοκληρώθηκε. Ενημερώνουμε το κοινό άλμπουμ…", file: "αρχείο", files: "αρχεία" },
    fr: { queue: "File d’ajout", waiting: "En attente", uploading: "Ajout en cours", complete: "Terminé", failed: "À vérifier", refreshing: "Ajout terminé. Actualisation de l’album partagé…", file: "fichier", files: "fichiers" },
    de: { queue: "Upload-Warteschlange", waiting: "Wartet", uploading: "Wird hochgeladen", complete: "Fertig", failed: "Prüfung nötig", refreshing: "Upload abgeschlossen. Das gemeinsame Album wird aktualisiert…", file: "Datei", files: "Dateien" },
    es: { queue: "Cola de subida", waiting: "En espera", uploading: "Subiendo", complete: "Completado", failed: "Requiere atención", refreshing: "Subida completada. Actualizando el álbum compartido…", file: "archivo", files: "archivos" },
    it: { queue: "Coda di caricamento", waiting: "In attesa", uploading: "Caricamento", complete: "Completato", failed: "Da controllare", refreshing: "Caricamento completato. Aggiornamento dell’album condiviso…", file: "file", files: "file" },
  };
  const m = copy[locale];
  return `<script>(()=>{const marker='memboux-upload-return';if(sessionStorage.getItem(marker)==='gallery'){sessionStorage.removeItem(marker);if(location.hash!=='#guest-moments')history.replaceState(null,'',location.pathname+location.search+'#guest-moments');requestAnimationFrame(()=>document.getElementById('guest-moments')?.scrollIntoView({block:'start'}))}document.querySelectorAll('form[data-multi-upload],form[enctype="multipart/form-data"]').forEach(form=>{const input=form.querySelector('input[type=file][name=file][multiple]');if(!input||form.dataset.queueReady)return;form.dataset.queueReady='true';const queue=document.createElement('section');queue.className='mt-4 hidden rounded-2xl border border-[#e5dff0] bg-white p-4';queue.setAttribute('aria-label',${JSON.stringify(m.queue)});queue.innerHTML='<div class="flex items-center justify-between gap-3"><strong class="text-sm text-[#49395a]">${m.queue}</strong><span data-queue-summary class="text-xs font-semibold text-[#756b82]"></span></div><div data-queue-items class="mt-3 space-y-2"></div>';input.insertAdjacentElement('afterend',queue);const items=queue.querySelector('[data-queue-items]'),summary=queue.querySelector('[data-queue-summary]');let files=[];const render=()=>{files=[...(input.files||[])];queue.classList.toggle('hidden',!files.length);summary.textContent=files.length?files.length+' '+(files.length===1?${JSON.stringify(m.file)}:${JSON.stringify(m.files)}):'';items.innerHTML=files.map((file,index)=>'<div data-queue-index="'+index+'" class="rounded-xl bg-[#f8f5ff] p-3"><div class="flex min-w-0 items-center justify-between gap-3"><span class="min-w-0 truncate text-xs font-semibold text-[#49395a]">'+escapeHtml(file.name)+'</span><span data-queue-state class="shrink-0 text-[11px] text-[#756b82]">${m.waiting}</span></div><div class="mt-2 h-1.5 overflow-hidden rounded-full bg-[#e8e1f1]"><span data-queue-bar class="block h-full w-0 rounded-full bg-[#8b5cf6] transition-[width]"></span></div></div>').join('')};const escapeHtml=value=>{const node=document.createElement('span');node.textContent=value;return node.innerHTML};input.addEventListener('change',render);const bind=()=>{const panel=form.querySelector('[data-upload-status]')?.closest('div.mt-3'),status=form.querySelector('[data-upload-status]'),percent=form.querySelector('[data-upload-percent]'),detail=form.querySelector('[data-upload-detail]');if(!panel||!status||panel.dataset.queueObserved)return false;panel.dataset.queueObserved='true';new MutationObserver(()=>{const label=status.textContent||'',match=label.match(/(\\d+)\\s*\\/\\s*(\\d+)/),active=match?Math.max(0,Number(match[1])-1):-1,overall=Math.max(0,Math.min(100,Number.parseInt(percent?.textContent||'0',10)||0)),total=files.reduce((sum,file)=>sum+file.size,0),done=total*overall/100;let before=0;files.forEach((file,index)=>{const row=items.querySelector('[data-queue-index="'+index+'"]'),state=row?.querySelector('[data-queue-state]'),bar=row?.querySelector('[data-queue-bar]');if(!row||!state||!bar)return;const progress=index<active?100:index===active?Math.max(0,Math.min(100,(done-before)/Math.max(1,file.size)*100)):overall===100?100:0;bar.style.width=progress+'%';state.textContent=progress>=100?${JSON.stringify(m.complete)}:index===active?${JSON.stringify(m.uploading)}:${JSON.stringify(m.waiting)};before+=file.size});if(detail?.classList.contains('text-red-700')&&active>=0){const row=items.querySelector('[data-queue-index="'+active+'"]');row?.querySelector('[data-queue-state]')?.replaceChildren(${JSON.stringify(m.failed)})}if(overall===100){sessionStorage.setItem(marker,'gallery');summary.textContent=${JSON.stringify(m.refreshing)}}}).observe(panel,{subtree:true,childList:true,characterData:true,attributes:true});return true};if(!bind()){const observer=new MutationObserver(()=>{if(bind())observer.disconnect()});observer.observe(form,{subtree:true,childList:true})}})})()<\/script>`;
}
