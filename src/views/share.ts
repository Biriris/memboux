...
const buttonClass =
  "inline-flex h-11 w-11 items-center justify-center rounded-full text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#2f6b5b] focus:ring-offset-2";
...
export function shareIconButtons(
  ...
  return `<div class="mt-5 flex flex-wrap justify-center gap-2">
    <a href="sms:?&body=${text}" class="${buttonClass} bg-[#4f7f6b] sm:hidden" aria-label="Text message" title="Text message">${icon(smsSvg)}</a>
  ...
  </div>${showOfficialLink ? `<a href="${esc(guestUrl)}/official?lang=${locale}" class="mt-4 inline-flex rounded-xl border border-[#dbe6e1] bg-white px-5 py-2 text-sm text-[#2f6b5b]">Official album</a>` : ""}
