...
const buttonClass =
  "inline-flex h-11 w-11 items-center justify-center rounded-full text-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#7c3aed] focus:ring-offset-2";
...
const instagramSvg =
  '<path fill-rule="evenodd" d="M7 2h10a5 5 0 0 1 5 5v10a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5V7a5 5 0 0 1 5-5Zm0 2a3 3 0 0 0-3 3v10a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3V7a3 3 0 0 0-3-3H7Zm5 3a5 5 0 1 1 0 10 5 5 0 0 1 0-10Zm0 2.2a2.8 2.8 0 1 0 0 5.6 2.8 2.8 0 0 0 0-5.6ZM18.3 5.5a1.2 1.2 0 1 1 0 2.4 1.2 1.2 0 0 1 0-2.4Z"/>';
const smsSvg =
  '<path d="M4 3h16a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H9l-5 4v-4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2Zm3 6h10v2H7V9Zm0 4h7v2H7v-2Z"/>';

const shareCopy: Record<Locale, {
  message: (eventName: string, guestUrl: string) => string;
  textMessage: string;
  share: string;
  officialAlbum: string;
}> = {
  en: { message: (name, url) => `View and add moments to ${name}: ${url}`, textMessage: "Text message", share: "Share", officialAlbum: "Official album" },
  el: { message: (name, url) => `Δες και πρόσθεσε στιγμές στο ${name}: ${url}`, textMessage: "Μήνυμα SMS", share: "Κοινοποίηση", officialAlbum: "Επίσημο album" },
  fr: { message: (name, url) => `Découvrez et ajoutez vos moments à ${name} : ${url}`, textMessage: "SMS", share: "Partager", officialAlbum: "Album officiel" },
  de: { message: (name, url) => `Sieh dir Momente von ${name} an und füge deine hinzu: ${url}`, textMessage: "SMS", share: "Teilen", officialAlbum: "Offizielles Album" },
  es: { message: (name, url) => `Descubre y añade momentos a ${name}: ${url}`, textMessage: "SMS", share: "Compartir", officialAlbum: "Álbum oficial" },
  it: { message: (name, url) => `Guarda e aggiungi momenti a ${name}: ${url}`, textMessage: "SMS", share: "Condividi", officialAlbum: "Album ufficiale" },
};

function shareButton(label: string, color: string, body: string, attrs: string) {
  return `<button type="button" ${attrs} class="${buttonClass} ${color}" aria-label="${label}" title="${label}">${icon(body)}</button>`;
}
...
export function shareIconButtons(
  ...
  return `<div class="mt-5 flex flex-wrap justify-center gap-2">
    ...
    <button type="button" data-native-share data-title="${esc(eventName)}" data-text="${esc(shareText)}" data-url="${esc(guestUrl)}" class="${buttonClass} bg-black" aria-label="${esc(copy.share)}" title="${esc(copy.share)}">${icon('<path d="M14 3h3c.3 2 1.5 3.4 4 3.8v3.1a8.2 8.2 0 0 1-4-1.2V15a7 7 0 1 1-6-6.9v3.2a3.8 3.8 0 1 0 3 3.7V3Z"/>')}</button>
  </div>${showOfficialLink ? `<a href="${esc(guestUrl)}/official?lang=${locale}" class="mt-4 inline-flex rounded-xl border border-[#e6dff0] bg-white px-5 py-2 text-sm text-[#7c3aed]">${esc(copy.officialAlbum)}</a>` : ""}
