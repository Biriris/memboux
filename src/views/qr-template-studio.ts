...
export function renderQrTemplateStudio(input: QrStudioInput) {
  ...
  (()=>{
    ...
    const state={family:data.families[0].key,format:data.formats[1].key,copy:data.copies[0].key,destination:data.destinations[0].key};
    ...
    applyCopy();applyFamily(state.family,false);track('opened');
  })();
  <\/script>`;
}
