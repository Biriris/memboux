...
const lightboxSwipeDismissBehavior = `<script>(()=>{const dialog=document.getElementById('media-lightbox'),content=document.getElementById('lightbox-content'),zoomReset=document.getElementById('lightbox-zoom-reset');if(!dialog||!content||dialog.dataset.swipeDismissReady)return;dialog.dataset.swipeDismissReady='1';let startX=0,startY=0,dragY=0,tracking=false,dismissing=false;const clear=()=>{tracking=false;dismissing=false;dragY=0};dialog.addEventListener('touchstart',event=>{const touch=event.touches[0],image=event.target.closest?.('.native-save-image');if(!dialog.open||event.touches.length!==1||!touch||!image||zoomReset?.textContent!=='100%'){clear();return}startX=touch.clientX;startY=touch.clientY;dragY=0;tracking=true;dismissing=false},{capture:true,passive:true});dialog.addEventListener('touchmove',event=>{if(!tracking||event.touches.length!==1)return;const touch=event.touches[0],dx=touch.clientX-startX,dy=touch.clientY-startY;if(!dismissing){if(Math.abs(dx)>Math.abs(dy)){tracking=false;return}if(dy>=-10)return;dismissing=true}event.preventDefault();event.stopImmediatePropagation();dragY=Math.min(0,dy);const distance=Math.min(Math.abs(dragY)/innerHeight,1);content.style.transition='none';content.style.transform='translateY('+dragY+'px) scale('+(1-distance*.05)+')';content.style.opacity=String(1-Math.min(distance,.7))},{capture:true,passive:false});dialog.addEventListener('touchend',event=>{if(!dismissing){clear();return}event.preventDefault();event.stopImmediatePropagation();content.style.transition='transform 180ms ease-out,opacity 180ms ease-out';if(dragY<-80){content.style.transform='translateY(-110vh) scale(.95)';content.style.opacity='0';setTimeout(()=>{if(dialog.open)dialog.close();content.style.transition='none';content.style.transform='';content.style.opacity='1';clear()},180)}else{content.style.transform='translateY(0) scale(1)';content.style.opacity='1';setTimeout(clear,180)}},{capture:true,passive:false});dialog.addEventListener('touchcancel',()=>{content.style.transform='';content.style.opacity='1';clear()},{capture:true})})()<\/script>`;
...
export function page(title: string, body: string, options: PageOptions = {}) {
  ...
  const renderedBody = settingsSubsection
    ...
    : baseRenderedBody;
  return `<!doctype html><html lang="${locale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><meta name="supported-color-schemes" content="light"><meta name="theme-color" content="#20152f"><meta name="application-name" content="Memboux"><meta name="apple-mobile-web-app-title" content="Memboux"><meta name="keywords" content="${keywords}"><meta name="description" content="${esc(description)}"><meta name="robots" content="${robots}">${canonical}${alternates}${social}${structuredData}<link rel="icon" type="image/png" href="/brand/memboux-icon.png"><link rel="apple-touch-icon" href="/brand/memboux-icon.png"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Manrope:wght@200..800&display=swap" rel="stylesheet">${options.additionalHead ?? ""}<title>${esc(title)}</title><link rel="stylesheet" href="/app-midnight.css?v=20260812-commercial-launch"></head><body data-design-system="memboux-v2" class="memboux-ui min-h-screen bg-[#faf8ff] text-[#2b174d]">${renderedBody}${creationBehavior}${invitationBehavior}${uploadBehavior}${brickwallBehavior}${uploaderBehavior}${swipeDismissBehavior}${privacySupport}${outsideDismissBehavior}</body></html>`;
}
...
export function eventHeader(
  ...
  return `<header class="app-shell-header mbx-app-header relative z-40"><div class="mbx-header-inner flex items-center justify-between gap-2 py-3 sm:gap-3 sm:py-4">${brandMark(`/${locale}/account`, true, true)}<div class="flex shrink-0 items-center gap-2">${secondaryAction}<span class="header-primary-action">${primaryAction}</span>${notificationBell(locale, notificationCount, true)}${accountMenuDark(locale, user, notificationCount)}</div></div></header>`;
}

export function accountHeader(locale: Locale, user: { name: string; email: string }, notificationCount = 0) {
  return `<header class="app-shell-header mbx-app-header relative z-40"><div class="mbx-header-inner flex max-w-5xl items-center justify-between gap-2 py-3 sm:py-4">${brandMark(`/${locale}/account`, true, true)}<div class="flex shrink-0 items-center gap-2">${notificationBell(locale, notificationCount, true)}${accountMenuDark(locale, user, notificationCount)}</div></div></header>`;
}
