...
const navigationStateBehavior = `<script>(()=>{if(window.__membouxNavigationState)return;window.__membouxNavigationState=true;const stateKey='membouxNavigationV1',pageUrl=location.pathname+location.search+location.hash;try{history.scrollRestoration='manual'}catch{}const snapshot=()=>{const galleries={};document.querySelectorAll('[data-gallery-more][data-gallery-offset]').forEach(button=>{const prefix=button.dataset.galleryMore,sort=document.querySelector('[data-gallery-sort="'+CSS.escape(prefix||'')+'"]');if(prefix)galleries[prefix]={offset:Number(button.dataset.galleryOffset)||0,sort:sort?.value||'chronology'}});const details=[...document.querySelectorAll('details[id][open]')].map(item=>item.id);return{url:pageUrl,x:scrollX,y:scrollY,galleries,details}};const save=()=>{try{history.replaceState({...history.state,[stateKey]:snapshot()},'',location.href)}catch{}};addEventListener('pagehide',save);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden')save()});let restoring=false;const restore=async()=>{if(restoring)return;const saved=history.state?.[stateKey];if(!saved||saved.url!==pageUrl)return;restoring=true;try{for(const id of saved.details||[]){const item=document.getElementById(id);if(item instanceof HTMLDetailsElement)item.open=true}for(const [prefix,gallery] of Object.entries(saved.galleries||{})){const button=document.querySelector('[data-gallery-more="'+CSS.escape(prefix)+'"]'),sort=document.querySelector('[data-gallery-sort="'+CSS.escape(prefix)+'"]'),load=button?.__membouxLoadMore;if(!button||typeof load!=='function')continue;if(sort&&gallery.sort&&sort.value!==gallery.sort){sort.value=gallery.sort;await load(true)}let previous=-1;while(Number(button.dataset.galleryOffset)<Number(gallery.offset)&&Number(button.dataset.galleryOffset)!==previous&&!button.classList.contains('hidden')){previous=Number(button.dataset.galleryOffset);await load(false)}}await document.fonts?.ready?.catch?.(()=>{});await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));scrollTo(Number(saved.x)||0,Number(saved.y)||0);document.dispatchEvent(new CustomEvent('memboux:navigation-restored'))}finally{restoring=false}};restore();addEventListener('pageshow',event=>{if(event.persisted)restore()})})()<\/script>`;

const lightboxSwipeDismissBehavior = `<script>(()=>{const dialog=document.getElementById('media-lightbox'),content=document.getElementById('lightbox-content'),zoomReset=document.getElementById('lightbox-zoom-reset');if(!dialog||!content||dialog.dataset.swipeDismissReady)return;dialog.dataset.swipeDismissReady='1';let startX=0,startY=0,dragY=0,tracking=false,dismissing=false,mediaType='image';const clear=()=>{tracking=false;dismissing=false;dragY=0};dialog.addEventListener('touchstart',event=>{const touch=event.touches[0],media=event.target.closest?.('.native-save-image,video');if(!dialog.open||event.touches.length!==1||!touch||!media||!content.contains(media)||zoomReset?.textContent!=='100%'){clear();return}if(media instanceof HTMLVideoElement&&touch.clientY>media.getBoundingClientRect().bottom-72){clear();return}startX=touch.clientX;startY=touch.clientY;dragY=0;tracking=true;dismissing=false;mediaType=media instanceof HTMLVideoElement?'video':'image'},{capture:true,passive:true});dialog.addEventListener('touchmove',event=>{if(!tracking||event.touches.length!==1)return;const touch=event.touches[0],dx=touch.clientX-startX,dy=touch.clientY-startY;if(!dismissing){if(Math.abs(dx)>Math.abs(dy)){tracking=false;return}if(Math.abs(dy)<10)return;dismissing=true}event.preventDefault();event.stopImmediatePropagation();dragY=dy;const distance=Math.min(Math.abs(dragY)/innerHeight,1);content.style.transition='none';content.style.transform='translateY('+dragY+'px) scale('+(1-distance*.05)+')';content.style.opacity=String(1-Math.min(distance,.7))},{capture:true,passive:false});dialog.addEventListener('touchend',event=>{if(!dismissing){clear();return}event.preventDefault();event.stopImmediatePropagation();content.style.transition='transform 180ms ease-out,opacity 180ms ease-out';if(Math.abs(dragY)>80){const direction=dragY>0?'down':'up';content.style.transform='translateY('+(direction==='down'?'110vh':'-110vh')+') scale(.95)';content.style.opacity='0';window.dispatchEvent(new CustomEvent('memboux:lightbox-dismiss',{detail:{method:'swipe_'+direction,mediaType}}));setTimeout(()=>{if(dialog.open)dialog.close();content.style.transition='none';content.style.transform='';content.style.opacity='1';clear()},180)}else{content.style.transform='translateY(0) scale(1)';content.style.opacity='1';setTimeout(clear,180)}},{capture:true,passive:false});dialog.addEventListener('touchcancel',()=>{content.style.transform='';content.style.opacity='1';clear()},{capture:true})})()<\/script>`;
...
function renderAccountMenu(
  ...
  const triggerClass = darkTrigger
    ...
    : "border-[#e2dcef] bg-white/70 text-[#2b174d] hover:bg-white focus-visible:ring-[#8b5cf6]/35";
  const item = (href: string, icon: AccountMenuIcon, label: string) => `<a data-account-menu-link href="${href}" class="group flex min-h-12 items-center gap-3 rounded-xl px-2.5 py-2 text-sm font-medium text-[#49395a] outline-none transition hover:bg-[#f7f3ff] focus-visible:bg-[#f1eaff] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#a78bfa]"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-[#e4ebe8] bg-white text-[#746a80] shadow-sm transition group-hover:border-[#cfdbd6] group-hover:text-[#583a78]">${accountMenuIcon(icon)}</span><span class="min-w-0 flex-1 truncate">${esc(label)}</span>${accountMenuIcon("chevron", "h-4 w-4 text-[#a1ada8] transition group-hover:translate-x-0.5")}</a>`;
  ...
  return `<details data-account-menu class="account-menu group/account relative z-50 shrink-0">
    <summary data-account-menu-trigger aria-label="${labels.menu}" aria-expanded="false" class="relative flex h-11 w-11 touch-manipulation cursor-pointer list-none select-none items-center justify-center gap-2 rounded-xl border p-0 outline-none transition focus-visible:ring-2 sm:h-auto sm:w-auto sm:justify-start sm:px-2.5 sm:py-1.5 ${triggerClass}">
      <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${darkTrigger ? "bg-white/15" : "bg-[#efe8f8] text-[#583a78]"} text-sm font-semibold">${initial}</span>
    ...
    <div data-account-menu-panel class="fixed inset-x-3 top-[4.75rem] z-[100] rounded-3xl border border-[#eae4f3] bg-white p-1.5 text-[#2b174d] shadow-[0_24px_70px_rgba(18,43,36,.24)] sm:absolute sm:inset-x-auto sm:right-0 sm:top-auto sm:mt-2 sm:w-[19rem]">
      <div class="flex min-w-0 items-center gap-3 rounded-[1.15rem] bg-[#f8f5ff] p-3">
          ...
          <span class="min-w-0 flex-1"><strong class="block truncate text-sm font-semibold text-[#2b174d]">${esc(user.name)}</strong><span class="block truncate text-xs text-[#7d7188]">${esc(user.email)}</span></span>
      </div>
      ...
      <div class="border-t border-[#eee8f5] p-2">
        <button type="button" data-logout class="group flex min-h-11 w-full items-center gap-3 rounded-xl px-2.5 py-2 text-left text-sm font-medium text-[#4b645b] outline-none transition hover:bg-[#f7f3ff] hover:text-[#2b174d] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#a78bfa]"><span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-[#e4ebe8] bg-white text-[#776c81] shadow-sm">${accountMenuIcon("signout")}</span><span class="flex-1">${esc(labels.signOut)}</span>${accountMenuIcon("chevron", "h-4 w-4 text-[#a1ada8]")}</button>
      </div>
    </div>
...
export function eventHeader(
  ...
  return `<header class="app-shell-header mbx-app-header relative z-40"><div class="mbx-header-inner flex items-center justify-between gap-2 py-3 sm:gap-3 sm:py-4">${brandMark(`/${locale}/account`, true)}<div class="flex shrink-0 items-center gap-2">${secondaryAction}<span class="header-primary-action">${primaryAction}</span>${notificationBell(locale, notificationCount)}${accountMenu(locale, user, notificationCount)}</div></div></header>`;
}

export function accountHeader(locale: Locale, user: { name: string; email: string }, notificationCount = 0) {
  return `<header class="app-shell-header mbx-app-header relative z-40"><div class="mbx-header-inner flex max-w-5xl items-center justify-between gap-2 py-3 sm:py-4">${brandMark(`/${locale}/account`, true)}<div class="flex shrink-0 items-center gap-2">${notificationBell(locale, notificationCount)}${accountMenu(locale, user, notificationCount)}</div></div></header>`;
}
...
function notificationBell(locale: Locale, notificationCount = 0, dark = false) {
  ...
  const colors = dark
    ? "border-white/15 bg-white/5 text-white hover:bg-white/10 focus-visible:ring-white/40"
    : "border-[#e2dcef] bg-white/70 text-[#2b174d] hover:bg-white focus-visible:ring-[#8b5cf6]/35";
  return `<details data-notification-menu data-locale="${locale}" class="group/notifications relative z-50 shrink-0"><summary data-notification-bell data-notification-trigger aria-label="${label}" title="${label}" class="relative flex h-11 w-11 cursor-pointer list-none items-center justify-center rounded-xl border outline-none transition focus-visible:ring-2 ${colors}"><svg aria-hidden="true" viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4"/></svg><span data-account-notification-count class="${notificationCount > 0 ? "flex" : "hidden"} absolute -right-1.5 -top-1.5 min-h-5 min-w-5 items-center justify-center rounded-full border-2 ${dark ? "border-[#2b174d]" : "border-white"} bg-[#ef4444] px-1 text-[10px] font-bold leading-none text-white">${countLabel}</span></summary><section class="fixed inset-x-3 top-[4.75rem] z-[110] overflow-hidden rounded-3xl border border-[#eae4f3] bg-white text-[#2b174d] shadow-[0_24px_70px_rgba(18,43,36,.25)] sm:absolute sm:inset-x-auto sm:right-0 sm:top-auto sm:mt-2 sm:w-[24rem]" aria-label="${label}"><header class="border-b border-[#eee8f5] px-5 py-4"><div class="flex items-center justify-between gap-3"><div><p class="text-[10px] font-bold uppercase tracking-[.18em] text-[#877d91]">Memboux</p><h2 class="mt-0.5 text-lg font-semibold">${label}</h2></div><a href="/${locale}/notifications?view=history" class="rounded-full bg-[#f6f2fc] px-3 py-1.5 text-xs font-bold text-[#65566f] hover:bg-[#e8efec]">${viewAll}</a></div><button type="button" data-notification-read-all class="mt-3 text-xs font-semibold text-[#6d28d9] hover:text-[#583a78] disabled:opacity-50">${readAll}</button></header><div data-notification-list class="max-h-[min(26rem,calc(100dvh-10rem))] overflow-y-auto overscroll-contain"><p class="px-5 py-8 text-center text-sm text-[#807588]">${loading}</p></div></section></details>${notificationPanelBehavior}`;
}
