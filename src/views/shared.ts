...
export function accountMenu(locale: Locale, user: { name: string; email: string }) {
  const labels = locale === "el"
    ? { events: "Τα events μου", profile: "Προφίλ", security: "Ασφάλεια", trash: "Κάδος", signOut: "Αποσύνδεση" }
    : { events: "My events", profile: "Profile", security: "Security", trash: "Trash", signOut: "Sign out" };
