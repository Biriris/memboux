...
const mediaInteractionCopy: Record<Locale, {
  ...
  en: { like: "Like photo", unlike: "Unlike photo", report: "Report", uploadedBy: "Uploaded by", selectMoment: "Select this moment", downloadOriginal: "Download original", originalsUpgrade: "Originals unlock with upgrade", previous: "Previous", next: "Next", lightbox: "Media viewer", likeCount: (count) => `${count} ${count === 1 ? "like" : "likes"}` },
  el: { like: "Βάλε καρδιά", unlike: "Αφαίρεση καρδιάς", report: "Αναφορά", uploadedBy: "Ανέβηκε από", selectMoment: "Επιλογή αυτής της στιγμής", downloadOriginal: "Λήψη πρωτότυπου", originalsUpgrade: "Τα πρωτότυπα αρχεία ξεκλειδώνουν με αναβάθμιση", previous: "Προηγούμενο", next: "Επόμενο", lightbox: "Προβολή πολυμέσων", likeCount: (count) => `${count} καρδιές` },
  fr: { like: "Aimer la photo", unlike: "Retirer le cœur", report: "Signaler", uploadedBy: "Ajouté par", selectMoment: "Sélectionner ce moment", downloadOriginal: "Télécharger l’original", originalsUpgrade: "Les originaux sont disponibles après mise à niveau", previous: "Précédent", next: "Suivant", lightbox: "Visionneuse multimédia", likeCount: (count) => `${count} ${count === 1 ? "mention J’aime" : "mentions J’aime"}` },
  de: { like: "Foto liken", unlike: "Like entfernen", report: "Melden", uploadedBy: "Hochgeladen von", selectMoment: "Diesen Moment auswählen", downloadOriginal: "Original herunterladen", originalsUpgrade: "Originale werden mit einem Upgrade freigeschaltet", previous: "Zurück", next: "Weiter", lightbox: "Medienansicht", likeCount: (count) => `${count} ${count === 1 ? "Like" : "Likes"}` },
  es: { like: "Dar me gusta", unlike: "Quitar me gusta", report: "Denunciar", uploadedBy: "Subido por", selectMoment: "Seleccionar este momento", downloadOriginal: "Descargar original", originalsUpgrade: "Los originales se desbloquean con una mejora", previous: "Anterior", next: "Siguiente", lightbox: "Visor multimedia", likeCount: (count) => `${count} ${count === 1 ? "me gusta" : "me gusta"}` },
  it: { like: "Metti Mi piace", unlike: "Rimuovi Mi piace", report: "Segnala", uploadedBy: "Caricato da", selectMoment: "Seleziona questo momento", downloadOriginal: "Scarica originale", originalsUpgrade: "Gli originali si sbloccano con un upgrade", previous: "Precedente", next: "Successivo", lightbox: "Visualizzatore multimediale", likeCount: (count) => `${count} Mi piace` },
};
...
export function cards(items: MediaCardRow[], options?: MediaCardOptions) {
  return items.map((m, index) => {
    ...
    const selector=options?.selectable?(options.deferredSelection?`<label class="media-selector absolute inset-0 z-20 hidden cursor-pointer bg-transparent transition"><input type="checkbox" aria-label="${esc(copy.selectMoment)}" class="media-select sr-only" value="${esc(m.id)}" data-download="/media/${encodeURIComponent(m.id)}?download=1"><span class="selection-tick absolute left-3 top-3 hidden h-8 w-8 items-center justify-center rounded-full bg-[#7c3aed] text-lg text-white shadow-lg" aria-hidden="true">✓</span></label>`:`<label class="absolute left-3 top-3 z-10 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-white/95 shadow"><input type="checkbox" aria-label="${esc(copy.selectMoment)}" class="media-select h-4 w-4" value="${esc(m.id)}" data-download="/media/${encodeURIComponent(m.id)}?download=1"></label>`):"";
    ...
    const uploader = (options?.showUploader ?? options?.lightbox) && uploaderName
      ...
      : "";
    return `<article data-media-type="${m.media_type}" data-media-order="${index}" data-media-uploaded="${Number(m.uploaded_at) || 0}" data-media-captured="${Number(m.captured_at ?? m.uploaded_at) || 0}" data-media-rating="${Math.max(0, Number(m.like_count ?? 0))}"${deferred ? ' data-gallery-deferred="true"' : ""} class="memboux-media-card selectable-media relative mb-3 overflow-hidden rounded-2xl bg-[#f7f3ff] shadow-sm transition sm:mb-4${deferred ? " hidden" : ""}">${selector}${options?.reportCode ? `<a href="/gallery/${encodeURIComponent(options.reportCode)}/removal/${encodeURIComponent(m.id)}?lang=${options.locale??"en"}" class="absolute right-2 top-2 z-10 rounded-full bg-black/55 px-3 py-1 text-xs text-white">${esc(copy.report)}</a>` : ""}${cover}${uploader}${like}${content}</article>`;
  }).join("");
}
...
export function lightboxMarkup(locale: Locale, likes = false, originalDownloads = true) {
  ...
  return `<dialog id="media-lightbox" aria-label="${esc(copy.lightbox)}" class="fixed inset-0 m-0 h-dvh w-screen max-h-none max-w-none overflow-hidden bg-transparent p-0 backdrop:bg-black/90">
    <div id="lightbox-stage" class="relative flex h-full w-full items-center justify-center p-3 sm:p-12">
      ...
      ${originalDownloads ? `<a id="lightbox-download" aria-label="${esc(downloadLabel)}" class="absolute right-4 top-4 z-30 inline-flex min-h-10 items-center gap-2 rounded-full border border-white/25 bg-black/55 px-3 py-2 text-xs font-bold text-white shadow-xl backdrop-blur sm:right-6 sm:top-6" download><svg aria-hidden="true" viewBox="0 0 24 24" class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3v12m-5-5 5 5 5-5M5 20h14"/></svg><span class="hidden sm:inline">${esc(downloadLabel)}</span></a>` : `<span class="absolute right-4 top-4 z-30 rounded-full border border-white/25 bg-black/55 px-3 py-2 text-xs font-bold text-white shadow-xl backdrop-blur sm:right-6 sm:top-6">${esc(copy.originalsUpgrade)}</span>`}
      ${lightboxLike}${doubleHeart}
    </div>
  </dialog>
  <script>(()=>{const items=[...document.querySelectorAll('.lightbox-item')],dialog=document.getElementById('media-lightbox'),content=document.getElementById('lightbox-content'),stage=document.getElementById('lightbox-stage'),title=document.getElementById('lightbox-title'),download=document.getElementById('lightbox-download'),likeButton=document.getElementById('lightbox-like'),doubleHeart=document.getElementById('lightbox-double-heart');if(!dialog||!content||!stage)return;let index=0;const activeItems=()=>items.filter(item=>!item.closest('[data-media-type]')?.classList.contains('hidden'));const resetPosition=()=>{content.style.transition='none';content.style.transform='translateX(0) scale(1)';content.style.opacity='1'};const syncLike=item=>{if(!likeButton)return;const visible=item.dataset.type==='image'&&item.dataset.mediaId;if(!visible){likeButton.classList.add('hidden');likeButton.classList.remove('inline-flex');return}const liked=item.dataset.liked==='true',heart=likeButton.querySelector('[data-like-heart]'),counter=likeButton.querySelector('[data-like-count]');likeButton.dataset.mediaId=item.dataset.mediaId;likeButton.setAttribute('aria-pressed',String(liked));likeButton.setAttribute('aria-label',liked?${JSON.stringify(unlikeLabel)}:${JSON.stringify(likeLabel)});likeButton.classList.remove('hidden');likeButton.classList.add('inline-flex');if(counter)counter.textContent=item.dataset.likeCount||'0';if(heart){heart.classList.toggle('fill-rose-500',liked);heart.classList.toggle('text-rose-500',liked);heart.classList.toggle('fill-transparent',!liked);heart.classList.toggle('text-white',!liked)}};const show=i=>{const available=activeItems();if(!available.length)return;index=(i+available.length)%available.length;resetPosition();const item=available[index],src=item.dataset.src||item.dataset.full,original=item.dataset.original||item.dataset.full||src;content.replaceChildren();if(item.dataset.type==='video'){const video=document.createElement('video');video.src=src;video.controls=true;video.autoplay=true;video.playsInline=true;video.preload='metadata';video.className='max-h-[86dvh] max-w-[94vw] bg-black object-contain sm:max-w-[86vw]';content.append(video)}else{const image=document.createElement('img');image.src=src;image.alt='';image.draggable=true;image.className='native-save-image max-h-[86dvh] max-w-[94vw] touch-manipulation select-none object-contain sm:max-w-[86vw]';content.append(image)}if(download)download.href=original;if(title)title.textContent=item.dataset.title||'';syncLike(item)};const showDoubleHeart=()=>{if(!doubleHeart)return;doubleHeart.classList.remove('lightbox-heart-pop');void doubleHeart.offsetWidth;doubleHeart.classList.add('lightbox-heart-pop')};items.forEach(item=>item.addEventListener('click',()=>{show(activeItems().indexOf(item));if(!dialog.open)dialog.showModal()}));document.getElementById('lightbox-prev')?.addEventListener('click',()=>show(index-1));document.getElementById('lightbox-next')?.addEventListener('click',()=>show(index+1));let touchX=0,touchY=0,dragX=0,dragging=false,tapStartedOnImage=false,lastTapAt=0,lastTapX=0,lastTapY=0;dialog.addEventListener('touchstart',event=>{if(!dialog.open||!event.touches.length)return;const touch=event.touches[0];touchX=touch.clientX;touchY=touch.clientY;dragX=0;dragging=false;tapStartedOnImage=Boolean(event.target.closest?.('.native-save-image'));content.style.transition='none'},{passive:true});dialog.addEventListener('touchmove',event=>{if(!dialog.open||!event.touches.length)return;const touch=event.touches[0],dx=touch.clientX-touchX,dy=touch.clientY-touchY;if(!dragging&&Math.abs(dx)>8&&Math.abs(dx)>Math.abs(dy))dragging=true;if(!dragging)return;event.preventDefault();dragX=dx;lastTapAt=0;const scale=1-Math.min(Math.abs(dx)/innerWidth,1)*.04;content.style.transform='translateX('+dx+'px) scale('+scale+')';content.style.opacity=String(1-Math.min(Math.abs(dx)/innerWidth,.35))},{passive:false});dialog.addEventListener('touchend',event=>{if(!dragging){resetPosition();const touch=event.changedTouches[0],now=Date.now(),isDouble=tapStartedOnImage&&touch&&now-lastTapAt<340&&Math.hypot(touch.clientX-lastTapX,touch.clientY-lastTapY)<36;if(isDouble){event.preventDefault();const item=activeItems()[index];if(item?.dataset.type==='image'&&likeButton){if(item.dataset.liked!=='true'&&likeButton.dataset.busy!=='1')likeButton.click();showDoubleHeart()}lastTapAt=0}else if(tapStartedOnImage&&touch){lastTapAt=now;lastTapX=touch.clientX;lastTapY=touch.clientY}else lastTapAt=0;tapStartedOnImage=false;return}content.style.transition='transform 180ms ease-out,opacity 180ms ease-out';if(Math.abs(dragX)>60){content.style.transform='translateX('+(dragX<0?'-110vw':'110vw')+') scale(.96)';content.style.opacity='0';setTimeout(()=>show(dragX<0?index+1:index-1),180)}else resetPosition();dragging=false;tapStartedOnImage=false;lastTapAt=0},{passive:false});dialog.addEventListener('click',event=>{if(event.target===dialog||event.target===stage)dialog.close()});document.addEventListener('keydown',event=>{if(!dialog.open)return;if(event.key==='ArrowLeft')show(index-1);if(event.key==='ArrowRight')show(index+1);if(event.key==='Escape')dialog.close()})})()<\/script>`;
}
