...
export function authPage(
  ...
  return page(
    ...
    `<main class="min-h-screen bg-[#f8f5ff] p-4 sm:p-6 lg:flex lg:items-center lg:justify-center">
      <section class="mx-auto grid w-full max-w-6xl overflow-hidden rounded-[2rem] border border-[#e6dff0] bg-white shadow-[0_30px_100px_rgba(24,60,51,.13)] lg:min-h-[720px] lg:grid-cols-[.9fr_1.1fr]">
        <aside class="relative hidden overflow-hidden bg-[#2b174d] p-12 text-white lg:flex lg:flex-col lg:justify-between">
          <div class="absolute -right-20 -top-20 h-72 w-72 rounded-full bg-[#7c3aed]/25 blur-3xl"></div>
          <div class="relative"><div class="[&_span]:text-white">${brandMark(`/${locale}`, true)}</div><p class="mt-16 text-xs font-semibold uppercase tracking-[.25em] text-[#c4b5fd]">Collecting moments</p><h2 class="mt-4 max-w-md text-5xl font-medium leading-[1.08]">${isRegister ? (el ? "Κάθε event αξίζει τον δικό του χώρο." : "Every event deserves its own space.") : (el ? "Οι στιγμές σου σε περιμένουν." : "Your moments are waiting.")}</h2></div>
        ...
        <div class="flex flex-col p-6 sm:p-10 lg:p-14">
          ...
          <div class="mx-auto my-auto w-full max-w-md py-8">
            ...
            <form id="authForm" class="space-y-4" novalidate>
              ...
              <button id="submit-auth" class="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#2b174d] px-5 py-4 font-semibold text-white transition hover:bg-[#2c473e] disabled:cursor-wait disabled:opacity-65"><span>${isRegister ? m.register : m.login}</span><span aria-hidden="true">→</span></button>
            </form>
