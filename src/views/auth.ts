...
export function authPage(
  ...
  const feature = (icon: string, title: string, description: string) => `<li class="flex gap-4"><span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-lg text-[#675885] shadow-sm">${icon}</span><span><strong class="block text-sm text-[#302b38]">${title}</strong><span class="mt-1 block text-sm leading-6 text-[#756f7c]">${description}</span></span></li>`;

  return page(
    `${isRegister ? m.register : m.login} – Memboux`,
    `<main class="min-h-screen bg-[#f7f6f9] p-4 sm:p-6 lg:flex lg:items-center lg:justify-center">
      <section class="mx-auto grid w-full max-w-6xl overflow-hidden rounded-[1.5rem] border border-[#e4e0e8] bg-white shadow-[0_24px_70px_rgba(41,35,49,.08)] lg:min-h-[720px] lg:grid-cols-[.9fr_1.1fr]">
        <aside class="relative hidden overflow-hidden border-r border-[#e4e0e8] bg-[#f0edf5] p-12 text-[#302b38] lg:flex lg:flex-col lg:justify-between">
          <div class="absolute -right-20 -top-20 h-72 w-72 rounded-full bg-white/70 blur-3xl"></div>
          <div class="relative">${brandMark(`/${locale}`, true)}<p class="mt-16 text-xs font-semibold uppercase tracking-[.25em] text-[#7967bd]">Collecting moments</p><h2 class="mt-4 max-w-md text-5xl font-medium leading-[1.08]">${isRegister ? (el ? "Κάθε event αξίζει τον δικό του χώρο." : "Every event deserves its own space.") : (el ? "Οι στιγμές σου σε περιμένουν." : "Your moments are waiting.")}</h2></div>
        ...
        <div class="flex flex-col p-6 sm:p-10 lg:p-14">
          ...
          <div class="mx-auto my-auto w-full max-w-md py-8">
            <p class="text-xs font-semibold uppercase tracking-[.2em] text-[#7c3aed]">${copy.eyebrow}</p>
            <h1 class="mt-3 text-4xl font-medium tracking-[-.03em] text-[#24143b] sm:text-5xl">${copy.title}</h1>
            <p class="mt-4 text-base leading-7 text-[#6f657c]">${copy.subtitle}</p>
            <div class="mt-8 grid gap-3">
              <button id="google" type="button" class="flex w-full items-center justify-center gap-3 rounded-2xl border border-[#e6dff0] bg-white px-4 py-3.5 font-semibold text-[#2b174d] shadow-sm transition hover:border-[#a78bfa] hover:bg-[#faf8ff] disabled:cursor-wait disabled:opacity-60">${googleIcon()}<span>${m.continueGoogle}</span></button>
            ...
            <div class="my-6 flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[.12em] text-[#94a79f]"><span class="h-px flex-1 bg-[#ece5f4]"></span>${copy.divider}<span class="h-px flex-1 bg-[#ece5f4]"></span></div>
            <form id="authForm" class="space-y-4" novalidate>
              ${isRegister ? `<label class="block text-sm font-medium text-[#493b57]">${m.name}<input name="name" required minlength="2" maxlength="100" autocomplete="name" class="mt-1.5 w-full rounded-2xl border border-[#dcd3e8] px-4 py-3.5 outline-none transition focus:border-[#8b5cf6] focus:ring-4 focus:ring-[#8b5cf6]/10"></label>` : ""}
              <label class="block text-sm font-medium text-[#493b57]">${m.email}<input name="email" type="email" required maxlength="254" autocomplete="email" inputmode="email" class="mt-1.5 w-full rounded-2xl border border-[#dcd3e8] px-4 py-3.5 outline-none transition focus:border-[#8b5cf6] focus:ring-4 focus:ring-[#8b5cf6]/10"></label>
              <label class="block text-sm font-medium text-[#493b57]">${m.password}<span class="relative mt-1.5 block"><input id="password" name="password" type="password" required minlength="10" maxlength="128" autocomplete="${isRegister ? "new-password" : "current-password"}" class="w-full rounded-2xl border border-[#dcd3e8] px-4 py-3.5 pr-24 outline-none transition focus:border-[#8b5cf6] focus:ring-4 focus:ring-[#8b5cf6]/10"><button id="toggle-password" type="button" class="absolute inset-y-0 right-3 my-auto h-9 rounded-xl px-3 text-xs font-semibold text-[#7c3aed]">${copy.show}</button></span></label>
              ${isRegister ? `<div><div class="h-1.5 overflow-hidden rounded-full bg-[#ece5f4]"><span id="password-strength" class="block h-full w-0 rounded-full transition-all"></span></div><p id="password-hint" class="mt-2 text-xs text-[#6f657c]">${copy.passwordHint}</p></div><label class="block text-sm font-medium text-[#493b57]">${copy.confirmPassword}<input id="confirm-password" name="confirmPassword" type="password" required minlength="10" maxlength="128" autocomplete="new-password" class="mt-1.5 w-full rounded-2xl border border-[#dcd3e8] px-4 py-3.5 outline-none transition focus:border-[#8b5cf6] focus:ring-4 focus:ring-[#8b5cf6]/10"></label><label class="flex items-start gap-3 rounded-2xl bg-[#faf8ff] p-4 text-sm leading-6 text-[#675a72]"><input id="terms" name="terms" type="checkbox" required class="mt-1 h-4 w-4 shrink-0 accent-[#7c3aed]"><span>${copy.termsStart} <a href="/${locale}/terms" target="_blank" class="font-semibold text-[#6d28d9]">${copy.terms}</a> ${copy.and} <a href="/${locale}/privacy-policy" target="_blank" class="font-semibold text-[#6d28d9]">${copy.privacy}</a>.</span></label>` : ""}
              <p id="error" role="alert" aria-live="polite" class="hidden rounded-2xl bg-red-50 p-3.5 text-sm text-red-700"></p>
              <button id="submit-auth" class="flex w-full items-center justify-center gap-2 rounded-xl bg-[#7967bd] px-5 py-4 font-semibold text-white transition hover:bg-[#6855ad] disabled:cursor-wait disabled:opacity-65"><span>${isRegister ? m.register : m.login}</span><span aria-hidden="true">→</span></button>
            </form>
            ${!isRegister ? `<a href="/${locale}/forgot-password" class="mt-4 block text-center text-sm font-semibold text-[#6d28d9]">${m.forgotPassword}</a>` : ""}
            <p class="mt-7 text-center text-sm text-[#6f657c]">${isRegister ? m.hasAccount : m.noAccount} <a class="font-semibold text-[#6d28d9]" href="/${locale}/${isRegister ? "login" : "register"}${redirectQuery}">${isRegister ? m.login : m.register}</a></p>
          </div>
        </div>
      </section>
    </main>
