...
export function adminShell(
  ...
  return page(
    `${title} – Memboux Admin`,
    `<style>.admin-ui,.admin-ui button,.admin-ui input,.admin-ui select,.admin-ui textarea{font-family:'Manrope',sans-serif}.admin-ui{font-weight:300}.admin-ui h1,.admin-ui h2,.admin-ui h3{font-family:'Manrope',sans-serif;font-weight:400}.admin-ui strong{font-weight:500}.admin-ui summary::-webkit-details-marker{display:none}</style><div class="admin-ui"><header class="border-b border-[#e4e0e8] bg-white text-[#302b38] shadow-[0_6px_24px_rgba(41,35,49,.045)]"><div class="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">${brandMark("/admin", true)}<div class="flex items-center gap-2"><a href="/admin/language/${locale === "el" ? "en" : "el"}" class="rounded-lg border border-[#ddd7e3] px-3 py-2 text-sm">${locale === "el" ? "EN" : "EL"}</a><details class="relative"><summary class="flex cursor-pointer list-none items-center gap-2 rounded-xl border border-[#ddd7e3] px-4 py-2 text-sm outline-none transition hover:bg-[#f7f6f9] focus-visible:ring-2 focus-visible:ring-[#7967bd]/30"><span class="flex h-7 w-7 items-center justify-center rounded-full bg-[#eeeaf4] text-[#57468e]">${actor ? actor.name.slice(0, 1).toUpperCase() : "A"}</span>${actor ? actor.name : "Admin"} <span>⌄</span></summary><div class="absolute right-0 top-full z-40 mt-2 w-64 rounded-xl border border-[#e4e0e8] bg-white p-2 text-[#302b38] shadow-xl">${identity}${menu}<form action="/admin/logout" method="post" class="mt-1 border-t"><button class="w-full rounded-xl px-3 py-3 text-left text-sm text-red-700 hover:bg-red-50">${locale === "el" ? "Έξοδος από το Admin" : "Exit Admin"}</button></form></div></details></div></div></header>${routedContent}</div>`,
    { locale },
  );
}
