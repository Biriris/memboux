...
export function renderWeddingPage(input: {
  ...
  const ceremonyTimestamp = profile.ceremony_at ? new Date(profile.ceremony_at).getTime() : Number.NaN;
  const hasCountdown = Number.isFinite(ceremonyTimestamp) && ceremonyTimestamp > Date.now();
  const features = new Set(selectedFeatures);
  const calendarAction = (kind: "ceremony" | "reception", startsAt: string | null) =>
    features.has("calendar_links") && startsAt
      ? `<a class="w-calendar-link" href="${demo ? "#schedule" : `/wedding/${encodeURIComponent(event.code)}/calendar/${kind}.ics?lang=${locale}`}">${esc(t(locale, "Add to calendar", "Προσθήκη στο ημερολόγιο", "Ajouter au calendrier", "Zum Kalender hinzufügen", "Añadir al calendario", "Aggiungi al calendario"))} <i>＋</i></a>`
      : "";
  ...
  const previewQuery = preview
    ? `&preview=1&theme=${encodeURIComponent(theme.key)}`
    : "";
  const languageUrl = (value: Locale) => demo
    ? `/${value}/wedding/demo-frame?theme=${encodeURIComponent(theme.key)}`
    : `/wedding/${encodeURIComponent(event.code)}?lang=${value}${previewQuery}`;
  const languageLabel = t(locale, "Language", "Γλώσσα", "Langue", "Sprache", "Idioma", "Lingua");
  ...
  const countdown = hasCountdown
    ? `<section id="countdown" class="w-section w-countdown" data-countdown-target="${ceremonyTimestamp}"><div class="w-inner" data-reveal><p class="w-eyebrow">${esc(t(locale, "Counting every moment", "Μετρώντας κάθε στιγμή", "Chaque instant compte", "Jeder Moment zählt", "Contando cada momento", "Contando ogni momento"))}</p><h2>${esc(t(locale, "Until we say “I do”", "Μέχρι να πούμε «ναι»", "Jusqu’à notre « oui »", "Bis zu unserem Ja", "Hasta nuestro «sí»", "Fino al nostro «sì»"))}</h2><div class="w-countdown-grid" role="timer" aria-live="off"><div><strong data-countdown-value="days">00</strong><span>${esc(t(locale, "Days", "Ημέρες", "Jours", "Tage", "Días", "Giorni"))}</span></div><div><strong data-countdown-value="hours">00</strong><span>${esc(t(locale, "Hours", "Ώρες", "Heures", "Stunden", "Horas", "Ore"))}</span></div><div><strong data-countdown-value="minutes">00</strong><span>${esc(t(locale, "Minutes", "Λεπτά", "Minutes", "Minuten", "Minutos", "Minuti"))}</span></div><div><strong data-countdown-value="seconds">00</strong><span>${esc(t(locale, "Seconds", "Δευτερόλεπτα", "Secondes", "Sekunden", "Segundos", "Secondi"))}</span></div></div></div></section>`
    : "";
  const countdownScript = hasCountdown ? `<script>(()=>{const section=document.querySelector('[data-countdown-target]');if(!section)return;const target=Number(section.dataset.countdownTarget),values=Object.fromEntries([...section.querySelectorAll('[data-countdown-value]')].map(node=>[node.dataset.countdownValue,node]));let timer;const render=()=>{const remaining=Math.max(0,target-Date.now()),total=Math.floor(remaining/1000),days=Math.floor(total/86400),hours=Math.floor(total%86400/3600),minutes=Math.floor(total%3600/60),seconds=total%60;values.days.textContent=String(days).padStart(2,'0');values.hours.textContent=String(hours).padStart(2,'0');values.minutes.textContent=String(minutes).padStart(2,'0');values.seconds.textContent=String(seconds).padStart(2,'0');if(remaining<=0&&timer)clearInterval(timer)};render();timer=setInterval(render,1000)})()<\/script>` : "";
  const draftBanner = preview ? `<aside class="w-preview"><strong>${esc(t(locale, "Private preview", "Ιδιωτική προεπισκόπηση", "Aperçu privé", "Private Vorschau", "Vista previa privada", "Anteprima privata"))}</strong><a href="/dashboard/${event.code}/wedding/setup?lang=${locale}&step=1">${esc(t(locale, "Edit website", "Επεξεργασία σελίδας", "Modifier le site", "Website bearbeiten", "Editar sitio", "Modifica sito"))}</a></aside>` : "";

  const scheduleCards = [
    ceremony || profile.ceremony_location ? `<article class="w-event-card"><span>01</span><p>${esc(t(locale, "Ceremony", "Τελετή", "Cérémonie", "Zeremonie", "Ceremonia", "Cerimonia"))}</p>${ceremony ? `<h3>${esc(ceremony)}</h3>` : ""}${profile.ceremony_location ? `<a href="${esc(mapUrl(profile.ceremony_location, profile.ceremony_lat, profile.ceremony_lng))}" target="_blank" rel="noopener">${esc(profile.ceremony_location)} <i>↗</i></a>` : ""}${calendarAction("ceremony", profile.ceremony_at)}</article>` : "",
  ...
  const body = `${draftBanner}<div id="top" class="w-page" data-wedding-theme="${theme.key}" data-wedding-layout="${theme.layout}" data-wedding-font="${theme.font}" data-wedding-name-scale="${nameScale}" style="--w-accent:${accent};--w-ink:${theme.palette[0]};--w-soft:${theme.palette[1]};--w-bg:${theme.palette[2]};--w-monogram:'${esc(monogram)}'">
  <style>
  ...
  <div class="w-top" role="navigation" aria-label="${esc(navigationLabel)}">${brandMark(demo ? "#top" : `/wedding/${event.code}?lang=${locale}`, true, true)}<nav class="w-nav">${nav.map(([id, label]) => `<a href="#${id}">${esc(label)}</a>`).join("")}</nav>${languagePicker}</div>
  <section class="w-hero" data-luxury-hero>${heroImage}<div class="w-hero-copy" data-reveal><p class="w-kicker">${esc(t(locale, "We are getting married", "Παντρευόμαστε", "Nous nous marions", "Wir heiraten", "Nos casamos", "Ci sposiamo"))}</p><h1>${esc(names)}</h1>${profile.welcome_message ? `<p class="w-hero-message">${esc(profile.welcome_message)}</p>` : ""}${ceremony ? `<p class="w-date">${esc(ceremony)}</p>` : ""}<a class="w-scroll" href="#${hasStory ? "story" : hasSchedule ? "schedule" : hasMenu ? "menu" : "moments"}" aria-label="${esc(scrollLabel)}">↓</a></div></section>
  ...
  ${divider1}
  ${hasCountdown ? `${ornament(1)}${countdown}${ornament(2)}` : ""}
  ${hasSchedule ? `<section id="schedule" class="w-section"><div class="w-inner" data-reveal><p class="w-eyebrow">${esc(t(locale, "Ceremony & reception", "Τελετή & δεξίωση", "Cérémonie et réception", "Zeremonie & Empfang", "Ceremonia y recepción", "Cerimonia e ricevimento"))}</p><h2>${esc(t(locale, "Our wedding day", "Η ημέρα του γάμου μας", "Le jour de notre mariage", "Unser Hochzeitstag", "El día de nuestra boda", "Il giorno del nostro matrimonio"))}</h2><div class="w-schedule-grid">${scheduleCards}</div>${profile.dress_code ? `<p class="w-hero-message" style="color:inherit;text-align:center">${esc(profile.dress_code)}</p>` : ""}</div></section>` : ""}
  ${divider2}
  ${preWeddingGallery}
  ...
  const renderedBody = experienceHtml
    ...
    : body;
  return page(names, `${renderedBody}${countdownScript}${experienceScripts}`, {
    locale,
    description: profile.welcome_message || t(locale, "A private wedding experience on Memboux.", "Μια ιδιωτική εμπειρία γάμου στο Memboux.", "Une expérience de mariage privée sur Memboux.", "Eine private Hochzeitserfahrung auf Memboux.", "Una experiencia de boda privada en Memboux.", "Un’esperienza di matrimonio privata su Memboux."),
    canonical: demo ? undefined : `https://memboux.com/wedding/${event.code}`,
    alternates: demo ? undefined : Object.fromEntries(supportedLocales.map((value) => [value, `https://memboux.com/wedding/${event.code}?lang=${value}`])),
    index: false,
    image: coverUpdatedAt ? `https://memboux.com/gallery/${event.code}/cover?v=${coverUpdatedAt}` : undefined,
    additionalHead: `<link href="https://fonts.googleapis.com/css2?family=EB+Garamond:ital,wght@0,400;0,500;1,400&family=GFS+Didot&family=Noto+Sans:wght@300;400;500;600;700&family=Noto+Serif:ital,wght@0,300;0,400;1,300&display=swap" rel="stylesheet">${demo ? "<style>html{scroll-behavior:smooth}</style>" : ""}`,
    suppressWidgets: demo,
  });
}
