...
function weddingDemoExperience(locale: Locale, media: readonly DemoMediaAsset[]) {
  ...
  return `<style>
    ...
    .w-demo-gallery{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.65rem;margin-top:2rem}.w-demo-photo{position:relative;display:grid;min-height:clamp(8rem,22vw,18rem);place-items:end start;overflow:hidden;background:var(--w-ink);color:#fff;font-size:.7rem;font-weight:750;letter-spacing:.12em;text-transform:uppercase}.w-demo-photo picture{position:absolute;inset:0}.w-demo-photo img{width:100%;height:100%;object-fit:cover}.w-demo-photo:after{content:"";position:absolute;inset:30% 0 0;background:linear-gradient(transparent,#090909c7)}.w-demo-photo figcaption{position:relative;z-index:1;padding:1rem;text-shadow:0 1px 8px #0009}
...
export function weddingDemoFrame(locale: Locale, theme: WeddingThemeKey) {
  ...
  const demoMedia = demoMediaFor("wedding", locale);
  ...
  return renderWeddingPage({
    ...
    coverUpdatedAt: null,
    experienceHtml: weddingDemoExperience(locale, demoMedia),
    demoMedia,
    demo: true,
  });
}
...
export function weddingDemoPage(locale: Locale, initialTheme: WeddingThemeKey = "cypress") {
  ...
  const registerUrl = `/${locale}/register?redirect=${encodeURIComponent(`/${locale}/account?create=wedding&template=${initialTheme}`)}`;
  ...
  const selectionScript = `<script>(()=>{const links=[...document.querySelectorAll('a[href^="/${locale}/register?redirect="]')],sync=theme=>{const account='/${locale}/account?create=wedding&template='+encodeURIComponent(theme),registration='/${locale}/register?redirect='+encodeURIComponent(account);links.forEach(link=>link.href=registration)};document.querySelectorAll('[data-wedding-demo-theme]').forEach(button=>button.addEventListener('click',()=>sync(button.dataset.weddingDemoTheme)));sync(${JSON.stringify(initialTheme)})})()<\/script>`;
  return page("Wedding previews | Memboux", body + selectionScript, {
    locale,
