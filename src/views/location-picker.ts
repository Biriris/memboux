...
export function locationPickerMarkup(options: {
  ...
  return `<div data-location-picker data-initial-location="${esc(value)}" data-initial-place-id="${esc(placeId)}" data-initial-latitude="${esc(latitude)}" data-initial-longitude="${esc(longitude)}" class="relative ${compact ? "min-w-0 flex-1" : "mt-2"}">
    <input id="${esc(id)}" data-location-input name="${esc(inputName)}" maxlength="200" autocomplete="off" value="${esc(value)}" placeholder="${el ? "Αναζήτησε εκκλησία, χώρο, επιχείρηση ή διεύθυνση" : "Search for a church, venue, business or address"}" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="${esc(id)}-results" class="w-full rounded-xl border border-[#e2dcef] bg-white px-4 py-3 pr-11 font-normal text-[#2b174d] outline-none focus:border-[#8b5cf6] focus:ring-2 focus:ring-[#ddcff5]">
    ...
    <button data-location-reset type="button" aria-label="${el ? "Καθαρισμός τοποθεσίας" : "Clear location"}" class="${value ? "flex" : "hidden"} absolute right-2 top-1.5 h-9 w-9 cursor-pointer items-center justify-center rounded-full text-xl text-[#756b82] hover:bg-[#f6f2fc]">×</button>
    <div id="${esc(id)}-results" data-location-results role="listbox" class="absolute left-0 right-0 z-50 mt-2 hidden overflow-hidden rounded-2xl border border-[#e2dcef] bg-white p-1 shadow-2xl"></div>
    <div class="mt-2 flex flex-wrap items-center gap-2"><button data-location-map-open type="button" class="inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-xl border border-[#e2dcef] bg-white px-3.5 py-2 text-xs font-semibold text-[#7c3aed] transition hover:border-[#c4b5fd] hover:bg-[#f8f5ff]"><span aria-hidden="true">⌖</span><span>${el ? "Επιλογή από χάρτη" : "Choose on map"}</span></button><span data-location-coordinate-label class="${latitude && longitude ? "inline-flex" : "hidden"} rounded-full bg-[#f4effc] px-3 py-1.5 text-[11px] font-semibold text-[#685775]">${latitude && longitude ? `${esc(latitude)}, ${esc(longitude)}` : ""}</span></div>
    <div data-location-map-panel class="mt-3 hidden overflow-hidden rounded-2xl border border-[#e2dcef] bg-[#f3effa] p-2 shadow-inner"><div data-location-map-canvas class="${largeMap ? "h-[22rem] sm:h-[30rem]" : "h-72"} w-full overflow-hidden rounded-xl bg-[#eae4f3]" role="application" aria-label="${el ? "Χάρτης επιλογής τοποθεσίας" : "Location selection map"}"></div><p class="px-2 pb-1 pt-2 text-[11px] leading-5 text-[#64766f]">${el ? "Πάτησε στο ακριβές σημείο. Χρήσιμο για εκκλησίες, κτήματα ή χώρους χωρίς πλήρη διεύθυνση." : "Tap the exact point. Useful for churches, estates, or venues without a complete address."}</p></div>
    <p data-location-status class="mt-2 flex items-center gap-1.5 text-xs ${hasLockedLocation ? "font-semibold text-emerald-700" : "text-[#756b82]"}"><span aria-hidden="true">${hasLockedLocation ? "✓" : "⌖"}</span><span>${status}</span></p>
  </div>`;
}
...
export function locationPickerScript(locale: Locale) {
  ...
  const setup = `
    ...
    const setStatus=(message,verified=false)=>{status.lastElementChild.textContent=message;status.firstElementChild.textContent=verified?'✓':'⌖';status.className='mt-2 flex items-center gap-1.5 text-xs '+(verified?'font-semibold text-emerald-700':'text-[#756b82]')};
    ...
    const attribution=()=>{const item=document.createElement('div');item.className='border-t border-[#f3effa] px-3 py-2 text-right text-xs font-normal text-[#5e5e5e]';item.setAttribute('translate','no');item.textContent='Google Maps';return item};
    const render=items=>{results.replaceChildren();items.forEach(item=>{const button=document.createElement('button');button.type='button';button.role='option';button.className='block w-full cursor-pointer rounded-xl px-3 py-2.5 text-left text-sm text-[#49395a] hover:bg-[#f2f7f4] focus:bg-[#f2ecff]';button.textContent=item.label;button.addEventListener('mousedown',event=>event.preventDefault());button.addEventListener('click',()=>choose(item));results.append(button)});results.append(attribution());results.classList.remove('hidden');input.setAttribute('aria-expanded','true')};
    const showMessage=(message,error=false)=>{results.replaceChildren();const line=document.createElement('p');line.className='px-3 py-3 text-sm '+(error?'text-red-700':'text-[#756b82]');line.textContent=message;results.append(line);if(!error)results.append(attribution());results.classList.remove('hidden')};
