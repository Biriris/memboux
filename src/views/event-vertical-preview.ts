...
export function eventVerticalPreviewPage(
  ...
  options: { demo?: boolean; guestExperienceOpen?: boolean; originalDownloads?: boolean; bulkDownloads?: boolean; guestItems?: readonly MediaCardRow[]; coverUpdatedAt?: number | null } = {},
) {
  ...
  const guestAlbumScripts = !options.demo && guestItems.length
    ? `${galleryFilterScript(guestItems, "event-guest-gallery")}${galleryProgressiveScript("event-guest-gallery")}${lightboxMarkup(locale, true, options.originalDownloads === true)}${options.originalDownloads === true ? directDownloadButtonsScript(locale, "#guest-album") : ""}${mediaLikesScript(event.code, locale)}`
    : "";
