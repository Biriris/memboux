...
function demoExperience(locale: Locale, vertical: EventVertical, card: string, accent: string) {
  ...
  return `<style>
    ...
    .v-demo-photo{display:flex;grid-column:span 4;align-items:flex-end;overflow:hidden;padding:1rem;color:#fff;font-size:.68rem;font-weight:800;letter-spacing:.14em;text-transform:uppercase}
  ...
  <section id="demo-gallery" class="v-demo-section" style="background:${card}">
    <div class="v-demo-inner"><p class="v-demo-kicker" style="color:${accent}">${esc(text({ en: "Shared album", el: "Κοινό album", fr: "Album partagé", de: "Gemeinsames Album", es: "Álbum compartido", it: "Album condiviso" }))}</p><h2 class="v-demo-title">${esc(text({ en: "The event through everyone’s eyes.", el: "Το event μέσα από τα μάτια όλων.", fr: "L’événement vu par tous.", de: "Das Event durch die Augen aller.", es: "El evento a través de todos.", it: "L’evento attraverso gli occhi di tutti." }))}</h2><div class="v-demo-gallery">${contributors.map((name, index) => `<div class="v-demo-photo" style="background:${index === 0 ? `linear-gradient(145deg,${accent},#251547)` : index === 1 ? `linear-gradient(35deg,#251547,${vertical.soft})` : `linear-gradient(160deg,${vertical.soft},${accent})`}">${esc(name)}</div>`).join("")}</div></div>
  </section>
