...
type WeddingExperienceInput = {
  ...
  guestMediaCount?: { total: number; photos: number; videos: number };
...
export function renderWeddingExperience(input: WeddingExperienceInput) {
  ...
  const guestbookOnlySettings = { ...settings, rsvp_enabled: 0 };
  ...
  const officialTitle = tr(locale, { en: "The professional album", el: "Το επαγγελματικό άλμπουμ", fr: "L’album professionnel", de: "Das professionelle Album", es: "El álbum profesional", it: "L’album professionale" });
  const liveTitle = tr(locale, { en: "Live moments", el: "Ζωντανές στιγμές", fr: "Moments en direct", de: "Live-Momente", es: "Momentos en directo", it: "Momenti live" });
  const liveEmpty = tr(locale, { en: "New uploads will appear here automatically.", el: "Οι νέες φωτογραφίες και τα βίντεο θα εμφανίζονται εδώ αυτόματα.", fr: "Les nouveaux ajouts apparaîtront automatiquement ici.", de: "Neue Beiträge erscheinen hier automatisch.", es: "Las nuevas fotos aparecerán aquí automáticamente.", it: "I nuovi contenuti appariranno qui automaticamente." });
  const guestExperienceKicker = tr(locale, { en: "Guest experience", el: "Εμπειρία καλεσμένων", fr: "Expérience invités", de: "Gästeerlebnis", es: "Experiencia de invitados", it: "Esperienza ospiti" });
  const guestUploadsKicker = tr(locale, { en: "Guest uploads", el: "Περιεχόμενο καλεσμένων", fr: "Contenu des invités", de: "Beiträge der Gäste", es: "Contenido de invitados", it: "Contenuti degli ospiti" });
  const shareKicker = tr(locale, { en: "QR & Share", el: "QR & κοινοποίηση", fr: "QR et partage", de: "QR & Teilen", es: "QR y compartir", it: "QR e condivisione" });
  const guestMomentsKicker = tr(locale, { en: "Guest moments", el: "Στιγμές καλεσμένων", fr: "Moments des invités", de: "Gästemomente", es: "Momentos de invitados", it: "Momenti degli ospiti" });
  const nameLabel = tr(locale, { en: "Your name", el: "Το όνομά σου", fr: "Votre nom", de: "Dein Name", es: "Tu nombre", it: "Il tuo nome" });
  const chooseMediaLabel = tr(locale, { en: "Choose photos or videos", el: "Επίλεξε φωτογραφίες ή βίντεο", fr: "Choisir des photos ou vidéos", de: "Fotos oder Videos auswählen", es: "Elegir fotos o vídeos", it: "Scegli foto o video" });
  const officialSection = officialItems.length ? `<section id="official-album" class="w-section w-integrated"><div class="w-inner"><p class="w-eyebrow">Memboux Studio</p><div class="w-section-head"><div><h2>${esc(officialTitle)}</h2><p class="w-story-copy">${esc(tr(locale, { en: `Curated by ${input.curatorName}.`, el: `Σε επιμέλεια ${input.curatorName}.`, fr: `Sélectionné par ${input.curatorName}.`, de: `Kuratiert von ${input.curatorName}.`, es: `Seleccionado por ${input.curatorName}.`, it: `A cura di ${input.curatorName}.` }))}</p></div><span class="w-count">${officialItems.length}</span></div><div class="mt-8 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">${cards(officialItems, { lightbox: true, locale, likes: true })}</div></div></section>` : "";
  const shareSection = `<section id="guest-share" class="w-section w-integrated"><div class="w-inner"><article class="w-integrated-card w-share-card"><p class="w-card-kicker">${esc(shareKicker)}</p><h3>${esc(tr(locale, { en: "Invite more guests", el: "Κάλεσε και άλλους", fr: "Invitez d’autres personnes", de: "Weitere Gäste einladen", es: "Invita a más personas", it: "Invita altri ospiti" }))}</h3><div class="w-qr">${guestQrSvg}</div>${shareIconButtons(guestUrl, eventName, locale, false)}<button id="copy-wedding-link" type="button" class="w-integrated-button secondary w-full">${esc(tr(locale, { en: "Copy event link", el: "Αντιγραφή συνδέσμου εκδήλωσης", fr: "Copier le lien", de: "Event-Link kopieren", es: "Copiar enlace", it: "Copia link" }))}</button></article></div></section>`;

  const html = `<section id="guest-experience" class="w-section w-integrated"><div class="w-inner"><p class="w-eyebrow">${esc(guestExperienceKicker)}</p><h2>${esc(tr(locale, { en: "Everything happens here", el: "Όλα συμβαίνουν εδώ", fr: "Tout se passe ici", de: "Alles passiert hier", es: "Todo sucede aquí", it: "Tutto accade qui" }))}</h2><p class="w-story-copy">${esc(tr(locale, { en: "Upload, discover wishes and celebrate without leaving the event page.", el: "Ανέβασε στιγμές, διάβασε ευχές και γιόρτασε χωρίς να φύγεις από τη σελίδα της εκδήλωσης.", fr: "Ajoutez des moments, découvrez les vœux et célébrez sans quitter la page.", de: "Momente hochladen, Wünsche entdecken und feiern – alles auf einer Seite.", es: "Sube momentos, descubre los deseos y celebra sin salir de la página.", it: "Carica momenti, scopri gli auguri e festeggia senza lasciare la pagina." }))}</p><div class="w-guest-grid"><article id="guest-upload" class="w-integrated-card"><p class="w-card-kicker">${esc(guestUploadsKicker)}</p><h3>${esc(uploadTitle)}</h3><p>${esc(uploadCopy)}</p><form data-multi-upload action="/api/upload/${encodeURIComponent(code)}" method="post" enctype="multipart/form-data" class="mt-6 space-y-3"><input type="hidden" name="locale" value="${locale}"><input name="name" maxlength="60" aria-label="${esc(nameLabel)}" placeholder="${esc(nameLabel)}" class="w-full rounded-xl border px-4 py-3"><label class="w-integrated-button w-full cursor-pointer"><span>${esc(chooseMediaLabel)}</span><input name="file" required multiple type="file" accept="${UPLOAD_ACCEPT}" aria-label="${esc(chooseMediaLabel)}" class="sr-only"></label><p class="text-xs opacity-70">${esc(uploadLimitsCopy(locale))}</p><label class="flex cursor-pointer items-start gap-3 border border-current/10 bg-white/60 p-4 text-sm leading-6"><input name="upload_confirmation" value="accepted" required type="checkbox" class="mt-1 h-4 w-4 shrink-0"><span>${esc(confirmation)}</span></label><button class="w-integrated-button w-full">${esc(tr(locale, { en: "Upload to album", el: "Προσθήκη στο άλμπουμ", fr: "Ajouter à l’album", de: "Zum Album hochladen", es: "Subir al álbum", it: "Carica nell’album" }))}</button></form></article></div></div></section>
  <section id="guest-moments" class="w-section w-integrated w-integrated-alt"><div class="w-inner"><div class="w-section-head"><div><p class="w-eyebrow">${esc(guestMomentsKicker)}</p><h2>${esc(tr(locale, { en: "Shared gallery", el: "Κοινό άλμπουμ", fr: "Galerie partagée", de: "Gemeinsame Galerie", es: "Galería compartida", it: "Galleria condivisa" }))}</h2>${galleryFilterControls(guestItems, "wedding-guest-gallery", locale, input.guestMediaCount)}</div><div class="w-select-actions"><button id="wedding-select-media" type="button">${esc(selectText)}</button><button id="wedding-download-selected" type="button" class="hidden">${esc(downloadText)}</button></div></div>${guestItems.length ? `<div data-gallery-grid="wedding-guest-gallery" class="mt-8 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">${cards(guestItems, { selectable: true, deferredSelection: true, lightbox: true, reportCode: code, locale, likes: true })}</div>${galleryProgressiveControls(input.guestMediaCount?.total ?? guestItems.length, "wedding-guest-gallery", locale, 24, `/api/gallery/${encodeURIComponent(code)}/media-page?lang=${locale}&surface=website`)}` : `<p class="w-empty">${esc(emptyGuest)}</p>`}</div></section>
  ${renderGuestParticipation(code, guestbookEntries, locale, guestbookOnlySettings)}
  ${officialSection}
