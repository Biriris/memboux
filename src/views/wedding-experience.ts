...
export function renderWeddingExperience(input: WeddingExperienceInput) {
  ...
  <section id="guest-moments" class="w-section w-integrated w-integrated-alt"><div class="w-inner"><div class="w-section-head"><div><p class="w-eyebrow">${esc(guestMomentsKicker)}</p><h2>${esc(tr(locale, { en: "Shared gallery", el: "Κοινό άλμπουμ", fr: "Galerie partagée", de: "Gemeinsame Galerie", es: "Galería compartida", it: "Galleria condivisa" }))}</h2>${galleryFilterControls(guestItems, "wedding-guest-gallery", locale)}</div><div class="w-select-actions"><button id="wedding-select-media" type="button">${esc(selectText)}</button><button id="wedding-download-selected" type="button" class="hidden">${esc(downloadText)}</button></div></div>${guestItems.length ? `<div data-gallery-grid="wedding-guest-gallery" class="mt-8 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">${cards(guestItems, { selectable: true, deferredSelection: true, lightbox: true, reportCode: code, locale, likes: true, deferAfter: 12 })}</div>${galleryProgressiveControls(guestItems.length, "wedding-guest-gallery", locale)}` : `<p class="w-empty">${esc(emptyGuest)}</p>`}</div></section>
