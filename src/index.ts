...
function cards(items: MediaRow[], options?: { code?: string; locale?: Locale; selectable?: boolean; manage?: boolean; lightbox?: boolean }) {
  return items.map((m) => {
    const media = m.media_type === "image" ? `<img src="/media/${encodeURIComponent(m.id)}" alt="${esc(m.title || m.uploaded_by)}" loading="lazy" class="h-full w-full object-cover">` : `<video src="/media/${encodeURIComponent(m.id)}" ${options?.lightbox ? "muted" : "controls"} preload="metadata" class="h-full w-full object-cover"></video>`;
    const content = options?.lightbox ? `<button type="button" class="lightbox-item block aspect-square w-full" data-src="/media/${encodeURIComponent(m.id)}" data-type="${m.media_type}" data-title="${esc(m.title || m.uploaded_by)}">${media}</button>` : options?.manage && options.code ? `<a href="/dashboard/${encodeURIComponent(options.code)}/media/${encodeURIComponent(m.id)}?lang=${options.locale ?? "en"}" class="block aspect-square">${media}</a>` : `<a href="/media/${encodeURIComponent(m.id)}" class="block aspect-square">${media}</a>`;
...
function lightboxMarkup(locale: Locale) {
  return `<dialog id="media-lightbox" class="h-screen w-screen max-h-none max-w-none bg-transparent p-0 backdrop:bg-black/85"><div class="relative flex h-full w-full items-center justify-center p-4 sm:p-12"><button id="lightbox-close" class="absolute right-4 top-4 z-20 flex h-11 w-11 items-center justify-center rounded-full bg-white/15 text-3xl text-white">×</button><button id="lightbox-prev" class="absolute left-3 z-20 flex h-12 w-12 items-center justify-center rounded-full bg-white/15 text-3xl text-white sm:left-6">‹</button><div id="lightbox-content" class="flex max-h-full max-w-full flex-col items-center"></div><button id="lightbox-next" class="absolute right-3 z-20 flex h-12 w-12 items-center justify-center rounded-full bg-white/15 text-3xl text-white sm:right-6">›</button><p id="lightbox-title" class="absolute bottom-4 left-1/2 max-w-[70vw] -translate-x-1/2 truncate rounded-full bg-black/45 px-5 py-2 text-sm text-white"></p></div></dialog><script>(()=>{const items=[...document.querySelectorAll('.lightbox-item')],dialog=document.getElementById('media-lightbox'),content=document.getElementById('lightbox-content'),title=document.getElementById('lightbox-title');let index=0;const show=i=>{index=(i+items.length)%items.length;const item=items[index],src=item.dataset.src;content.innerHTML=item.dataset.type==='video'?'<video src="'+src+'" controls autoplay class="max-h-[80vh] max-w-[86vw] rounded-xl"></video>':'<img src="'+src+'" alt="" class="max-h-[80vh] max-w-[86vw] rounded-xl object-contain">';title.textContent=item.dataset.title||''};items.forEach((item,i)=>item.onclick=()=>{show(i);dialog.showModal()});document.getElementById('lightbox-close').onclick=()=>dialog.close();document.getElementById('lightbox-prev').onclick=()=>show(index-1);document.getElementById('lightbox-next').onclick=()=>show(index+1);dialog.onclick=e=>{if(e.target===dialog)dialog.close()};document.addEventListener('keydown',e=>{if(!dialog.open)return;if(e.key==='ArrowLeft')show(index-1);if(e.key==='ArrowRight')show(index+1);if(e.key==='Escape')dialog.close()})})()<\/script>`;
}
...
app.get("/:locale{el|en}/account", async (c) => {
  ...
  const eventCards = events.results.map(event=>`<article class="group relative rounded-2xl border bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"><a href="/dashboard/${event.code}?lang=${locale}" class="block rounded-2xl p-5 pr-16"><div class="flex items-start justify-between gap-3"><div><p class="font-mono text-sm text-[#8a654f]">${esc(event.code)}</p><h2 class="mt-1 text-2xl">${esc(event.eventName)}</h2></div><span class="rounded-full bg-[#f1e8e1] px-2.5 py-1 text-xs">${event.role==="owner"?(locale==="el"?"Ιδιοκτήτης":"Owner"):(locale==="el"?"Συνεργάτης":"Collaborator")}</span></div><p class="mt-2 text-sm font-medium text-[#76533d]">${esc(formatEventDates(event,locale))}</p><p class="mt-2 text-sm text-[#776a63]">${event.media_count} uploads</p></a>${event.role==="owner"?`<details class="absolute right-3 top-3 z-10"><summary class="flex h-10 w-10 cursor-pointer list-none items-center justify-center rounded-full bg-white text-2xl shadow-sm hover:bg-[#f7f3ed]" aria-label="Event actions">⋯</summary><div class="absolute right-0 mt-2 w-44 rounded-2xl border bg-white p-2 shadow-xl"><a href="/dashboard/${event.code}/edit?lang=${locale}" class="block rounded-xl px-3 py-2 text-sm hover:bg-[#f7f3ed]">${locale==="el"?"Επεξεργασία":"Edit"}</a><form action="/api/account/events/${event.code}/trash" method="post" onsubmit="return confirm('${locale==="el"?"Μεταφορά του event στον κάδο;":"Move this event to trash?"}')"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl px-3 py-2 text-left text-sm text-red-700 hover:bg-red-50">${locale==="el"?"Διαγραφή":"Delete"}</button></form></div></details>`:""}</article>`).join("");
...
app.get("/dashboard/:code", async(c)=>{
  ...
  const items=await getMedia(c.env.DB,event.id);
  return c.html(page(event.eventName,`<header class="border-b bg-white"><div class="mx-auto flex max-w-6xl items-center justify-between gap-3 p-5">${brandMark(`/${locale}`,true)}<div class="flex items-center gap-2"><a href="/${locale}/account" class="rounded-xl border px-4 py-2 text-sm">← ${locale==="el"?"Τα events μου":"My events"}</a>${membership.role==="owner"?`<a href="/dashboard/${event.code}/edit?lang=${locale}" class="rounded-xl bg-[#76533d] px-4 py-2 text-sm text-white">Edit</a>`:""}</div></div></header><main class="mx-auto max-w-6xl p-5 md:p-10"><section class="mb-8 text-center"><p class="text-xs uppercase tracking-[.25em] text-[#9b725c]">Collect All Moments</p><h1 class="mt-3 text-5xl md:text-6xl">${esc(event.eventName)}</h1><p class="mt-3 text-lg text-[#76533d]">${esc(formatEventDates(event,locale))}</p></section><section class="rounded-3xl bg-white p-5 shadow-lg sm:p-7"><div class="mb-5 flex items-center justify-between"><h2 class="text-3xl">Gallery</h2><span class="text-sm text-[#776a63]">${items.length} ${locale==="el"?"αρχεία":"items"}</span></div>${items.length?`<div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">${cards(items,{lightbox:true})}</div>`:`<p class="py-16 text-center text-[#776a63]">${locale==="el"?"Δεν υπάρχουν φωτογραφίες ακόμη.":"No photos yet."}</p>`}</section></main>${lightboxMarkup(locale)}${logoutScript(locale)}`));
});
...
app.get("/dashboard/:code/edit", async(c)=>{
  ...
  const memberRows=members.map(member=>`<div class="flex items-center justify-between gap-3 rounded-2xl border p-4"><div class="min-w-0"><p class="truncate font-medium">${esc(member.name)}</p><p class="truncate text-sm text-[#776a63]">${esc(member.email)}</p></div>${member.role==="owner"?`<span class="rounded-full bg-[#f1e8e1] px-3 py-1 text-xs">Owner</span>`:`<form action="/api/account/events/${event.code}/members/remove" method="post"><input type="hidden" name="userId" value="${esc(member.user_id)}"><input type="hidden" name="locale" value="${locale}"><button class="text-sm text-red-700">${locale==="el"?"Αφαίρεση":"Remove"}</button></form>`}</div>`).join("");
  const inviteRows=invitations.map(invite=>`<div class="flex items-center justify-between gap-3 rounded-2xl border border-dashed p-4"><div class="min-w-0"><p class="truncate">${esc(invite.email)}</p><p class="text-xs text-[#776a63]">${locale==="el"?"Σε αναμονή":"Pending"}</p></div><form action="/api/account/events/${event.code}/members/remove" method="post"><input type="hidden" name="invitationId" value="${invite.id}"><input type="hidden" name="locale" value="${locale}"><button class="text-sm text-red-700">${locale==="el"?"Ακύρωση":"Cancel"}</button></form></div>`).join("");
  return c.html(page(`${event.eventName} – Edit`,`<header class="border-b bg-white"><div class="mx-auto flex max-w-5xl items-center justify-between p-5">${brandMark(`/${locale}`,true)}<div class="flex gap-2"><a href="/${locale}/account" class="rounded-xl border px-4 py-2 text-sm">← ${locale==="el"?"Τα events μου":"My events"}</a><a href="/dashboard/${event.code}?lang=${locale}" class="rounded-xl bg-[#76533d] px-4 py-2 text-sm text-white">${locale==="el"?"Προβολή":"Open"}</a></div></div></header><main class="mx-auto max-w-5xl p-5 md:p-10"><div><p class="text-xs uppercase tracking-[.2em] text-[#8a654f]">Event settings</p><h1 class="text-4xl">${locale==="el"?"Επεξεργασία event":"Edit event"}</h1></div><section class="mt-6 rounded-3xl bg-white p-6 shadow"><h2 class="text-3xl">${locale==="el"?"Τίτλος και ημερομηνίες":"Title and dates"}</h2><form action="/api/account/events/${event.code}/details" method="post" class="mt-5 grid gap-4 md:grid-cols-2"><input type="hidden" name="locale" value="${locale}"><label class="md:col-span-2">${locale==="el"?"Τίτλος":"Title"}<input name="eventName" required maxlength="100" value="${esc(event.eventName)}" class="mt-1 w-full rounded-xl border px-4 py-3"></label><label>${locale==="el"?"Έναρξη":"Start date"}<input name="eventStartDate" type="date" required value="${esc(event.event_start_date||"")}" class="mt-1 w-full rounded-xl border px-4 py-3"></label><label>${locale==="el"?"Λήξη":"End date"}<input name="eventEndDate" type="date" value="${esc(event.event_end_date||"")}" class="mt-1 w-full rounded-xl border px-4 py-3"></label><button class="rounded-xl bg-[#76533d] py-3 text-white md:col-span-2">${locale==="el"?"Αποθήκευση":"Save changes"}</button></form></section><section class="mt-6 rounded-3xl bg-white p-6 shadow"><div class="grid gap-8 md:grid-cols-2"><div><h2 class="text-3xl">${locale==="el"?"Συνεργάτες":"Collaborators"}</h2><div class="mt-4 space-y-3">${memberRows}${inviteRows}</div></div><div class="rounded-2xl bg-[#faf6f1] p-5"><h2 class="text-3xl">${locale==="el"?"Πρόσκληση":"Invite people"}</h2><p class="mt-1 text-sm text-[#776a63]">${locale==="el"?"Η πρόσκληση δίνει πρόσβαση μόνο σε αυτό το event.":"The invitation only grants access to this event."}</p><form action="/api/account/events/${event.code}/invite" method="post" class="mt-5 space-y-3"><input type="hidden" name="locale" value="${locale}"><input name="email" type="email" required placeholder="name@example.com" class="w-full rounded-xl border bg-white px-4 py-3"><button class="w-full rounded-xl bg-[#76533d] py-3 text-white">${locale==="el"?"Αποστολή πρόσκλησης":"Send invitation"}</button></form></div></div></section></main>${logoutScript(locale)}`));
});
