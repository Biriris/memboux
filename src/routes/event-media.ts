...
export function mediaDetailActions(
  ...
  const downloadAction=canDownload
    ? `<a href="/media/${mediaId}?download=1" class="mt-5 block rounded-xl bg-[#7c3aed] px-4 py-3 text-center font-semibold text-white">↓ ${locale==="el"?"Λήψη πρωτοτύπου":"Download original"}</a>`
    : `<div class="mt-5 rounded-2xl border border-violet-200 bg-violet-50 p-4 text-sm leading-6 text-violet-950"><strong>${locale==="el"?"Το πρωτότυπο είναι ασφαλές":"Your original is safe"}</strong><p class="mt-1">${locale==="el"?"Μπορείς να το δεις σε preview. Η λήψη πλήρους ανάλυσης ξεκλειδώνει με την αναβάθμιση του event.":"You can view the preview. Full-resolution download unlocks when the event is upgraded."}</p><a href="/dashboard/${eventCode}/checkout?lang=${locale}" class="mt-3 inline-flex font-bold text-[#6d28d9]">${locale==="el"?"Δες τα πακέτα →":"View packages →"}</a></div>`;
  const trashAction=canManage?`<form action="/api/account/events/${eventCode}/media/${mediaId}/trash" method="post" class="mt-3" onsubmit="return confirm('Move this media to trash?')"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl border border-red-200 px-4 py-3 text-red-700">${locale==="el"?"Μεταφορά στον κάδο":"Move to trash"}</button></form>`:"";
  return downloadAction + trashAction;
}
...
eventMediaRoutes.get("/dashboard/:code/media/:id", async(c)=>{
  ...
  if(!roleCan(membership,"view"))return c.text("Forbidden",403);
  ...
  const canDownload=eventAccessAllows(await getEventAccess(c.env.DB,event.id),"original_downloads");
  const canManage=roleCan(membership,"manage_media");
  const preview=media.media_type==="image"?`<img src="/media/${media.id}?variant=preview" class="max-h-[70vh] w-full rounded-2xl object-contain bg-black">`:`<video src="/media/${media.id}" controls class="max-h-[70vh] w-full rounded-2xl bg-black"></video>`;
  const chronologicalDate=media.captured_at??media.uploaded_at;
  const actions=mediaDetailActions(locale,event.code,media.id,canDownload,canManage);
  return c.html(page(event.eventName,`<header class="border-b bg-white"><div class="mx-auto flex max-w-5xl items-center justify-between p-5">${brandMark(`/${locale}`,true)}${accountMenu(locale,user)}</div></header><main class="mx-auto max-w-5xl p-5 md:p-10"><a href="/dashboard/${event.code}?lang=${locale}" class="text-sm text-[#7c3aed]">← ${locale==="el"?"Πίσω στο event":"Back to event"}</a><div class="mt-5 grid gap-6 lg:grid-cols-[1fr_320px]"><div>${preview}</div><aside class="rounded-2xl bg-white p-5 shadow"><p class="text-sm text-[#6f657c]">${locale==="el"?"Ημερομηνία":"Date"}: ${formatDateTime(chronologicalDate,locale)}</p>${actions}</aside></div></main>${lightboxMarkup(locale)}${logoutScript(locale)}`));
});
...
eventMediaRoutes.get("/dashboard-legacy/:code", async (c) => {
  ...
  return c.html(page(`${event.eventName} – Διαχείριση`, `<main class="mx-auto max-w-6xl p-4 sm:p-5 md:p-10"><section class="mb-6 rounded-3xl bg-white p-5 shadow-lg sm:p-7"><p class="text-sm font-semibold text-[#6d28d9]">ΙΔΙΩΤΙΚΗ ΔΙΑΧΕΙΡΙΣΗ</p><h1 class="mt-2 break-words text-3xl font-bold sm:text-4xl">${esc(event.eventName)}</h1><p class="mt-3">Κωδικός: <strong class="font-mono text-2xl text-[#6d28d9]">${esc(event.code)}</strong></p><div class="mt-7 grid items-center gap-7 md:grid-cols-[minmax(0,220px)_minmax(0,1fr)]"><div class="mx-auto w-full max-w-[220px] overflow-hidden rounded-2xl border bg-white p-3">${qrSvg}</div><div class="min-w-0"><h2 class="text-xl font-bold">QR Code καλεσμένων</h2><p class="mt-2 text-sm text-[#6f657c]">Οι καλεσμένοι σκανάρουν το QR και ανοίγουν απευθείας το gallery του event.</p><a href="${esc(guestUrl)}" target="_blank" class="mt-3 block max-w-full break-all text-sm font-semibold text-[#6d28d9]">${esc(guestUrl)}</a><div class="mt-4 flex flex-col gap-2 sm:flex-row"><input id="link" readonly value="${esc(guestUrl)}" class="w-full min-w-0 flex-1 rounded-xl border px-4 py-3"><button id="copy" class="shrink-0 rounded-xl bg-[#2b174d] px-5 py-3 text-white">Αντιγραφή</button></div></div></div></section><section class="rounded-3xl bg-white p-5 shadow-lg sm:p-7"><h2 class="mb-5 text-2xl font-bold">Gallery (${items.length})</h2>${items.length ? `<div class="grid grid-cols-2 gap-4 md:grid-cols-3">${cards(items)}</div>` : `<p class="py-12 text-center text-[#6f657c]">Δεν υπάρχουν uploads ακόμη.</p>`}</section></main><script>document.getElementById('copy').onclick=()=>navigator.clipboard.writeText(document.getElementById('link').value)<\/script>`));
});
