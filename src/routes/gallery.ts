...
galleryRoutes.get("/gallery/:code", async (c) => {
  ...
  if (!(await hasGalleryAccess(c.req.raw, event))) {
    return c.html(
      page(
        event.eventName,
        `<main class="flex min-h-screen items-center justify-center p-5"><section class="w-full max-w-md rounded-3xl bg-white p-8 text-center shadow-xl"><div class="flex items-center justify-between">${brandMark("/", true)}<a href="/gallery/${event.code}?lang=${otherLocale}" class="rounded-xl border px-3 py-2 text-sm">${otherLocale.toUpperCase()}</a></div><h1 class="mt-7 text-4xl">${locale === "el" ? "Ιδιωτική gallery" : "Private gallery"}</h1><p class="mt-2 text-[#64748b]">${locale === "el" ? "Βάλε το PIN του event για να δεις τη gallery και να ανεβάσεις φωτογραφίες ή βίντεο." : "Enter the event PIN to view the gallery and upload photos or videos."}</p><form action="/gallery/${encodeURIComponent(event.code)}/unlock" method="post" class="mt-6 space-y-3"><input type="hidden" name="locale" value="${locale}"><input name="pin" type="password" inputmode="numeric" pattern="[0-9]{4,8}" required autofocus placeholder="PIN" class="w-full rounded-xl border px-4 py-3 text-center text-xl tracking-[.3em]"><button class="w-full rounded-xl bg-[#4f46e5] px-5 py-3 text-white">${locale === "el" ? "Άνοιγμα gallery" : "Open gallery"}</button></form></section></main>`,
      ),
      401,
    );
  }
  ...
  return c.html(
    page(
      ...
      `<main class="mx-auto max-w-6xl p-5 md:p-10"><section class="mb-6 rounded-3xl bg-white p-7 text-center shadow-lg"><div class="mb-4 flex items-center justify-between">${brandMark("/", true)}<a href="/gallery/${event.code}?lang=${otherLocale}" class="rounded-xl border px-3 py-2 text-sm">${otherLocale.toUpperCase()}</a></div><h1 class="mt-2 text-4xl font-bold">${esc(event.eventName)}</h1><p class="mt-2 font-medium text-[#4f46e5]">${esc(formatEventDates(event, locale))}</p><p class="mt-2 text-[#64748b]">${locale === "el" ? "Μοιράσου τις αγαπημένες σου στιγμές" : "Share your favorite moments"}</p>${shareIconButtons(guestUrl, event.eventName, locale)}<form action="/api/upload/${event.code}" method="post" enctype="multipart/form-data" class="gallery-upload mx-auto mt-7 max-w-xl space-y-3 text-left"><input type="hidden" name="locale" value="${locale}"><input name="name" maxlength="60" placeholder="${locale === "el" ? "Το όνομά σου" : "Your name"}" class="w-full rounded-xl border px-4 py-3"><input name="file" required multiple type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime" class="w-full rounded-xl border p-3"><p class="text-xs text-[#64748b]">${locale === "el" ? "Επίλεξε έως 20 φωτογραφίες ή βίντεο μαζί · έως 100 MB ανά αρχείο και έως 100 MB συνολικά." : "Select up to 20 photos or videos · 100 MB per file and up to 100 MB total."}</p><div class="rounded-2xl bg-[#f5f7ff] p-4 text-sm text-[#475569]"><p>${locale === "el" ? "Το περιεχόμενο θα αποθηκευτεί στην ιδιωτική συλλογή αυτού του event. Μπορείς να ζητήσεις αφαίρεση οποιαδήποτε στιγμή." : "Your content will be stored in this event’s private gallery. You can request removal at any time."}</p><label class="mt-3 flex items-start gap-3"><input name="upload_confirmation" value="accepted" required type="checkbox" class="mt-1 h-4 w-4 shrink-0"><span>${locale === "el" ? "Επιβεβαιώνω ότι έχω δικαίωμα να ανεβάσω αυτό το περιεχόμενο και ότι δεν παραβιάζει παράνομα την ιδιωτικότητα ή τα δικαιώματα άλλων." : "I confirm that I am entitled to upload this content and that it does not unlawfully infringe the privacy or rights of others."}</span></label></div><button class="w-full rounded-xl bg-gradient-to-r from-[#7c3aed] to-[#4f46e5] py-3 font-semibold text-white">${locale === "el" ? "Ανέβασμα" : "Upload"}</button></form></section><section class="rounded-3xl bg-white p-7 shadow-lg"><div class="mb-5 flex items-center justify-between gap-3"><div><h2 class="text-2xl font-bold">Gallery (${items.length})</h2>${galleryFilterControls(items, "guest-gallery", locale)}</div><div class="flex gap-2"><button id="select-media" class="rounded-xl border px-4 py-2 text-sm">${locale === "el" ? "Επιλογή" : "Select"}</button><button id="download-selected" class="hidden rounded-xl bg-[#4f46e5] px-4 py-2 text-sm text-white">${locale === "el" ? "Λήψη επιλεγμένων" : "Download selected"}</button></div></div>${items.length ? `<div class="grid grid-cols-2 gap-4 md:grid-cols-3">${cards(items, { selectable: true, deferredSelection: true, lightbox: true, reportCode: event.code, locale })}</div>` : `<p class="py-12 text-center text-[#64748b]">${locale === "el" ? "Γίνε ο πρώτος που θα ανεβάσει μια στιγμή!" : "Be the first to upload a moment!"}</p>`}</section></main>${galleryFilterScript(items, "guest-gallery")}${lightboxMarkup(locale)}${selectionScript}`,
    ),
  );
});
...
galleryRoutes.get("/gallery/:code/official", async (c) => {
  ...
  return c.html(
    page(
      ...
      `<main class="mx-auto max-w-6xl p-5 md:p-10"><header class="flex items-center justify-between">${brandMark("/", true)}<a href="/gallery/${event.code}?lang=${locale}" class="rounded-xl border px-4 py-2">← Gallery</a></header><section class="mt-6 rounded-3xl bg-white p-7 shadow-lg"><p class="text-xs uppercase tracking-[.2em] text-[#4338ca]">Memboux Studio</p><h1 class="mt-2 text-4xl">${locale === "el" ? "Official album" : "Official album"}</h1><p class="mt-2 text-[#64748b]">${esc(event.eventName)}</p><div class="mt-6 grid grid-cols-2 gap-4 md:grid-cols-3">${items.results.length ? cards(items.results, { lightbox: true, locale }) : `<p class="col-span-full py-12 text-center text-[#64748b]">${locale === "el" ? "Το official album ετοιμάζεται." : "The official album is being prepared."}</p>`}</div></section></main>${lightboxMarkup(locale)}`,
    ),
  );
});
...
galleryRoutes.get("/gallery/:code/removal/:mediaId", async (c) => {
  ...
  return c.html(
    page(
      "Request removal",
      `<main class="mx-auto flex min-h-screen max-w-xl items-center p-5"><section class="w-full rounded-3xl bg-white p-7 shadow-xl">${brandMark("/", true)}<p class="mt-7 text-xs uppercase tracking-[.2em] text-[#4338ca]">Privacy request</p><h1 class="mt-2 text-4xl">Request photo removal</h1><p class="mt-3 text-[#64748b]">Use this form if you appear in this content or believe it infringes your privacy or rights. The event owner will receive the request for review.</p><form action="/gallery/${encodeURIComponent(event.code)}/removal/${encodeURIComponent(c.req.param("mediaId"))}" method="post" class="mt-6 space-y-4"><label class="block">Email<input name="email" type="email" required maxlength="254" class="mt-1 w-full rounded-xl border px-4 py-3"></label><label class="block">Reason<textarea name="reason" required minlength="10" maxlength="1000" rows="5" class="mt-1 w-full rounded-xl border px-4 py-3"></textarea></label><button class="w-full rounded-xl bg-[#4f46e5] px-5 py-3 text-white">Submit removal request</button></form></section></main>`,
    ),
  );
});
...
galleryRoutes.post("/gallery/:code/removal/:mediaId", async (c) => {
  ...
  return c.html(
    page(
      "Request received",
      `<main class="flex min-h-screen items-center justify-center p-5"><section class="max-w-lg rounded-3xl bg-white p-8 text-center shadow-xl"><h1 class="text-4xl">Request received</h1><p class="mt-3 text-[#64748b]">Your removal request was recorded and will be reviewed by the event owner.</p><a href="/gallery/${encodeURIComponent(event.code)}" class="mt-6 inline-block rounded-xl bg-[#4f46e5] px-5 py-3 text-white">Back to gallery</a></section></main>`,
    ),
  );
});
