...
import { weddingLandingPage } from "../views/event-landing";
import { eventVerticalFor, eventVerticals } from "../event-verticals";
...
publicRoutes.get("/health/email", async (c) => {
  c.header("Cache-Control", "no-store");
  const dns = await checkEmailDnsHealth();
  const outboundConfigured = Boolean(c.env.RESEND_API_KEY?.trim());
  const deliveryTrackingConfigured = Boolean(c.env.RESEND_WEBHOOK_SECRET?.trim());
  const ready = dns.ready && outboundConfigured && deliveryTrackingConfigured;
  ...
  return c.json(
    {
      ...
      dns: Object.fromEntries(
        dns.checks.map((check) => [check.key, check.status]),
      ),
    },
    ready ? 200 : 503,
  );
...
publicRoutes.get("/:locale{el|en|fr|de|es|it}/wedding/demo-frame", (c) => {
  ...
  return c.html(weddingDemoFrame(normalizeLocale(c.req.param("locale")), normalizeWeddingTheme(c.req.query("theme"))));
});
publicRoutes.get("/:locale{el|en|fr|de|es|it}/events/:type", (c) => {
  const vertical = eventVerticalFor(c.req.param("type"));
  if (!vertical) return c.notFound();
  return c.html(eventVerticalLandingPage(normalizeLocale(c.req.param("locale")), vertical));
});
publicRoutes.get("/:locale{el|en|fr|de|es|it}/events/:type/preview", (c) => {
  const vertical = eventVerticalFor(c.req.param("type"));
  if (!vertical) return c.notFound();
  return c.html(eventVerticalDemoPage(normalizeLocale(c.req.param("locale")), vertical, normalizeDemoTheme(c.req.query("theme"))));
});
publicRoutes.get("/:locale{el|en|fr|de|es|it}/events/:type/demo-frame", (c) => {
  const vertical = eventVerticalFor(c.req.param("type"));
  if (!vertical) return c.notFound();
  c.header("Content-Security-Policy", "frame-ancestors 'self'");
  return c.html(eventVerticalDemoFrame(normalizeLocale(c.req.param("locale")), vertical, normalizeDemoTheme(c.req.query("theme"))));
});
...
publicRoutes.get("/:locale{el|en|fr|de|es|it}/verify-email", (c) => {
  ...
  return c.html(page(el ? "Έλεγξε το email σου" : "Check your email", `<main class="min-h-screen bg-[#f8f5ff] p-4 sm:p-6"><section class="mx-auto flex min-h-[calc(100vh-2rem)] max-w-5xl items-center justify-center"><div class="w-full overflow-hidden rounded-[2rem] border border-[#e6dff0] bg-white shadow-[0_30px_100px_rgba(24,60,51,.13)]"><header class="flex items-center justify-between border-b border-[#ece5f4] px-6 py-5 sm:px-10">${brandMark(`/${locale}`, true)}<a href="/${locale === "el" ? "en" : "el"}/verify-email" class="rounded-full border px-3 py-2 text-xs font-semibold text-[#6d28d9]">${el ? "EN" : "EL"}</a></header><div class="grid lg:grid-cols-[1.05fr_.95fr]"><div class="p-7 sm:p-12"><span class="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#f1eaff] text-2xl">✉</span><p class="mt-8 text-xs font-semibold uppercase tracking-[.2em] text-[#7c3aed]">${el ? "Ένα τελευταίο βήμα" : "One last step"}</p><h1 class="mt-3 text-4xl font-medium tracking-[-.03em] text-[#24143b] sm:text-5xl">${el ? "Έλεγξε το email σου" : "Check your email"}</h1><p class="mt-4 max-w-xl text-base leading-7 text-[#6f657c]">${el ? "Στείλαμε τις σωστές οδηγίες για τον λογαριασμό σου. Ο σύνδεσμος επιβεβαίωσης λήγει σε μία ώρα." : "We sent the right next step for your account. Verification links expire after one hour."}</p><div id="email-summary" class="mt-6 hidden rounded-2xl bg-[#faf8ff] p-4"><p class="text-xs uppercase tracking-[.15em] text-[#6f657c]">Email</p><strong id="masked-email" class="mt-1 block break-all text-[#2b174d]"></strong></div><div class="mt-7 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"><strong>${el ? "Έχεις ήδη λογαριασμό ή χρησιμοποίησες Google;" : "Already registered or used Google?"}</strong><p class="mt-1">${el ? "Συνδέσου αντί να κάνεις νέα εγγραφή. Για λόγους προστασίας προσωπικών δεδομένων η εγγραφή δείχνει την ίδια επιτυχία, αλλά ένας ήδη επιβεβαιωμένος λογαριασμός δεν λαμβάνει δεύτερο verification link." : "Sign in instead of registering again. For privacy, sign-up shows the same success state, but an already verified account does not receive another verification link."}</p><a href="/${locale}/login" class="mt-3 inline-block font-semibold text-amber-950 underline">${el ? "Μετάβαση στη σύνδεση" : "Go to sign in"}</a></div></div><aside class="border-t bg-[#faf8ff] p-7 sm:p-12 lg:border-l lg:border-t-0"><h2 class="text-2xl font-medium text-[#2b174d]">${el ? "Δεν ήρθε email;" : "Didn't receive it?"}</h2><ol class="mt-4 space-y-2 text-sm leading-6 text-[#6f657c]"><li>1. ${el ? "Έλεγξε Spam/Junk και Promotions." : "Check Spam/Junk and Promotions."}</li><li>2. ${el ? "Βεβαιώσου ότι το email γράφτηκε σωστά." : "Make sure the email address is correct."}</li><li>3. ${el ? "Περίμενε ένα λεπτό και κάνε επαναποστολή." : "Wait a minute, then resend."}</li></ol><form id="resend-verification" class="mt-7 space-y-3"><label class="block text-sm font-medium text-[#493b57]">Email<input id="verification-email" name="email" type="email" required maxlength="254" autocomplete="email" class="mt-1.5 w-full rounded-2xl border border-[#dcd3e8] bg-white px-4 py-3.5 outline-none focus:border-[#8b5cf6] focus:ring-4 focus:ring-[#8b5cf6]/10"></label><button id="resend-button" class="w-full rounded-2xl bg-[#7c3aed] px-6 py-3.5 font-semibold text-white disabled:cursor-wait disabled:opacity-60">${el ? "Επαναποστολή email" : "Resend verification email"}</button><p id="resend-message" role="status" aria-live="polite" class="hidden rounded-2xl p-3.5 text-sm leading-6"></p></form><div class="mt-6 flex flex-wrap gap-4 text-sm"><a href="/${locale}/register" class="font-semibold text-[#6d28d9]">${el ? "Αλλαγή email" : "Change email"}</a><a href="/${locale}/forgot-password" class="font-semibold text-[#6d28d9]">${el ? "Ξέχασα τον κωδικό" : "Forgot password"}</a></div></aside></div></div></section></main><script>const emailInput=document.getElementById('verification-email'),emailSummary=document.getElementById('email-summary'),maskedEmail=document.getElementById('masked-email'),resendForm=document.getElementById('resend-verification'),resendButton=document.getElementById('resend-button'),resendMessage=document.getElementById('resend-message'),storedEmail=sessionStorage.getItem('membouxVerificationEmail')||'';const maskEmail=(email)=>{const parts=email.split('@');if(parts.length!==2)return email;const local=parts[0];return (local.length<3?local[0]+'*':local.slice(0,2)+'***'+local.slice(-1))+'@'+parts[1]};if(storedEmail){emailInput.value=storedEmail;maskedEmail.textContent=maskEmail(storedEmail);emailSummary.classList.remove('hidden')}resendForm.onsubmit=async(event)=>{event.preventDefault();if(!resendForm.reportValidity())return;resendButton.disabled=true;resendMessage.classList.add('hidden');try{const response=await fetch('/api/auth/send-verification-email',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:emailInput.value.trim().toLowerCase(),callbackURL:'/${locale}/account'})});resendMessage.classList.remove('hidden');resendMessage.className=response.ok?'rounded-2xl bg-emerald-50 p-3.5 text-sm leading-6 text-emerald-700':'rounded-2xl bg-red-50 p-3.5 text-sm leading-6 text-red-700';resendMessage.textContent=response.ok?${JSON.stringify(sentMessage)}:${JSON.stringify(failedMessage)}}catch{resendMessage.className='rounded-2xl bg-red-50 p-3.5 text-sm leading-6 text-red-700';resendMessage.textContent=${JSON.stringify(failedMessage)};resendMessage.classList.remove('hidden')}finally{resendButton.disabled=false}}<\/script>`));
});
