...
adminMediaRoutes.get("/admin/reported", async (c) => {
  ...
  const cards = media.results
    .map(
      (item) =>
        `<article class="selectable-media relative overflow-hidden rounded-2xl border bg-white shadow-sm transition"><label class="media-selector absolute inset-0 z-20 hidden cursor-pointer"><input type="checkbox" class="media-select sr-only" value="${item.id}"><span class="selection-tick absolute left-3 top-3 hidden h-8 w-8 items-center justify-center rounded-full bg-[#4f46e5] text-white shadow">✓</span></label><button type="button" class="lightbox-item block aspect-square w-full bg-[#e8edff]" data-src="/admin/media/${encodeURIComponent(item.id)}" data-type="${item.media_type}">${item.media_type === "image" ? `<img src="/admin/media/${encodeURIComponent(item.id)}" alt="" loading="lazy" class="h-full w-full object-cover">` : `<video src="/admin/media/${encodeURIComponent(item.id)}" muted preload="metadata" class="h-full w-full object-cover"></video>`}</button><div class="p-4"><h2 class="truncate text-xl">${item.eventName}</h2><p class="mt-1 truncate text-xs text-[#64748b]">${item.requester_email ?? ""}</p><p class="mt-2 line-clamp-3 text-sm">${item.reason ?? ""}</p><p class="mt-2 text-xs text-red-700">Reported ${formatDateTime(item.reported_at, "el")}</p></div></article>`,
    )
    .join("");
  ...
  return c.html(
    adminShell(
      "Reported",
      `<main class="mx-auto max-w-7xl p-5 md:p-10"><a href="/admin" class="text-sm text-[#4338ca]">← ${locale === "el" ? "Πίσω στη βιβλιοθήκη" : "Back to library"}</a><div class="mt-4 flex flex-wrap items-end justify-between gap-3"><div><h1 class="text-4xl">${locale === "el" ? "Reported φωτογραφίες" : "Reported media"}</h1><p class="mt-2 text-[#64748b]">${locale === "el" ? "Κρυμμένες από guest και owner galleries μέχρι να τις ελέγξει ο admin." : "Hidden from guest and owner galleries until reviewed by the admin."}</p></div><div class="flex flex-wrap gap-2"><button id="reported-select" class="rounded-xl border px-4 py-2">${locale === "el" ? "Επιλογή" : "Select"}</button><button id="reported-restore" class="hidden rounded-xl bg-[#4f46e5] px-4 py-2 text-white">${locale === "el" ? "Επαναφορά επιλεγμένων" : "Restore selected"}</button><button id="reported-delete" class="hidden rounded-xl border border-red-200 px-4 py-2 text-red-700">${locale === "el" ? "Διαγραφή επιλεγμένων" : "Delete selected"}</button></div></div><form id="reported-restore-form" action="/admin/reported/restore" method="post"><input id="reported-restore-ids" type="hidden" name="ids"></form><form id="reported-delete-form" action="/admin/reported/trash" method="post"><input id="reported-delete-ids" type="hidden" name="ids"></form><div class="mt-7 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">${cards || `<p class="col-span-full rounded-2xl bg-white p-10 text-center text-[#64748b]">${locale === "el" ? "Δεν υπάρχουν reported φωτογραφίες." : "No reported media."}</p>`}</div></main>${lightboxMarkup(locale)}${script}`,
      locale,
    ),
  );
});
...
adminMediaRoutes.get("/admin/privacy-requests", async (c) => {
  ...
  const rows = requests.results
    .map(
      (item) =>
        `<article class="rounded-2xl border bg-white p-5 shadow-sm"><div class="flex flex-wrap items-start justify-between gap-3"><div><span class="rounded-full bg-[#eef2ff] px-3 py-1 text-xs uppercase">${item.request_type}</span><h2 class="mt-3 break-all text-xl">${item.email}</h2><p class="mt-1 text-xs text-[#64748b]">${formatDateTime(item.created_at, locale)} · ${item.id}</p></div><span class="rounded-full px-3 py-1 text-xs ${item.status === "pending" ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}">${item.status}</span></div><p class="mt-4 whitespace-pre-wrap rounded-xl bg-[#f8faff] p-4 text-sm">${item.details}</p>${item.status === "pending" ? `<div class="mt-4 flex gap-2"><form action="/admin/privacy-requests/${encodeURIComponent(item.id)}/resolve" method="post"><button class="rounded-xl bg-[#4f46e5] px-4 py-2 text-sm text-white">${locale === "el" ? "Επίλυση" : "Resolve"}</button></form><form action="/admin/privacy-requests/${encodeURIComponent(item.id)}/dismiss" method="post"><button class="rounded-xl border px-4 py-2 text-sm">${locale === "el" ? "Απόρριψη" : "Dismiss"}</button></form></div>` : ""}</article>`,
    )
    .join("");
  return c.html(
    adminShell(
      locale === "el" ? "Αιτήματα απορρήτου" : "Privacy requests",
      `<main class="mx-auto max-w-5xl p-5 md:p-10"><div class="flex flex-wrap items-end justify-between gap-4"><div><p class="text-xs uppercase tracking-[.2em] text-[#4338ca]">Data rights</p><h1 class="mt-2 text-4xl">${locale === "el" ? "Αιτήματα απορρήτου" : "Privacy requests"}</h1></div><form><select name="status" onchange="this.form.submit()" class="rounded-xl border bg-white px-4 py-3"><option value="pending"${status === "pending" ? " selected" : ""}>Pending</option><option value="resolved"${status === "resolved" ? " selected" : ""}>Resolved</option><option value="dismissed"${status === "dismissed" ? " selected" : ""}>Dismissed</option></select></form></div><div class="mt-7 grid gap-4">${rows || `<p class="rounded-2xl bg-white p-10 text-center text-[#64748b]">${locale === "el" ? "Δεν υπάρχουν αιτήματα." : "No requests."}</p>`}</div></main>`,
      locale,
    ),
  );
});
...
adminMediaRoutes.get("/admin/trash", async (c) => {
  ...
  return c.html(
    adminShell(
      ...
      `<main class="mx-auto max-w-7xl p-5 md:p-10"><a href="/admin" class="text-sm text-[#4338ca]">← ${locale === "el" ? "Πίσω στη βιβλιοθήκη" : "Back to library"}</a><div class="mt-4 flex flex-wrap items-end justify-between gap-3"><div><h1 class="text-4xl">${locale === "el" ? "Κάδος φωτογραφιών" : "Media trash"}</h1><p class="mt-2 text-[#64748b]">${locale === "el" ? "Οι φωτογραφίες διαγράφονται οριστικά 30 ημέρες μετά τη μεταφορά τους στον κάδο. Πάτησε το preview για μεγέθυνση." : "Media is permanently deleted 30 days after being moved to trash. Select a preview to enlarge it."}</p></div><div class="flex flex-wrap gap-2"><button id="trash-select" class="rounded-xl border px-4 py-2">${locale === "el" ? "Επιλογή" : "Select"}</button><button id="trash-restore" class="hidden rounded-xl bg-[#4f46e5] px-4 py-2 text-white">${locale === "el" ? "Επαναφορά επιλεγμένων" : "Restore selected"}</button><button id="trash-delete" class="hidden rounded-xl border border-red-200 px-4 py-2 text-red-700">${locale === "el" ? "Οριστική διαγραφή" : "Delete permanently"}</button></div></div><form id="trash-restore-form" action="/admin/trash/restore" method="post"><input id="trash-restore-ids" type="hidden" name="ids"></form><form id="trash-delete-form" action="/admin/trash/delete" method="post"><input id="trash-delete-ids" type="hidden" name="ids"></form><div class="mt-7 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">${rows || `<p class="rounded-2xl bg-white p-8 text-center text-[#64748b]">${locale === "el" ? "Ο κάδος είναι άδειος." : "Trash is empty."}</p>`}</div></main>${lightboxMarkup(locale)}${trashScript}`,
      locale,
    ),
  );
});
