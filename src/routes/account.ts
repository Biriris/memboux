...
import { accountMenu, brandMark, eventHeader, logoutScript, page } from "../views/shared";
...
accountRoutes.get("/:locale{el|en}/plan", async (c) => {
  ...
  return c.html(page(el?"Plan & χρήση":"Plan & usage",`<header class="border-b bg-white"><div class="mx-auto flex max-w-4xl items-center justify-between p-5">${brandMark(`/${locale}`,true)}${accountMenu(locale,user)}</div></header><main class="mx-auto max-w-4xl p-5 md:p-10"><p class="text-xs uppercase tracking-[.2em] text-[#6e4f3e]">Account capacity</p><h1 class="mt-2 text-4xl">${el?"Plan και χρήση":"Plan and usage"}</h1><section class="mt-7 rounded-3xl bg-white p-6 shadow"><div class="flex items-center justify-between"><div><p class="text-sm text-[#625750]">${el?"Τρέχον plan":"Current plan"}</p><h2 class="text-3xl capitalize">${esc(entitlement.planKey)}</h2></div><span class="rounded-full bg-[#eee4dc] px-4 py-2 text-sm">Beta</span></div><div class="mt-7 grid gap-5 md:grid-cols-2"><div class="rounded-2xl bg-[#f8f3ee] p-5"><p class="text-sm text-[#625750]">Storage</p><p class="mt-1 text-2xl">${formatBytes(entitlement.usedBytes)} / ${formatBytes(entitlement.storageLimitBytes)}</p><div class="mt-3 h-2 overflow-hidden rounded-full bg-white"><div class="h-full bg-[#8b6250]" style="width:${storagePercent}%"></div></div><p class="mt-2 text-xs text-[#625750]">${el?"Ο κάδος μετρά μέχρι την οριστική διαγραφή.":"Trash counts until permanent deletion."}</p></div><div class="rounded-2xl bg-[#f8f3ee] p-5"><p class="text-sm text-[#625750]">Events</p><p class="mt-1 text-2xl">${eventCount?.total??0} / ${entitlement.eventLimit}</p><p class="mt-2 text-xs text-[#625750]">${el?`Έως ${entitlement.memberLimit} συνεργάτες ανά event.`:`Up to ${entitlement.memberLimit} collaborators per event.`}</p></div></div><p class="mt-6 rounded-xl bg-amber-50 p-4 text-sm text-amber-900">${el?"Το Beta plan είναι προσωρινό και χωρίς χρέωση. Οι εμπορικές τιμές δεν έχουν ενεργοποιηθεί.":"The Beta plan is temporary and free of charge. Commercial pricing is not active."}</p></section></main>${logoutScript(locale)}`));
});
  ...
  for (let attempt=0; attempt<5; attempt++) {
    ...
    try {
      ...
      if(attempt===4){
        ...
        return c.text(locale==="el"?"Δεν μπόρεσα να δημιουργήσω το event.":"Could not create event.",500);
      }
    }
  }
