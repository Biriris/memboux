...
import { eventTypeFieldLabel, eventTypeLabel, eventTypeOptions, isEventType, normalizeEventType } from "../event-types";
...
export function dashboardEventTypeBadge(locale: Locale, eventType: unknown) {
  return `<span class="shrink-0 rounded-full bg-[#f3eff8] px-2 py-1 text-[9px] font-bold uppercase tracking-[.08em] text-[#715d84]">${esc(eventTypeLabel(normalizeEventType(eventType), locale))}</span>`;
}
...
export function renderNewEventTypeField(locale: Locale, selected?: unknown) {
  const validSelection = isEventType(selected) ? selected : null;
  const choices = eventTypeOptions(locale, "other")
    .replace(/ selected/g, "")
    .replace(validSelection ? `value="${validSelection}"` : "\u0000", validSelection ? `value="${validSelection}" selected` : "\u0000");
  const label = eventTypeFieldLabel(locale);
  return `<label class="md:col-span-2"><span class="mb-1 block text-sm font-medium">${esc(label)}</span><select name="eventType" required class="w-full cursor-pointer rounded-xl border px-4 py-3"><option value=""${validSelection ? "" : " selected"} disabled>${esc(label)}</option>${choices}</select></label>`;
}

type ActivityNotification = {
  id: string;
  type: "invitation_accepted" | "media_uploaded" | "trial_ending_3d" | "trial_ending_1d" | "trial_expired";
...
function notificationTitle(notification: ActivityNotification, viewerId: string, locale: Locale) {
  const trialCopy = {
    en: {
      trial_ending_3d: "Your trial ends in 3 days",
      trial_ending_1d: "Your trial ends within 24 hours",
      trial_expired: "Your trial has ended — your memories remain safe",
    },
    el: {
      trial_ending_3d: "Η δοκιμή σου λήγει σε 3 ημέρες",
      trial_ending_1d: "Η δοκιμή σου λήγει μέσα σε 24 ώρες",
      trial_expired: "Η δοκιμή έληξε — οι αναμνήσεις σου παραμένουν ασφαλείς",
    },
    fr: {
      trial_ending_3d: "Votre essai se termine dans 3 jours",
      trial_ending_1d: "Votre essai se termine dans moins de 24 heures",
      trial_expired: "Votre essai est terminé — vos souvenirs restent en sécurité",
    },
    de: {
      trial_ending_3d: "Deine Testphase endet in 3 Tagen",
      trial_ending_1d: "Deine Testphase endet innerhalb von 24 Stunden",
      trial_expired: "Deine Testphase ist beendet — deine Erinnerungen bleiben sicher",
    },
    es: {
      trial_ending_3d: "Tu prueba termina en 3 días",
      trial_ending_1d: "Tu prueba termina en menos de 24 horas",
      trial_expired: "Tu prueba ha terminado — tus recuerdos siguen protegidos",
    },
    it: {
      trial_ending_3d: "La prova termina tra 3 giorni",
      trial_ending_1d: "La prova termina entro 24 ore",
      trial_expired: "La prova è terminata — i tuoi ricordi restano al sicuro",
    },
  } as const;
  if (notification.type === "trial_ending_3d"
    || notification.type === "trial_ending_1d"
    || notification.type === "trial_expired")
    return trialCopy[locale][notification.type];
  const el = locale === "el";
...
function notificationIcon(notification: ActivityNotification) {
  if (notification.type === "invitation_accepted")
    return { symbol: "✓", classes: "bg-emerald-50 text-emerald-700" };
  if (notification.type.startsWith("trial_"))
    return { symbol: "◷", classes: notification.type === "trial_expired" ? "bg-amber-50 text-amber-800" : "bg-violet-50 text-violet-800" };
  return { symbol: "＋", classes: "bg-[#f1eaff] text-[#6d28d9]" };
}
...
accountRoutes.get("/:locale{el|en|fr|de|es|it}/notifications", async (c) => {
  ...
  const pendingCards = pending.map((invitation) => `<article class="rounded-2xl border border-[#ddcff5] bg-white p-5 shadow-sm"><div class="flex items-start justify-between gap-4"><div><p class="text-xs font-bold uppercase tracking-[.16em] text-[#7c3aed]">${invitation.invitation_kind === "professional" ? "Professional invitation" : (el ? "Πρόσκληση album" : "Album invitation")}</p><h2 class="mt-2 text-2xl">${esc(invitation.event_name)}</h2><p class="mt-2 text-sm text-[#6f657c]">${el ? `${esc(invitation.inviter_name)} σε προσκάλεσε` : `Invited by ${esc(invitation.inviter_name)}`}</p></div><span class="h-2.5 w-2.5 rounded-full bg-red-500"></span></div><div class="mt-5 flex gap-2"><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/accept" method="post" class="flex-1"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl bg-[#7c3aed] px-4 py-2.5 text-sm font-semibold text-white">${el ? "Αποδοχή" : "Accept"}</button></form><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/decline" method="post"><input type="hidden" name="locale" value="${locale}"><button class="rounded-xl border px-4 py-2.5 text-sm font-semibold">${el ? "Απόρριψη" : "Decline"}</button></form></div></article>`).join("");
  const activityCards = visibleActivity.map((notification) => {
    ...
    return `<article class="group flex items-start gap-3 rounded-2xl border ${isUnread ? "border-[#ddcff5] bg-[#faf8ff]" : "border-[#ece6f3] bg-white"} p-3 transition hover:shadow-md sm:p-4"><a href="${href}" ${isUnread ? `data-notification-id="${esc(notification.id)}"` : ""} class="flex min-w-0 flex-1 items-start gap-3 rounded-xl p-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a78bfa]"><span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${icon.classes}">${icon.symbol}</span><span class="min-w-0 flex-1"><span class="flex items-start gap-2"><strong class="block flex-1 text-[#2b174d]">${message}</strong>${isUnread ? `<span class="mt-1.5 h-2.5 w-2.5 shrink-0 rounded-full bg-red-500" aria-label="${el ? "Μη αναγνωσμένη" : "Unread"}"></span>` : ""}</span><span class="mt-1 block truncate text-sm text-[#6f657c]">${esc(notification.event_name || "Memboux")} · ${formatDateTime(notification.created_at, locale)}</span></span></a><form action="/api/account/notifications/${encodeURIComponent(notification.id)}/status" method="post" class="shrink-0"><input type="hidden" name="locale" value="${locale}"><input type="hidden" name="view" value="${view}"><input type="hidden" name="action" value="${isUnread ? "read" : "unread"}"><button class="rounded-full border border-[#e2dcef] bg-white px-3 py-2 text-xs font-semibold text-[#65566f] hover:bg-[#f7f3ff]">${isUnread ? (el ? "Αναγνώστηκε" : "Mark read") : (el ? "Μη αναγνωσμένο" : "Mark unread")}</button></form></article>`;
  }).join("");
  const tab = (value: typeof view, label: string, count: number) => `<a href="/${locale}/notifications?view=${value}" aria-current="${view === value ? "page" : "false"}" class="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition ${view === value ? "bg-[#2b174d] text-white shadow-sm" : "bg-white text-[#746a80] hover:bg-[#f3effa]"}">${label}<span class="rounded-full ${view === value ? "bg-white/15" : "bg-[#f3effa]"} px-2 py-0.5 text-[11px]">${count}</span></a>`;
...
accountRoutes.get("/:locale{el|en|fr|de|es|it}/settings", async (c) => {
  ...
  const settingsCard = (href: string, icon: keyof typeof icons, title: string, description: string) => `<a href="${href}" class="group flex min-h-40 cursor-pointer flex-col rounded-3xl border border-[#e8e1f1] bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-[#d7c9ee] hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7c3aed] sm:p-6"><span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#f2ecff] text-[#6d28d9]"><svg aria-hidden="true" viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${icons[icon]}</svg></span><h2 class="mt-5 text-xl font-semibold text-[#2b174d]">${esc(title)}</h2><p class="mt-2 flex-1 text-sm leading-6 text-[#756b82]">${esc(description)}</p><span class="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-[#6d28d9]">${esc(copy.open)} <span aria-hidden="true" class="transition group-hover:translate-x-1">→</span></span></a>`;
  const languageOption = (value: Locale) => `<button name="locale" value="${value}" class="flex w-full items-center justify-between gap-4 rounded-2xl border ${locale === value ? "border-[#7c3aed] bg-[#f7f3ff]" : "border-[#e7e0f0] bg-white"} p-5 text-left hover:border-[#b79de3]"><span><strong class="block text-lg text-[#2b174d]">${localeNames[value]}</strong><span class="mt-1 block text-sm uppercase tracking-[.12em] text-[#756b82]">${value}</span></span><span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border ${locale === value ? "border-[#7c3aed] bg-[#7c3aed] text-white" : "border-[#ddd4e8] text-transparent"}">✓</span></button>`;
  return c.html(page(copy.title, `${accountHeader(locale, user)}<main class="mx-auto max-w-5xl p-5 pb-14 md:p-10"><div class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p class="page-kicker">${esc(copy.eyebrow)}</p><h1 class="page-title mt-2">${esc(copy.title)}</h1><p class="page-subtitle mt-3 max-w-2xl">${esc(copy.intro)}</p></div><div class="flex min-w-0 items-center gap-3 rounded-2xl border border-[#e8e1f1] bg-white px-4 py-3 shadow-sm"><span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#304c43] font-semibold text-white">${esc((user.name.trim().slice(0, 1) || "M").toUpperCase())}</span><span class="min-w-0"><strong class="block max-w-52 truncate text-sm text-[#2b174d]">${esc(user.name)}</strong><span class="block max-w-52 truncate text-xs text-[#756b82]">${esc(user.email)}</span></span></div></div><section aria-label="${esc(copy.title)}" class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">${settingsCard(`/${locale}/profile`, "profile", copy.profile, copy.profileText)}${settingsCard(`/${locale}/security`, "security", copy.security, copy.securityText)}${settingsCard(`/${locale}/backups`, "backups", copy.backups, copy.backupsText)}${settingsCard(`/${locale}/plan`, "plan", copy.plan, copy.planText)}${settingsCard(`/${locale}/privacy`, "privacy", copy.privacy, copy.privacyText)}${settingsCard(`/${locale}/trash`, "trash", copy.trash, copy.trashText)}</section><section class="mt-6 rounded-3xl border border-[#e8e1f1] bg-white p-6 shadow-sm sm:p-8"><div class="flex items-center gap-4"><span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#f2ecff] text-xl text-[#6d28d9]">文</span><div><h2 class="text-2xl">${esc(copy.language)}</h2><p class="mt-1 text-sm text-[#756b82]">${esc(copy.languageText)}</p></div></div><form action="/api/account/settings/language" method="post" class="mt-6 grid gap-3 sm:grid-cols-2">${supportedLocales.map(languageOption).join("")}</form></section></main>${logoutScript(locale)}`));
});
...
accountRoutes.get("/:locale{el|en|fr|de|es|it}/profile", async (c) => {
  ...
  return c.html(page(el ? "Προφίλ" : "Profile", `${accountHeader(locale, user)}<main class="mx-auto max-w-5xl p-5 md:p-10"><div><p class="page-kicker">Identity &amp; work</p><h1 class="page-title mt-2">${el ? "Το προφίλ σου" : "Your profile"}</h1><p class="page-subtitle mt-3">${el ? "Διαχειρίσου τα προσωπικά στοιχεία και την επαγγελματική σου παρουσία." : "Manage your personal details and professional presence."}</p></div><div class="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]"><div class="space-y-6"><section class="rounded-[1.75rem] border border-[#e9e3f2] bg-white p-6 shadow-sm"><div class="flex items-center gap-4"><span class="flex h-14 w-14 items-center justify-center rounded-full bg-[#2b174d] text-xl font-semibold text-white">${esc(user.name.slice(0, 1).toUpperCase())}</span><div><h2 class="text-2xl">${el ? "Προσωπικά στοιχεία" : "Personal details"}</h2><p class="text-sm text-[#6f657c]">${esc(user.email)}</p></div></div><form id="profile-form" class="mt-6 space-y-4"><label class="block text-sm font-semibold text-[#443653]">${el ? "Ονοματεπώνυμο" : "Full name"}<input name="name" required maxlength="100" value="${esc(user.name)}" class="mt-2 w-full rounded-xl border px-4 py-3 font-normal"></label><p id="profile-message" class="hidden rounded-xl p-3 text-sm"></p><button class="rounded-xl bg-[#2b174d] px-6 py-3 font-semibold text-white">${el ? "Αποθήκευση" : "Save details"}</button></form></section><section class="rounded-[1.75rem] border border-[#d5e6df] bg-white p-6 shadow-sm"><div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p class="text-xs font-bold uppercase tracking-[.18em] text-[#7c3aed]">Memboux Professional</p><h2 class="mt-2 text-3xl">${el ? "Επαγγελματικό προφίλ" : "Professional profile"}</h2><p class="mt-2 max-w-xl text-sm leading-6 text-[#6f657c]">${el ? "Για φωτογράφους και δημιουργούς που διαχειρίζονται official albums μέσω του Studio." : "For photographers and creators who manage official albums through Studio."}</p></div><span class="inline-flex h-fit rounded-full ${isProfessional ? "bg-emerald-50 text-emerald-800" : "bg-[#f7f3ff] text-[#6f657c]"} px-3 py-1.5 text-xs font-bold">${isProfessional ? (el ? "Ενεργό" : "Active") : (el ? "Ανενεργό" : "Inactive")}</span></div><form action="/api/account/profile/professional" method="post" class="mt-6 space-y-4"><input type="hidden" name="locale" value="${locale}"><label class="flex items-start gap-3 rounded-2xl bg-[#f9f6ff] p-4"><input id="professional-enabled" name="enabled" value="true" type="checkbox" ${isProfessional ? "checked" : ""} class="mt-1 h-5 w-5"><span><strong class="block text-[#2b174d]">${el ? "Είμαι Professional" : "I am a Professional"}</strong><span class="mt-1 block text-sm text-[#6f657c]">${el ? "Ενεργοποιεί το Studio profile και professional invitations." : "Enables your Studio profile and professional invitations."}</span></span></label><div id="professional-fields" class="grid gap-4"><label class="text-sm font-semibold text-[#443653]">${el ? "Επωνυμία / επαγγελματικό όνομα" : "Business or professional name"}<input name="businessName" maxlength="100" value="${esc(professional?.business_name ?? user.name)}" class="mt-2 w-full rounded-xl border px-4 py-3 font-normal"></label><label class="text-sm font-semibold text-[#443653]">Website<input name="website" type="url" maxlength="300" value="${esc(professional?.website ?? "")}" placeholder="https://" class="mt-2 w-full rounded-xl border px-4 py-3 font-normal"></label><label class="text-sm font-semibold text-[#443653]">Bio<textarea name="bio" maxlength="1000" rows="4" class="mt-2 w-full rounded-xl border px-4 py-3 font-normal">${esc(professional?.bio ?? "")}</textarea></label></div><button class="rounded-xl bg-[#7c3aed] px-6 py-3 font-semibold text-white">${el ? "Αποθήκευση Professional profile" : "Save Professional profile"}</button></form></section></div><aside class="h-fit rounded-[1.75rem] bg-[#2b174d] p-6 text-white shadow-xl"><p class="text-xs font-bold uppercase tracking-[.18em] text-[#c4b5fd]">Professional plan</p><h2 class="mt-3 text-2xl">${esc(planLabel)}</h2><p class="mt-4 text-sm leading-6 text-white/65">${el ? "Η μηνιαία συνδρομή δεν χρεώνεται ακόμη. Η υποδομή plan και billing είναι έτοιμη και θα ενεργοποιηθεί όταν οριστούν η τιμή και το Stripe." : "Monthly billing is not active yet. Plan and billing infrastructure is ready for pricing and Stripe activation."}</p><ul class="mt-6 space-y-3 text-sm text-white/80"><li>✓ Studio workspace</li><li>✓ Official albums</li><li>✓ Professional invitations</li><li>✓ Cloud backup support</li></ul></aside></div></main><script>document.getElementById('profile-form').onsubmit=async event=>{event.preventDefault();const message=document.getElementById('profile-message'),name=new FormData(event.target).get('name'),response=await fetch('/api/auth/update-user',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});message.classList.remove('hidden');message.className=response.ok?'rounded-xl bg-emerald-50 p-3 text-sm text-emerald-700':'rounded-xl bg-red-50 p-3 text-sm text-red-700';message.textContent=response.ok?${JSON.stringify(el ? "Το προφίλ ενημερώθηκε." : "Profile updated.")}:${JSON.stringify(el ? "Η ενημέρωση απέτυχε." : "Update failed.")};if(response.ok)setTimeout(()=>location.reload(),600)}</script>${logoutScript(locale)}`));
});
...
accountRoutes.get("/:locale{el|en|fr|de|es|it}/plan", async (c) => {
  ...
  return c.html(page(el?"Plan & χρήση":"Plan & usage",`${accountHeader(locale,user)}<main class="mx-auto max-w-4xl p-5 md:p-10"><p class="text-xs uppercase tracking-[.2em] text-[#6d28d9]">Account capacity</p><h1 class="mt-2 text-4xl">${el?"Plan και χρήση":"Plan and usage"}</h1><section class="mt-7 rounded-3xl bg-white p-6 shadow"><div class="flex items-center justify-between"><div><p class="text-sm text-[#6f657c]">${el?"Τρέχον plan":"Current plan"}</p><h2 class="text-3xl capitalize">${esc(entitlement.planKey)}</h2></div><span class="rounded-full bg-[#f1eaff] px-4 py-2 text-sm">Beta</span></div><div class="mt-7 grid gap-5 md:grid-cols-2"><div class="rounded-2xl bg-[#faf8ff] p-5"><p class="text-sm text-[#6f657c]">Storage</p><p class="mt-1 text-2xl">${formatBytes(entitlement.usedBytes)} / ${formatBytes(entitlement.storageLimitBytes)}</p><div class="mt-3 h-2 overflow-hidden rounded-full bg-white"><div class="h-full bg-[#356f5e]" style="width:${storagePercent}%"></div></div><p class="mt-2 text-xs text-[#6f657c]">${el?"Ο κάδος μετρά μέχρι την οριστική διαγραφή.":"Trash counts until permanent deletion."}</p></div><div class="rounded-2xl bg-[#faf8ff] p-5"><p class="text-sm text-[#6f657c]">Events</p><p class="mt-1 text-2xl">${eventCount?.total??0} / ${entitlement.eventLimit}</p><p class="mt-2 text-xs text-[#6f657c]">${el?`Έως ${entitlement.memberLimit} συνεργάτες ανά event.`:`Up to ${entitlement.memberLimit} collaborators per event.`}</p></div></div><p class="mt-6 rounded-xl bg-amber-50 p-4 text-sm text-amber-900">${el?"Το Beta plan είναι προσωρινό και χωρίς χρέωση. Οι εμπορικές τιμές δεν έχουν ενεργοποιηθεί.":"The Beta plan is temporary and free of charge. Commercial pricing is not active."}</p></section></main>${logoutScript(locale)}`));
});
...
accountRoutes.get("/:locale{el|en|fr|de|es|it}/account", async (c) => {
  ...
  const invitationCards = pendingInvitations
    .map(
      (invitation) =>
        `<article class="rounded-2xl border border-[#ddcff5] bg-white p-5 shadow-sm"><div class="flex items-start justify-between gap-4"><div class="min-w-0"><p class="text-xs uppercase tracking-[.16em] text-[#7c3aed]">${locale === "el" ? "Πρόσκληση album" : "Album invitation"}</p><h3 class="mt-2 truncate text-2xl">${esc(invitation.event_name)}</h3><p class="mt-2 text-sm text-[#6f657c]">${locale === "el" ? `${esc(invitation.inviter_name)} σε προσκάλεσε` : `Invited by ${esc(invitation.inviter_name)}`} · ${invitation.role === "viewer" ? (locale === "el" ? "Θεατής" : "Viewer") : locale === "el" ? "Διαχειριστής" : "Manager"}</p></div><span class="rounded-full bg-[#f1eaff] px-3 py-1 text-xs text-[#6d28d9]">${locale === "el" ? "Νέα" : "New"}</span></div><div class="mt-5 flex flex-col gap-2 sm:flex-row"><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/accept" method="post" class="flex-1"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl bg-[#7c3aed] px-5 py-3 text-sm font-semibold text-white">${locale === "el" ? "Αποδοχή" : "Accept"}</button></form><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/decline" method="post"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl border px-5 py-3 text-sm font-semibold text-[#675a72]">${locale === "el" ? "Απόρριψη" : "Decline"}</button></form></div></article>`,
    )
    .join("");
  ...
  const searchAction = `<button id="open-event-search" type="button" aria-label="${locale === "el" ? "Αναζήτηση events" : "Search events"}" title="${locale === "el" ? "Αναζήτηση events" : "Search events"}" class="flex h-11 w-11 items-center justify-center rounded-xl border border-white/15 bg-white/5 text-white hover:bg-white/10"><svg aria-hidden="true" viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></svg></button><dialog id="event-search" aria-labelledby="event-search-title" class="fixed left-1/2 top-1/2 m-0 max-h-[92dvh] w-[min(94vw,680px)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-3xl border-0 bg-white p-0 text-[#2b174d] shadow-2xl"><div class="border-b border-[#e9e3f2] p-5 sm:p-6"><div class="flex items-center justify-between gap-4"><div><p class="text-xs font-bold uppercase tracking-[.18em] text-[#7c3aed]">Memboux</p><h2 id="event-search-title" class="mt-1 text-2xl">${locale === "el" ? "Αναζήτηση events" : "Search events"}</h2></div><button id="close-event-search" type="button" aria-label="${locale === "el" ? "Κλείσιμο" : "Close"}" class="flex h-10 w-10 items-center justify-center rounded-full border border-[#e2dcef] text-xl">×</button></div><label class="relative mt-5 block"><svg aria-hidden="true" viewBox="0 0 24 24" class="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#877d91]" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></svg><input id="event-search-input" type="search" autocomplete="off" placeholder="${locale === "el" ? "Όνομα event…" : "Event name…"}" class="w-full rounded-2xl border border-[#e2dcef] bg-[#faf8ff] py-3 pl-12 pr-4 text-base"></label></div><div id="event-search-results" role="status" aria-live="polite" class="max-h-[60dvh] overflow-y-auto p-3 sm:p-4"><p class="p-6 text-center text-sm text-[#756b82]">${locale === "el" ? "Άνοιξε την αναζήτηση για να δεις τα πρόσφατα events." : "Open search to see your recent events."}</p></div></dialog><script>(()=>{const dialog=document.getElementById('event-search'),open=document.getElementById('open-event-search'),close=document.getElementById('close-event-search'),input=document.getElementById('event-search-input'),results=document.getElementById('event-search-results'),locale=${JSON.stringify(locale)},labels=${JSON.stringify({ empty: locale === "el" ? "Δεν βρέθηκαν events." : "No events found.", error: locale === "el" ? "Η αναζήτηση δεν είναι διαθέσιμη αυτή τη στιγμή." : "Search is unavailable right now.", owner: locale === "el" ? "Ιδιοκτήτης" : "Owner", viewer: locale === "el" ? "Θεατής" : "Viewer", editor: locale === "el" ? "Διαχειριστής" : "Manager", images: locale === "el" ? "εικόνες" : "images" })};let timer,request=0;const render=events=>{results.replaceChildren();if(!events.length){const empty=document.createElement('p');empty.className='p-8 text-center text-sm text-[#756b82]';empty.textContent=labels.empty;results.append(empty);return}events.forEach(event=>{const link=document.createElement('a');link.href='/dashboard/'+encodeURIComponent(event.code)+'?lang='+locale;link.className='flex items-center justify-between gap-4 rounded-2xl p-4 outline-none hover:bg-[#f8f5ff] focus-visible:bg-[#f2ecff]';const content=document.createElement('span');content.className='min-w-0';const name=document.createElement('strong');name.className='block truncate text-base font-semibold text-[#2b174d]';name.textContent=event.name;const meta=document.createElement('span');meta.className='mt-1 block truncate text-xs text-[#756b82]';meta.textContent=event.dates+' · '+event.imageCount+' '+labels.images;const role=document.createElement('span');role.className='shrink-0 rounded-full bg-[#f2ecff] px-3 py-1 text-xs font-semibold text-[#6d28d9]';role.textContent=labels[event.role]||event.role;content.append(name,meta);link.append(content,role);results.append(link)})};const search=async()=>{const current=++request;results.innerHTML='<p class="p-6 text-center text-sm text-[#756b82]">…</p>';try{const response=await fetch('/api/account/events/search?locale='+locale+'&q='+encodeURIComponent(input.value.trim()),{credentials:'include'}),data=await response.json();if(current!==request)return;if(!response.ok)throw new Error();render(data.events||[])}catch{if(current!==request)return;results.innerHTML='';const error=document.createElement('p');error.className='p-8 text-center text-sm text-red-700';error.textContent=labels.error;results.append(error)}};open.addEventListener('click',()=>{dialog.showModal();requestAnimationFrame(()=>input.focus());search()});close.addEventListener('click',()=>dialog.close());dialog.addEventListener('click',event=>{if(event.target===dialog)dialog.close()});input.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(search,180)})})()<\/script>`;
  return c.html(
    page(
      m.dashboard,
      `${eventHeader(locale, user, "", notificationCount, searchAction)}<main class="mx-auto max-w-6xl p-5 md:p-10"><div class="flex flex-wrap items-end justify-between gap-3"><div><p class="text-sm uppercase tracking-[.2em] text-[#6d28d9]">Dashboard</p><h1 class="text-4xl">${m.dashboard}</h1></div></div>${invitationSection}<form method="get" aria-label="${locale === "el" ? "Φίλτρα και ταξινόμηση events" : "Event filters and sorting"}" class="event-sort-toolbar mt-6 flex flex-wrap items-center justify-end gap-2 text-xs text-[#756b82]"><label class="inline-flex items-center gap-2 rounded-full border border-[#eae4f3] bg-white/70 py-1.5 pl-3 pr-1.5 shadow-sm"><span>${locale === "el" ? "Προβολή" : "View"}</span><select name="filter" aria-label="${locale === "el" ? "Φίλτρο events" : "Event filter"}" class="min-h-0 rounded-full border-0 bg-[#f8f5ff] px-3 py-1.5 text-xs font-semibold text-[#443653] outline-none">${Object.entries(
        filterLabel,
      )
        .map(
          ([v, l]) =>
  ...
  try {
    ...
    for (let attempt=0; attempt<5; attempt++) {
      ...
      await c.env.DB.batch([
        ...
        c.env.DB.prepare(`INSERT INTO event_access (
