...
accountRoutes.get("/:locale{el|en}/plan", async (c) => {
  ...
  return c.html(page(el?"Plan & χρήση":"Plan & usage",`${accountHeader(locale,user)}<main class="mx-auto max-w-4xl p-5 md:p-10"><p class="text-xs uppercase tracking-[.2em] text-[#4338ca]">Account capacity</p><h1 class="mt-2 text-4xl">${el?"Plan και χρήση":"Plan and usage"}</h1><section class="mt-7 rounded-3xl bg-white p-6 shadow"><div class="flex items-center justify-between"><div><p class="text-sm text-[#64748b]">${el?"Τρέχον plan":"Current plan"}</p><h2 class="text-3xl capitalize">${esc(entitlement.planKey)}</h2></div><span class="rounded-full bg-[#eef2ff] px-4 py-2 text-sm">Beta</span></div><div class="mt-7 grid gap-5 md:grid-cols-2"><div class="rounded-2xl bg-[#f8faff] p-5"><p class="text-sm text-[#64748b]">Storage</p><p class="mt-1 text-2xl">${formatBytes(entitlement.usedBytes)} / ${formatBytes(entitlement.storageLimitBytes)}</p><div class="mt-3 h-2 overflow-hidden rounded-full bg-white"><div class="h-full bg-[#7c3aed]" style="width:${storagePercent}%"></div></div><p class="mt-2 text-xs text-[#64748b]">${el?"Ο κάδος μετρά μέχρι την οριστική διαγραφή.":"Trash counts until permanent deletion."}</p></div><div class="rounded-2xl bg-[#f8faff] p-5"><p class="text-sm text-[#64748b]">Events</p><p class="mt-1 text-2xl">${eventCount?.total??0} / ${entitlement.eventLimit}</p><p class="mt-2 text-xs text-[#64748b]">${el?`Έως ${entitlement.memberLimit} συνεργάτες ανά event.`:`Up to ${entitlement.memberLimit} collaborators per event.`}</p></div></div><p class="mt-6 rounded-xl bg-amber-50 p-4 text-sm text-amber-900">${el?"Το Beta plan είναι προσωρινό και χωρίς χρέωση. Οι εμπορικές τιμές δεν έχουν ενεργοποιηθεί.":"The Beta plan is temporary and free of charge. Commercial pricing is not active."}</p></section></main>${logoutScript(locale)}`));
});
...
accountRoutes.get("/:locale{el|en}/account", async (c) => {
  ...
  if (!user) return c.redirect(`/${locale}/login`);
  const now = Date.now();
  const pendingInvitations = await listPendingInvitations(c.env.DB,user,now);
  ...
  const renderEventCard = (event: EventRow & {role:string;media_count:number;image_count:number;video_count:number}) => `<article class="group relative rounded-2xl border bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"><a href="/dashboard/${event.code}?lang=${locale}" class="block rounded-2xl p-5 pr-16"><div class="flex items-start justify-between gap-3"><div><h2 class="mt-1 text-2xl">${esc(event.eventName)}</h2></div><span class="rounded-full bg-[#eef2ff] px-2.5 py-1 text-xs">${event.role==="owner"?(locale==="el"?"Ιδιοκτήτης":"Owner"):event.role==="viewer"?(locale==="el"?"Θεατής":"Viewer"):(locale==="el"?"Διαχειριστής":"Manager")}</span></div><p class="mt-2 text-sm font-medium text-[#4f46e5]">${esc(formatEventDates(event,locale))}</p><p class="mt-2 text-sm text-[#64748b]">${event.image_count} images · ${event.video_count} videos</p></a>${event.role==="owner"?`<details class="absolute right-3 top-3 z-10"><summary class="flex h-10 w-10 cursor-pointer list-none items-center justify-center rounded-full bg-white text-2xl shadow-sm hover:bg-[#f5f7ff]" aria-label="Event actions">⋯</summary><div class="absolute right-0 mt-2 w-44 rounded-2xl border bg-white p-2 shadow-xl"><a href="/dashboard/${event.code}?lang=${locale}#settings" class="block rounded-xl px-3 py-2 text-sm hover:bg-[#f5f7ff]">${locale==="el"?"Ρυθμίσεις":"Settings"}</a><form action="/api/account/events/${event.code}/trash" method="post" onsubmit="return confirm('${locale==="el"?"Μεταφορά του event στον κάδο;":"Move this event to trash?"}')"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl px-3 py-2 text-left text-sm text-red-700 hover:bg-red-50">${locale==="el"?"Διαγραφή":"Delete"}</button></form></div></details>`:""}</article>`;
  const ownedCards = events.results.filter(event=>event.role==="owner").map(renderEventCard).join("");
  const sharedCards = events.results.filter(event=>event.role!=="owner").map(renderEventCard).join("");
  const section = (id:string,title:string,subtitle:string,content:string,empty:string) => `<section id="${id}" class="mt-8"><div class="flex flex-wrap items-end justify-between gap-3"><div><p class="text-xs uppercase tracking-[.18em] text-[#4f46e5]">${subtitle}</p><h2 class="mt-1 text-3xl">${title}</h2></div></div><div class="mt-4 grid gap-4 md:grid-cols-2">${content||`<div class="rounded-2xl border border-dashed bg-white p-8 text-center text-sm text-[#64748b] md:col-span-2">${empty}</div>`}</div></section>`;
  const invitationCards = pendingInvitations.map(invitation=>`<article class="rounded-2xl border border-[#c7d2fe] bg-white p-5 shadow-sm"><div class="flex items-start justify-between gap-4"><div class="min-w-0"><p class="text-xs uppercase tracking-[.16em] text-[#4f46e5]">${locale==="el"?"Πρόσκληση album":"Album invitation"}</p><h3 class="mt-2 truncate text-2xl">${esc(invitation.event_name)}</h3><p class="mt-2 text-sm text-[#64748b]">${locale==="el"?`${esc(invitation.inviter_name)} σε προσκάλεσε`:`Invited by ${esc(invitation.inviter_name)}`} · ${invitation.role==="viewer"?(locale==="el"?"Θεατής":"Viewer"):(locale==="el"?"Διαχειριστής":"Manager")}</p></div><span class="rounded-full bg-[#eef2ff] px-3 py-1 text-xs text-[#4338ca]">${locale==="el"?"Νέα":"New"}</span></div><div class="mt-5 flex flex-col gap-2 sm:flex-row"><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/accept" method="post" class="flex-1"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl bg-[#4f46e5] px-5 py-3 text-sm font-semibold text-white">${locale==="el"?"Αποδοχή":"Accept"}</button></form><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/decline" method="post"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl border px-5 py-3 text-sm font-semibold text-[#475569]">${locale==="el"?"Απόρριψη":"Decline"}</button></form></div></article>`).join("");
  const invitationSection = pendingInvitations.length ? section("invitations",locale==="el"?"Προσκλήσεις":"Invitations",locale==="el"?"Χρειάζονται την απάντησή σου":"Waiting for your response",invitationCards,"") : `<div id="invitations"></div>`;
  const ownedSection = section("my-events",locale==="el"?"Τα events μου":"My events",locale==="el"?"Albums που σου ανήκουν":"Albums you own",ownedCards,locale==="el"?"Δεν βρέθηκαν δικά σου events.":"No owned events found.");
  const sharedSection = section("shared-with-me",locale==="el"?"Μοιρασμένα μαζί μου":"Shared with me",locale==="el"?"Πρόσβαση από άλλους owners":"Access granted by other owners",sharedCards,locale==="el"?"Δεν υπάρχουν κοινόχρηστα albums.":"No shared albums yet.");
  const eventSections = filter==="owner" ? ownedSection : filter==="shared" ? sharedSection : `${ownedSection}${sharedSection}`;
  ...
  return c.html(page(m.dashboard,`${eventHeader(locale,user,`<button id="open-new-event" class="rounded-xl bg-[#4f46e5] px-4 py-2 text-white">＋ ${m.createEvent}</button>`,pendingInvitations.length)}<main class="mx-auto max-w-6xl p-5 md:p-10"><div class="flex flex-wrap items-end justify-between gap-3"><div><p class="text-sm uppercase tracking-[.2em] text-[#4338ca]">Dashboard</p><h1 class="text-4xl">${m.dashboard}</h1></div></div>${invitationSection}<form method="get" class="mt-8 grid gap-3 rounded-2xl bg-white p-4 shadow-sm md:grid-cols-[1fr_auto_auto_auto]"><input name="q" value="${esc(query)}" placeholder="${locale==="el"?"Αναζήτηση event":"Search events"}" class="rounded-xl border px-4 py-3"><select name="filter" class="rounded-xl border px-4 py-3">${Object.entries(filterLabel).map(([v,l])=>`<option value="${v}"${filter===v?" selected":""}>${l}</option>`).join("")}</select><select name="sort" class="rounded-xl border px-4 py-3"><option value="date_desc"${sort==="date_desc"?" selected":""}>${locale==="el"?"Νεότερη ημερομηνία":"Newest date"}</option><option value="date_asc"${sort==="date_asc"?" selected":""}>${locale==="el"?"Παλαιότερη ημερομηνία":"Oldest date"}</option><option value="name_asc"${sort==="name_asc"?" selected":""}>A → Z</option><option value="name_desc"${sort==="name_desc"?" selected":""}>Z → A</option><option value="created_desc"${sort==="created_desc"?" selected":""}>${locale==="el"?"Πρόσφατα albums":"Recently created"}</option></select><button class="rounded-xl bg-[#172033] px-5 text-white">${locale==="el"?"Εφαρμογή":"Apply"}</button></form>${eventSections}</main><dialog id="new-event" class="fixed left-1/2 top-1/2 m-0 flex max-h-[92dvh] w-[min(92vw,600px)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-3xl border-0 bg-white p-0 shadow-2xl backdrop:bg-[#172033]/60"><div class="max-h-[92dvh] overflow-y-auto p-6 sm:p-8"><div class="flex items-center justify-between gap-4"><div><p class="text-xs uppercase tracking-[.2em] text-[#4338ca]">Memboux</p><h2 class="text-3xl">${m.createEvent}</h2></div><button type="button" id="close-new-event" class="flex h-10 w-10 items-center justify-center rounded-full border text-xl">×</button></div><form action="/api/account/events" method="post" class="mt-6 grid gap-4 md:grid-cols-2"><input type="hidden" name="locale" value="${locale}"><label class="md:col-span-2"><span class="mb-1 block text-sm font-medium">${m.eventName}</span><input name="eventName" required maxlength="100" placeholder="${m.eventName}" class="w-full rounded-xl border px-4 py-3"></label><label class="text-sm">${locale==="el"?"Ημερομηνία έναρξης":"Start date"}<input name="eventStartDate" type="date" required class="mt-1 w-full rounded-xl border px-4 py-3"></label><label class="text-sm">${locale==="el"?"Ημερομηνία λήξης (προαιρετικά)":"End date (optional)"}<input name="eventEndDate" type="date" class="mt-1 w-full rounded-xl border px-4 py-3"></label><button class="rounded-xl bg-[#4f46e5] py-3 font-medium text-white md:col-span-2">${m.createEvent}</button></form></div></dialog><script>const newEventDialog=document.getElementById('new-event');document.getElementById('open-new-event').onclick=()=>newEventDialog.showModal();document.getElementById('close-new-event').onclick=()=>newEventDialog.close();newEventDialog.onclick=e=>{if(e.target===newEventDialog)newEventDialog.close()}<\/script>${logoutScript(locale)}`));
});
  ...
  try {
    ...
    for (let attempt=0; attempt<5; attempt++) {
      ...
      await c.env.DB.batch([
        c.env.DB.prepare("INSERT INTO events (id,code,couple,eventName,admin_token_hash,created_at,expires_at,status,notes,updated_at,default_locale,event_start_date,event_end_date) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(id,code,eventName,eventName,tokenHash,now,now+365*86400000,"active","",now,locale,eventStartDate,eventEndDate),
        c.env.DB.prepare("INSERT INTO event_members (event_id,user_id,role,created_at) VALUES (?,?,?,?)").bind(id,user.id,"owner",now)
      ]);
        const redirect = `/${locale}/account`;
