...
eventRoutes.get("/dashboard/:code", async(c)=>{
  ...
  const sharePanel=`<section class="mb-7 rounded-3xl bg-white p-5 shadow-lg sm:p-7"><div class="grid items-center gap-6 md:grid-cols-[160px_minmax(0,1fr)]"><div class="mx-auto w-full max-w-[160px] rounded-2xl border bg-white p-3">${qrSvg}</div><div class="min-w-0"><p class="text-xs uppercase tracking-[.2em] text-[#5b21b6]">${locale==="el"?"Κοινοποίηση event":"Share event"}</p><h2 class="mt-1 text-3xl">QR Code & link</h2><a href="${esc(guestUrl)}" target="_blank" class="mt-3 block break-all text-sm text-[#4f46e5]">${esc(guestUrl)}</a><div class="mt-4 flex flex-col gap-2 sm:flex-row"><input id="guest-link" readonly value="${esc(guestUrl)}" class="min-w-0 flex-1 rounded-xl border px-4 py-3"><button id="copy-guest-link" class="rounded-xl bg-[#4f46e5] px-5 py-3 text-white">${locale==="el"?"Αντιγραφή":"Copy link"}</button></div>${shareIconButtons(guestUrl,event.eventName,locale)}${event.gallery_pin_hash?`<p class="mt-3 text-xs text-[#64748b]">🔒 ${locale==="el"?"Το gallery προστατεύεται με PIN.":"This gallery is PIN protected."}</p>`:""}</div></div></section>`;
  ...
  return c.html(page(event.eventName,`<header class="border-b bg-white"><div class="mx-auto flex max-w-6xl items-center justify-between gap-3 p-5">${brandMark(`/${locale}`,true)}${accountMenu(locale,user)}</div></header><main class="mx-auto max-w-6xl p-5 md:p-10"><section class="relative mb-8 text-center">${membership==="owner"?`<details class="absolute right-0 top-0 z-20 text-left"><summary class="flex h-11 w-11 cursor-pointer list-none items-center justify-center rounded-full border bg-white text-2xl shadow-sm" aria-label="Event actions">⋯</summary><div class="absolute right-0 mt-1 w-44 rounded-2xl border bg-white p-2 shadow-xl"><a href="/dashboard/${event.code}/edit?lang=${locale}" class="block rounded-xl px-3 py-2 text-sm hover:bg-[#f5f7ff]">${locale==="el"?"Επεξεργασία event":"Edit event"}</a></div></details>`:""}<p class="text-xs uppercase tracking-[.25em] text-[#5b21b6]">Collecting Moments</p><h1 class="mt-3 text-5xl md:text-6xl">${esc(event.eventName)}</h1><p class="mt-3 text-lg text-[#4f46e5]">${esc(formatEventDates(event,locale))}</p></section>${sharePanel}<section class="rounded-3xl bg-white p-5 shadow-lg sm:p-7"><div class="mb-5 flex items-center justify-between"><div><h2 class="text-3xl">Gallery</h2>${galleryFilterControls(items,"owner-gallery",locale)}</div><div class="flex flex-wrap items-center justify-end gap-2"><span class="text-sm text-[#64748b]">${items.length} ${locale==="el"?"αρχεία":"items"}</span><button id="owner-select-media" class="rounded-xl border px-3 py-2 text-sm">${locale==="el"?"Επιλογή":"Select"}</button><button id="owner-download-selected" class="hidden rounded-xl bg-[#4f46e5] px-3 py-2 text-sm text-white">${locale==="el"?"Λήψη επιλεγμένων":"Download selected"}</button>${canManageMedia?`<button id="owner-delete-selected" class="hidden rounded-xl border border-red-200 px-3 py-2 text-sm text-red-700">${locale==="el"?"Διαγραφή επιλεγμένων":"Delete selected"}</button>`:""}</div></div><form id="owner-bulk-media" action="/api/account/events/${event.code}/media/bulk-trash" method="post"><input type="hidden" name="locale" value="${locale}"><input id="owner-media-ids" type="hidden" name="ids"></form>${items.length?`<div class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">${cards(items,{lightbox:true,selectable:true,deferredSelection:true})}</div>`:`<p class="py-16 text-center text-[#64748b]">${locale==="el"?"Δεν υπάρχουν φωτογραφίες ακόμη.":"No photos yet."}</p>`}</section></main><script>document.getElementById('copy-guest-link').onclick=()=>navigator.clipboard.writeText(document.getElementById('guest-link').value);document.querySelectorAll('[data-native-share]').forEach(button=>button.onclick=async()=>{if(navigator.share){try{await navigator.share({title:${JSON.stringify(event.eventName)},text:${JSON.stringify(shareText)},url:${JSON.stringify(guestUrl)}});return}catch(error){if(error.name==='AbortError')return}}await navigator.clipboard.writeText(${JSON.stringify(shareText)});alert(${JSON.stringify(locale==="el"?"Το link αντιγράφηκε. Άνοιξε την εφαρμογή και κάνε επικόλληση.":"Link copied. Open the app and paste it.")})<\/script>${ownerSelectionScript}${galleryFilterScript(items,"owner-gallery")}${lightboxMarkup(locale)}${logoutScript(locale)}`));
});
...
eventRoutes.get("/dashboard/:code/edit", async(c)=>{
  ...
  const invitations=(await c.env.DB.prepare("SELECT id,event_id,email,role,created_at,expires_at,accepted_at,declined_at FROM event_invitations WHERE event_id=? AND accepted_at IS NULL AND declined_at IS NULL AND expires_at>? ORDER BY created_at DESC").bind(event.id,Date.now()).all<EventInvitationRow>()).results;
...
eventRoutes.get("/dashboard/:code/manage-legacy", async (c) => {
  ...
  const invitations = canManageMembers ? (await c.env.DB.prepare("SELECT id,event_id,email,role,created_at,expires_at,accepted_at,declined_at FROM event_invitations WHERE event_id=? AND accepted_at IS NULL AND declined_at IS NULL AND expires_at>? ORDER BY created_at DESC").bind(event.id, Date.now()).all<EventInvitationRow>()).results : [];
  const guestUrl = `${new URL(c.req.url).origin}/gallery/${event.code}`;
  ...
  const detailsPanel = canManageMembers ? `<section id="event-details" class="mb-6 rounded-3xl bg-white p-5 shadow-lg sm:p-7"><h2 class="text-2xl">${labels.eventDates}</h2><form action="/api/account/events/${encodeURIComponent(event.code)}/details" method="post" class="mt-4 grid gap-3 md:grid-cols-2"><input type="hidden" name="locale" value="${locale}"><label class="md:col-span-2"><span class="mb-1 block text-sm font-medium">${locale === "el" ? "Όνομα event" : "Event name"}</span><input name="eventName" required maxlength="100" value="${esc(event.eventName)}" class="w-full rounded-xl border px-4 py-3"></label><label><span class="mb-1 block text-sm font-medium">${labels.startDate}</span><input name="eventStartDate" type="date" required value="${esc(event.event_start_date ?? "")}" class="w-full rounded-xl border px-4 py-3"></label><label><span class="mb-1 block text-sm font-medium">${labels.endDate}</span><input name="eventEndDate" type="date" value="${esc(event.event_end_date ?? "")}" class="w-full rounded-xl border px-4 py-3"></label><button class="rounded-xl bg-[#4f46e5] px-5 py-3 font-medium text-white md:col-span-2">${labels.saveDates}</button></form></section>` : "";
  let teamPanel = canManageMembers ? `${detailsPanel}<section class="mb-6 rounded-3xl bg-white p-5 shadow-lg sm:p-7"><div class="grid gap-7 lg:grid-cols-[1fr_1fr]"><div><h2 class="text-2xl">${labels.team}</h2><div class="mt-4 space-y-3">${members.map((member) => `<div class="flex items-center justify-between gap-3 rounded-2xl border p-4"><div class="min-w-0"><p class="truncate font-medium">${esc(member.name)}</p><p class="truncate text-sm text-[#64748b]">${esc(member.email)}</p></div>${member.role === "owner" ? `<span class="rounded-full bg-[#eef2ff] px-3 py-1 text-xs">Owner</span>` : `<form action="/api/account/events/${encodeURIComponent(event.code)}/members/remove" method="post"><input type="hidden" name="userId" value="${esc(member.user_id)}"><input type="hidden" name="locale" value="${locale}"><button class="text-sm font-medium text-red-700">${labels.remove}</button></form>`}</div>`).join("")}${invitations.map((invite) => `<div class="flex items-center justify-between gap-3 rounded-2xl border border-dashed p-4"><div class="min-w-0"><p class="truncate">${esc(invite.email)}</p><p class="text-xs text-[#64748b]">${labels.pending}</p></div><form action="/api/account/events/${encodeURIComponent(event.code)}/members/remove" method="post"><input type="hidden" name="invitationId" value="${esc(invite.id)}"><input type="hidden" name="locale" value="${locale}"><button class="text-sm font-medium text-red-700">${labels.remove}</button></form></div>`).join("")}</div></div><div class="rounded-2xl bg-[#f8faff] p-5"><h2 class="text-2xl">${labels.invite}</h2><p class="mt-1 text-sm text-[#64748b]">${labels.inviteHelp}</p><form action="/api/account/events/${encodeURIComponent(event.code)}/invite" method="post" class="mt-5 space-y-3"><input type="hidden" name="locale" value="${locale}"><input name="email" type="email" required maxlength="254" placeholder="name@example.com" class="w-full rounded-xl border bg-white px-4 py-3"><button class="w-full rounded-xl bg-[#4f46e5] px-5 py-3 font-medium text-white">${labels.sendInvite}</button></form></div></div></section>` : "";
  const professionalPanel=canManageMembers?`<section class="mb-6 rounded-3xl bg-[#172033] p-6 text-white shadow-lg"><p class="text-xs uppercase tracking-[.2em] text-white/60">Memboux Studio</p><div class="mt-2 flex flex-wrap items-center justify-between gap-4"><div><h2 class="text-2xl">${locale==="el"?"Official photographer":"Official photographer"}</h2><p class="mt-1 text-sm text-white/70">${locale==="el"?"Ανάθεσε επαγγελματία για το official album.":"Assign a professional to curate the official album."}</p></div><a href="/dashboard/${event.code}/professional?lang=${locale}" class="rounded-xl bg-white px-5 py-3 text-sm text-[#172033]">${locale==="el"?"Διαχείριση":"Manage"}</a></div></section>`:"";
  teamPanel+=professionalPanel;
  return c.html(page(`${event.eventName} – ${labels.title}`, `<header class="border-b bg-white"><div class="mx-auto flex max-w-6xl items-center justify-between gap-3 p-4 sm:p-5">${brandMark(`/${locale}`, true)}<div class="flex items-center gap-2"><a href="${toggleUrl}" class="rounded-lg border px-3 py-2 text-sm font-semibold">${otherLocale.toUpperCase()}</a></div></div></header><main class="mx-auto max-w-6xl p-4 sm:p-5 md:p-10"><section class="mb-6 rounded-3xl bg-white p-5 shadow-lg sm:p-7"><p class="text-sm font-semibold uppercase tracking-[.18em] text-[#5b21b6]">${labels.title}</p><h1 class="mt-2 break-words text-3xl font-bold sm:text-4xl">${esc(event.eventName)}</h1><p class="mt-2 text-lg font-medium text-[#4f46e5]">${esc(formatEventDates(event, locale))}</p><p class="mt-3">${labels.code}: <strong class="font-mono text-2xl text-[#4f46e5]">${esc(event.code)}</strong></p><div class="mt-7 grid items-center gap-7 md:grid-cols-[minmax(0,220px)_minmax(0,1fr)]"><div class="mx-auto w-full max-w-[220px] overflow-hidden rounded-2xl border bg-white p-3">${qrSvg}</div><div class="min-w-0"><h2 class="text-xl font-bold">${labels.qr}</h2><p class="mt-2 text-sm text-[#64748b]">${labels.qrHelp}</p><a href="${esc(guestUrl)}" target="_blank" class="mt-3 block max-w-full break-all text-sm font-semibold text-[#4f46e5]">${esc(guestUrl)}</a><div class="mt-4 flex flex-col gap-2 sm:flex-row"><input id="link" readonly value="${esc(guestUrl)}" class="w-full min-w-0 flex-1 rounded-xl border px-4 py-3"><button id="copy" class="shrink-0 rounded-xl bg-[#1e293b] px-5 py-3 text-white">${labels.copy}</button></div></div></div></section>${teamPanel}<section class="rounded-3xl bg-white p-5 shadow-lg sm:p-7"><div class="mb-5 flex flex-wrap items-center justify-between gap-3"><h2 class="text-2xl font-bold">${labels.gallery} (${items.length})</h2><div class="flex gap-2"><button id="download-selected" class="rounded-lg border px-3 py-2 text-sm">${locale==="el"?"Λήψη επιλεγμένων":"Download selected"}</button><button id="delete-selected" class="rounded-lg border border-red-200 px-3 py-2 text-sm text-red-700">${locale==="el"?"Διαγραφή επιλεγμένων":"Delete selected"}</button></div></div><form id="bulk-media" action="/api/account/events/${event.code}/media/bulk-trash" method="post"><input type="hidden" name="locale" value="${locale}"><input type="hidden" id="media-ids" name="ids">${items.length ? `<div class="grid grid-cols-2 gap-4 md:grid-cols-3">${cards(items,{code:event.code,locale,selectable:true,manage:true})}</div>` : `<p class="py-12 text-center text-[#64748b]">${labels.empty}</p>`}</form></section></main><script>document.getElementById('copy').onclick=()=>navigator.clipboard.writeText(document.getElementById('link').value);const selected=()=>[...document.querySelectorAll('.media-select:checked')];document.getElementById('download-selected').onclick=()=>selected().forEach((box,i)=>setTimeout(()=>{const a=document.createElement('a');a.href=box.dataset.download;a.download='';a.click()},i*250));document.getElementById('delete-selected').onclick=()=>{const ids=selected().map(x=>x.value);if(!ids.length)return;if(confirm('Move selected media to trash?')){document.getElementById('media-ids').value=ids.join(',');document.getElementById('bulk-media').submit()}}<\/script>`));
});
...
eventRoutes.post("/api/account/events/:code/invite", async (c) => {
  ...
  if (existingUser) {
    ...
    if (existingMember) return wantsJson
      ...
      : c.redirect(`/dashboard/${event.code}/edit?lang=${locale}`, 303);
  }
  const invitationId = crypto.randomUUID();
  const invitationToken = createInvitationToken();
  const now = Date.now();
  await createOrReplaceInvitation(c.env.DB,{id:invitationId,eventId:event.id,email,role,invitedBy:user.id,createdAt:now,expiresAt:now+14*86400000,tokenHash:await hashInvitationToken(invitationToken)});
