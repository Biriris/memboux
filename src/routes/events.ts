...
eventRoutes.post("/api/account/events/:code/invite", async (c) => {
  ...
  await sendEmail(c.env, {
    ...
    subject,
    text,
    html: `<div style="font-family:Manrope,Arial,sans-serif;max-width:560px;margin:auto;color:#111827"><h1 style="font-family:Manrope,Arial,sans-serif;font-weight:500">Memboux</h1><p>${esc(text)}</p><p><a href="${accountUrl}" style="display:inline-block;background:#4f46e5;color:white;padding:12px 20px;border-radius:10px;text-decoration:none">${locale === "el" ? "Αποδοχή πρόσκλησης" : "Accept invitation"}</a></p><p style="color:#64748b;font-size:13px">${locale === "el" ? "Η πρόσκληση λήγει σε 14 ημέρες και αφορά μόνο αυτό το event." : "This invitation expires in 14 days and only grants access to this event."}</p></div>`,
  });
  return c.redirect(`/dashboard/${event.code}/edit?lang=${locale}`, 303);
});
