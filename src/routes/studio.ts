...
studioRoutes.get("/studio", async (c) => {
  ...
  if (!profile)
    return c.html(
      page(
        "Memboux Studio",
        `<header class="border-b bg-[#2b174d] text-white"><div class="mx-auto flex max-w-5xl items-center justify-between p-5">${brandMark(studioDashboardHref(locale), true, true)}${accountMenuDark(locale, user)}</div></header><main class="mx-auto max-w-3xl p-5 md:p-10"><section class="rounded-3xl bg-white p-8 text-center shadow"><h1 class="text-4xl">Memboux Studio</h1><p class="mt-3 text-[#6f657c]">${locale === "el" ? "Ο λογαριασμός σου δεν έχει ακόμη εγκεκριμένο professional profile." : "Your account does not have an approved professional profile yet."}</p></section></main>${logoutScript(locale)}`,
      ),
    );
  ...
  const cards = assignments.results
    .map(
      (event) =>
        `<article class="rounded-2xl border bg-white p-5 shadow-sm"><span class="rounded-full bg-[#f1eaff] px-3 py-1 text-xs uppercase">${esc(event.assignment_status)}</span><h2 class="mt-3 text-2xl">${esc(event.eventName)}</h2><p class="mt-1 text-sm text-[#7c3aed]">${esc(formatEventDates(event, locale))}</p>${event.assignment_status === "invited" ? `<form action="/studio/assignments/${encodeURIComponent(event.event_id)}/accept" method="post" class="mt-4"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl bg-[#7c3aed] px-4 py-3 text-white">${locale === "el" ? "Αποδοχή ανάθεσης" : "Accept assignment"}</button></form>` : `<a href="/studio/events/${event.code}?lang=${locale}" class="mt-4 block rounded-xl bg-[#2b174d] px-4 py-3 text-center text-white">${locale === "el" ? "Άνοιγμα workspace" : "Open workspace"}</a>`}</article>`,
    )
    .join("");
  return c.html(
    page(
      "Memboux Studio",
      `<header class="border-b bg-[#2b174d] text-white"><div class="mx-auto flex max-w-6xl items-center justify-between p-5">${brandMark(studioDashboardHref(locale), true, true)}${accountMenuDark(locale, user)}</div></header><main class="mx-auto max-w-6xl p-5 md:p-10"><div class="flex flex-wrap items-end justify-between gap-4"><div><p class="text-xs uppercase tracking-[.2em] text-[#6d28d9]">Professional workspace</p><h1 class="mt-2 text-4xl">${esc(profile.business_name)}</h1></div><a href="/studio/trash?lang=${locale}" class="rounded-xl border bg-white px-4 py-2 text-sm">${locale === "el" ? "Κάδος Studio" : "Studio trash"}</a></div><div class="mt-7 grid gap-4 md:grid-cols-2">${cards || `<p class="rounded-2xl bg-white p-8 text-[#6f657c]">${locale === "el" ? "Δεν υπάρχουν αναθέσεις." : "No assignments yet."}</p>`}</div></main>${logoutScript(locale)}`,
    ),
  );
});
...
studioRoutes.get("/studio/media/:id", async (c) => {
  ...
  const media = await c.env.DB.prepare(
    "SELECT object_key,content_type,media_type,event_id FROM media WHERE id=? AND deleted_at IS NULL AND reported_at IS NULL",
  )
    .bind(c.req.param("id"))
    .first<{ object_key: string; content_type: string; media_type: "image" | "video"; event_id: string }>();
  ...
  const preview = media.media_type === "image" && !originalDownloadsAllowed
    ? await getOrCreateMediaVariant(c.env, media.object_key, "preview")
    : null;
  if (preview && preview.object.key === media.object_key)
    return c.text("Preview unavailable. The original remains protected.", 503);
  const object = preview?.object ?? await c.env.MEDIA.get(media.object_key);
  if (!object) return c.text("Media not found", 404);
  return new Response(object.body, {
    headers: {
      "Content-Type": preview ? "image/webp" : media.content_type,
      "Cache-Control": "private, no-store",
      "X-Content-Type-Options": "nosniff",
      ...(originalDownloadsAllowed ? {} : { "X-Memboux-Media-Access": "preview-only" }),
    },
  });
});
...
studioRoutes.get("/studio/trash/media/:id", async (c) => {
  ...
  if (!user) return c.text("Unauthorized", 401);
  const media = await c.env.DB.prepare(
    `SELECT m.object_key,m.content_type,m.media_type,m.event_id
     FROM media m
...
studioRoutes.get("/studio/trash", async (c) => {
  ...
  return c.html(
    page(
      ...
      `<header class="border-b bg-[#2b174d] text-white"><div class="mx-auto flex max-w-6xl items-center justify-between p-5">${brandMark(studioDashboardHref(locale), true, true)}${accountMenuDark(locale, user)}</div></header><main class="mx-auto max-w-6xl p-5 md:p-10"><a href="/studio?lang=${locale}" class="text-sm text-[#7c3aed]">← Studio</a><h1 class="mt-3 text-4xl">${locale === "el" ? "Κάδος Studio" : "Studio trash"}</h1><p class="mt-2 text-[#6f657c]">${locale === "el" ? "Τα δικά σου official uploads διατηρούνται για 30 ημέρες." : "Your official uploads are retained for 30 days."}</p><form action="/studio/trash/restore" method="post" class="mt-7"><input type="hidden" name="locale" value="${locale}"><div class="grid grid-cols-2 gap-4 md:grid-cols-4">${tiles || `<p class="col-span-full rounded-2xl bg-white p-10 text-center text-[#6f657c]">${locale === "el" ? "Ο κάδος είναι άδειος." : "Trash is empty."}</p>`}</div>${media.results.length ? `<button class="mt-5 rounded-xl bg-[#7c3aed] px-5 py-3 text-white">${locale === "el" ? "Επαναφορά επιλεγμένων" : "Restore selected"}</button>` : ""}</form></main>${logoutScript(locale)}`,
    ),
  );
});
