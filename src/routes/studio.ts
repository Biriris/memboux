...
studioRoutes.get("/studio", async (c) => {
  ...
  if (!profile)
    return c.html(
      page(
        "Memboux Studio",
        `<header class="border-b bg-[#33251f] text-white"><div class="mx-auto flex max-w-5xl items-center justify-between p-5">${brandMark(`/${locale}`, true, true)}${accountMenuDark(locale, user)}</div></header><main class="mx-auto max-w-3xl p-5 md:p-10"><section class="rounded-3xl bg-white p-8 text-center shadow"><h1 class="text-4xl">Memboux Studio</h1><p class="mt-3 text-[#625750]">${locale === "el" ? "Ο λογαριασμός σου δεν έχει ακόμη εγκεκριμένο professional profile." : "Your account does not have an approved professional profile yet."}</p></section></main>${logoutScript(locale)}`,
      ),
    );
  ...
  const cards = assignments.results
    .map(
      (event) =>
        `<article class="rounded-2xl border bg-white p-5 shadow-sm"><span class="rounded-full bg-[#eee4dc] px-3 py-1 text-xs uppercase">${esc(event.assignment_status)}</span><h2 class="mt-3 text-2xl">${esc(event.eventName)}</h2><p class="mt-1 text-sm text-[#654534]">${esc(formatEventDates(event, locale))}</p>${event.assignment_status === "invited" ? `<form action="/studio/assignments/${encodeURIComponent(event.event_id)}/accept" method="post" class="mt-4"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl bg-[#654534] px-4 py-3 text-white">${locale === "el" ? "Αποδοχή ανάθεσης" : "Accept assignment"}</button></form>` : `<a href="/studio/events/${event.code}?lang=${locale}" class="mt-4 block rounded-xl bg-[#33251f] px-4 py-3 text-center text-white">${locale === "el" ? "Άνοιγμα workspace" : "Open workspace"}</a>`}</article>`,
    )
    .join("");
  return c.html(
    page(
      "Memboux Studio",
      `<header class="border-b bg-[#33251f] text-white"><div class="mx-auto flex max-w-6xl items-center justify-between p-5">${brandMark("/studio", true, true)}${accountMenuDark(locale, user)}</div></header><main class="mx-auto max-w-6xl p-5 md:p-10"><div class="flex flex-wrap items-end justify-between gap-4"><div><p class="text-xs uppercase tracking-[.2em] text-[#6e4f3e]">Professional workspace</p><h1 class="mt-2 text-4xl">${esc(profile.business_name)}</h1></div><a href="/studio/trash?lang=${locale}" class="rounded-xl border bg-white px-4 py-2 text-sm">${locale === "el" ? "Κάδος Studio" : "Studio trash"}</a></div><div class="mt-7 grid gap-4 md:grid-cols-2">${cards || `<p class="rounded-2xl bg-white p-8 text-[#625750]">${locale === "el" ? "Δεν υπάρχουν αναθέσεις." : "No assignments yet."}</p>`}</div></main>${logoutScript(locale)}`,
    ),
  );
});
