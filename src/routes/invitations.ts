...
invitationRoutes.get("/invite/:token", async (c) => {
  ...
  if (invitation.accepted_at !== null) {
    actions = matchingUser
      ? `<a href="/dashboard/${encodeURIComponent(invitation.event_code)}?lang=${locale}" class="inline-flex rounded-xl bg-[#2f6b5b] px-6 py-3 font-semibold text-white">${el ? "Άνοιγμα album" : "Open album"}</a>`
    ...
    actions = `<p class="rounded-2xl bg-[#f6faf8] p-4 text-sm text-[#4a6159]">${el ? "Η πρόσκληση έχει απορριφθεί." : "This invitation has been declined."}</p>`;
  } else if (expired) {
    ...
    actions = `<div class="grid gap-3 sm:grid-cols-2"><a href="/${locale}/login?redirect=${encodeURIComponent(returnPath)}" class="rounded-xl bg-[#183c33] px-6 py-3 text-center font-semibold text-white">${el ? "Σύνδεση για αποδοχή" : "Sign in to accept"}</a><a href="/${locale}/register?redirect=${encodeURIComponent(returnPath)}" class="rounded-xl border border-[#ccd9d4] px-6 py-3 text-center font-semibold text-[#183c33]">${el ? "Δημιουργία λογαριασμού" : "Create account"}</a></div><p class="mt-3 text-xs leading-5 text-[#65756f]">${el ? "Χρησιμοποίησε το email στο οποίο στάλθηκε η πρόσκληση. Μετά τη σύνδεση θα επιστρέψεις εδώ." : "Use the email address that received the invitation. You will return here after signing in."}</p>`;
  } else if (!matchingUser) {
    actions = `<div class="rounded-2xl bg-amber-50 p-4 text-sm leading-6 text-amber-900"><strong>${el ? "Διαφορετικός λογαριασμός" : "Different account"}</strong><p>${el ? `Η πρόσκληση προορίζεται για ${esc(maskEmail(invitation.email))}. Συνδέσου με εκείνο το email.` : `This invitation is for ${esc(maskEmail(invitation.email))}. Sign in with that email.`}</p></div><button type="button" data-logout class="mt-3 rounded-xl border border-[#ccd9d4] px-6 py-3 font-semibold">${el ? "Αποσύνδεση" : "Sign out"}</button>`;
  } else {
    actions = `<div class="flex flex-col gap-3 sm:flex-row"><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/accept" method="post" class="flex-1"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl bg-[#2f6b5b] px-6 py-3 font-semibold text-white">${el ? "Αποδοχή και άνοιγμα" : "Accept and open"}</button></form><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/decline" method="post"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl border border-[#ccd9d4] px-6 py-3 font-semibold text-[#4a6159]">${el ? "Απόρριψη" : "Decline"}</button></form></div>`;
  }

  return c.html(page(
    `${invitation.event_name} – ${el ? "Πρόσκληση" : "Invitation"}`,
    `${header}<main class="mx-auto flex min-h-[75vh] max-w-4xl items-center p-5"><section class="grid w-full overflow-hidden rounded-[2rem] border border-[#dbe6e1] bg-white shadow-[0_30px_100px_rgba(24,60,51,.13)] lg:grid-cols-[.82fr_1.18fr]"><aside class="bg-[#183c33] p-8 text-white sm:p-10"><p class="text-xs uppercase tracking-[.22em] text-[#a9c9bc]">Album invitation</p><h1 class="mt-4 break-words text-4xl leading-tight">${esc(invitation.event_name)}</h1><p class="mt-5 text-sm leading-6 text-white/65">${el ? `${esc(invitation.inviter_name)} σε προσκαλεί σε ένα ιδιωτικό album στο Memboux.` : `${esc(invitation.inviter_name)} invited you to a private album on Memboux.`}</p><div class="mt-8 rounded-2xl bg-white/10 p-4"><p class="text-xs uppercase tracking-[.15em] text-white/50">${el ? "Ρόλος" : "Role"}</p><strong class="mt-1 block text-sm">${roleLabel}</strong></div></aside><div class="p-8 sm:p-10"><p class="text-xs uppercase tracking-[.2em] text-[#2f6b5b]">${resolved ? (el ? "Κατάσταση πρόσκλησης" : "Invitation status") : (el ? "Μοιρασμένο μαζί σου" : "Shared with you")}</p><h2 class="mt-3 text-3xl">${el ? "Αποδέχεσαι την πρόσκληση;" : "Accept this invitation?"}</h2><p class="mt-4 text-sm leading-6 text-[#65756f]">${el ? "Η πρόσβαση αφορά μόνο αυτό το album και μπορεί να αφαιρεθεί οποιαδήποτε στιγμή από τον ιδιοκτήτη." : "Access is limited to this album and can be removed by the owner at any time."}</p><dl class="my-7 space-y-3 rounded-2xl bg-[#f6faf8] p-5 text-sm"><div class="flex justify-between gap-4"><dt class="text-[#65756f]">Email</dt><dd class="break-all text-right font-medium">${esc(maskEmail(invitation.email))}</dd></div><div class="flex justify-between gap-4"><dt class="text-[#65756f]">${el ? "Λήξη" : "Expires"}</dt><dd class="text-right font-medium">${esc(formatDateTime(invitation.expires_at, locale))}</dd></div></dl>${actions}</div></section></main>${user ? logoutScript(locale) : ""}`,
    { locale },
  ));
});
...
invitationRoutes.post("/api/account/invitations/:id/:action{accept|decline}", async (c) => {
  ...
  if (result.status === "accepted") {
    ...
    return c.redirect(result.professional ? `/studio?lang=${locale}` : `/dashboard/${result.eventCode}?lang=${locale}`, 303);
  }
