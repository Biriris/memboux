...
invitationRoutes.get("/invite/:token", async (c) => {
  ...
  if (invitation.accepted_at !== null) {
    actions = matchingUser
      ? `<a href="/dashboard/${encodeURIComponent(invitation.event_code)}?lang=${locale}" class="inline-flex rounded-xl bg-[#7c3aed] px-6 py-3 font-semibold text-white">${el ? "Άνοιγμα album" : "Open album"}</a>`
    ...
    actions = `<p class="rounded-2xl bg-[#faf8ff] p-4 text-sm text-[#675a72]">${el ? "Η πρόσκληση έχει απορριφθεί." : "This invitation has been declined."}</p>`;
  } else if (expired) {
    ...
    actions = `<div class="grid gap-3 sm:grid-cols-2"><a href="/${locale}/login?redirect=${encodeURIComponent(returnPath)}" class="rounded-xl bg-[#2b174d] px-6 py-3 text-center font-semibold text-white">${el ? "Σύνδεση για αποδοχή" : "Sign in to accept"}</a><a href="/${locale}/register?redirect=${encodeURIComponent(returnPath)}" class="rounded-xl border border-[#dcd3e8] px-6 py-3 text-center font-semibold text-[#2b174d]">${el ? "Δημιουργία λογαριασμού" : "Create account"}</a></div><p class="mt-3 text-xs leading-5 text-[#6f657c]">${el ? "Χρησιμοποίησε το email στο οποίο στάλθηκε η πρόσκληση. Μετά τη σύνδεση θα επιστρέψεις εδώ." : "Use the email address that received the invitation. You will return here after signing in."}</p>`;
  } else if (!matchingUser) {
    actions = `<div class="rounded-2xl bg-amber-50 p-4 text-sm leading-6 text-amber-900"><strong>${el ? "Διαφορετικός λογαριασμός" : "Different account"}</strong><p>${el ? `Η πρόσκληση προορίζεται για ${esc(maskEmail(invitation.email))}. Συνδέσου με εκείνο το email.` : `This invitation is for ${esc(maskEmail(invitation.email))}. Sign in with that email.`}</p></div><button type="button" data-logout class="mt-3 rounded-xl border border-[#dcd3e8] px-6 py-3 font-semibold">${el ? "Αποσύνδεση" : "Sign out"}</button>`;
  } else {
    actions = `<div class="flex flex-col gap-3 sm:flex-row"><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/accept" method="post" class="flex-1"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl bg-[#7c3aed] px-6 py-3 font-semibold text-white">${el ? "Αποδοχή και άνοιγμα" : "Accept and open"}</button></form><form action="/api/account/invitations/${encodeURIComponent(invitation.id)}/decline" method="post"><input type="hidden" name="locale" value="${locale}"><button class="w-full rounded-xl border border-[#dcd3e8] px-6 py-3 font-semibold text-[#675a72]">${el ? "Απόρριψη" : "Decline"}</button></form></div>`;
  }
