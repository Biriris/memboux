...
backupRoutes.get("/:locale{el|en|fr|de|es|it}/backups", async (c) => {
  ...
  return c.html(page(copy.title, `${accountHeader(locale, user)}<main class="mx-auto max-w-5xl p-5 md:p-10">
    <section class="overflow-hidden rounded-[2rem] bg-[#2b174d] p-6 text-white shadow-xl sm:p-9">
      <p class="text-xs font-semibold uppercase tracking-[.22em] text-[#c4b5fd]">${copy.eyebrow}</p>
