...
backupRoutes.post("/api/account/events/:code/backups/google", async (c) => {
  ...
  if (!owner) return c.text("Only the event owner can create a cloud backup", 403);
  const connection = await c.env.DB.prepare(
