...
accountTrashRoutes.get("/account/trash/media/:id", async (c) => {
  ...
  const media = await c.env.DB.prepare(`SELECT m.object_key,m.content_type,m.media_type,m.event_id FROM media m
...
accountTrashRoutes.get("/:locale{el|en|fr|de|es|it}/trash", async (c) => {
  ...
  const eventRows = events.results.map((event) => `<article class="owner-event-trash-selectable relative overflow-hidden rounded-2xl border bg-white shadow-sm transition">
    <label class="owner-event-trash-selector absolute inset-0 z-20 hidden cursor-pointer">
      ...
      <span class="owner-event-trash-tick absolute left-3 top-3 hidden h-9 w-9 items-center justify-center rounded-full bg-[#7c3aed] text-white shadow-lg">✓</span>
    </label>
    <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
      <div class="min-w-0"><span class="text-xs font-bold uppercase tracking-[.14em] text-[#6d28d9]">Event</span><h3 class="mt-1 truncate text-2xl">${esc(event.eventName)}</h3><p class="mt-1 text-sm text-red-700">${label(locale, "Permanent deletion", "Οριστική διαγραφή")}: ${formatDateTime(event.purge_at!, locale)}</p></div>
  ...
  const body = `<header class="border-b bg-white"><div class="mx-auto flex max-w-5xl items-center justify-between p-5">${brandMark(`/${locale}`, true)}${accountMenu(locale, user)}</div></header><main class="mx-auto max-w-5xl p-5 md:p-10">${restoreNotice}<a href="/${locale}/account" class="text-sm text-[#7c3aed]">← ${label(locale, "My events", "Τα events μου")}</a><h1 class="mt-4 text-4xl">${label(locale, "Trash", "Κάδος")}</h1><p class="mt-2 text-[#6f657c]">${label(locale, "Items are permanently deleted 30 days after being moved here.", "Τα στοιχεία διαγράφονται οριστικά 30 ημέρες μετά τη μεταφορά τους εδώ.")}</p>
    ...
    <div class="mt-3 space-y-3">${eventRows || `<p class="rounded-2xl bg-white p-6 text-[#6f657c]">${label(locale, "No deleted events.", "Δεν υπάρχουν διαγραμμένα events.")}</p>`}</div>
    ...
    <div class="mt-3 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">${mediaRows || `<p class="col-span-full rounded-2xl bg-white p-6 text-[#6f657c]">${label(locale, "No deleted photos.", "Δεν υπάρχουν διαγραμμένες φωτογραφίες.")}</p>`}</div></main>`;
