...
adminUserRoutes.get("/admin/users/:id", async (c) => {
  ...
  const paymentRows = payments.results.map((payment) => `<tr class="border-t"><td class="px-3 py-3">${payment.paid_at ? formatDateTime(payment.paid_at, locale) : "—"}</td><td class="px-3 py-3">${esc(formatSubscriptionMoney(payment.amount_minor, payment.currency, locale))}</td><td class="px-3 py-3 capitalize">${esc(payment.status)}</td><td class="px-3 py-3">${esc(payment.provider)}</td><td class="px-3 py-3 text-[#65756f]">${esc(payment.note ?? "")}</td></tr>`).join("");
  const eventCards = events.results.map((event) => `<article class="rounded-2xl border bg-white p-5 shadow-sm"><div class="flex flex-wrap items-start justify-between gap-3"><div><h3 class="text-xl">${esc(event.eventName)}</h3><p class="mt-1 text-sm text-[#65756f]">${esc(formatEventDates(event, locale))}</p></div>${event.role === "owner" ? `<span title="${locale === "el" ? "Η ιδιοκτησία αλλάζει μόνο με ασφαλή μεταβίβαση" : "Ownership can only be changed through a safe transfer"}">${roleBadge(event.role, locale)}</span>` : `<form action="/admin/users/${encodeURIComponent(user.id)}/events/${encodeURIComponent(event.id)}/role" method="post"><label class="sr-only" for="role-${esc(event.id)}">Role</label><select id="role-${esc(event.id)}" name="role" onchange="this.form.requestSubmit()" class="rounded-xl border bg-white px-3 py-2 text-xs font-semibold"><option value="editor"${selected("editor", event.role)}>${locale === "el" ? "Διαχειριστής" : "Manager"}</option><option value="viewer"${selected("viewer", event.role)}>${locale === "el" ? "Θεατής" : "Viewer"}</option></select><button class="sr-only">Save role</button></form>`}</div><div class="mt-4 flex flex-wrap gap-4 text-sm text-[#65756f]"><span>${event.media_count} files</span><span>${formatBytes(event.media_bytes)}</span><span>${event.deleted_at ? (locale === "el" ? "Στον κάδο" : "In trash") : event.status}</span></div><a href="/admin/events/${encodeURIComponent(event.code)}" class="mt-4 inline-flex rounded-xl bg-[#183c33] px-4 py-2 text-sm text-white">${locale === "el" ? "Διαχείριση album" : "Manage album"}</a></article>`).join("");
