...
eventProfessionalRoutes.get("/dashboard/:code/professional", async (c) => {
  ...
  return c.html(
    page(
      ...
      `<header class="border-b bg-white"><div class="mx-auto flex max-w-4xl items-center justify-between p-5">${brandMark(`/${locale}`, true)}${accountMenu(locale, user)}</div></header><main class="mx-auto max-w-4xl p-5 md:p-10"><a href="/dashboard/${event.code}?lang=${locale}#people" class="text-sm text-[#4f46e5]">← ${el ? "Επιστροφή στο event" : "Back to event workspace"}</a><h1 class="mt-4 text-4xl">${el ? "Official photographer" : "Official photographer"}</h1><p class="mt-2 text-[#64748b]">${el ? "Ο επαγγελματίας αποκτά πρόσβαση μόνο στο Studio workspace και στο official album." : "The professional receives access only to the Studio workspace and official album."}</p>${assignment ? `<section class="mt-6 rounded-2xl bg-white p-5 shadow"><h2 class="text-2xl">${esc(assignment.business_name)}</h2><p class="text-sm text-[#64748b]">${esc(assignment.email)} · ${esc(assignment.status)}</p><form action="/api/account/events/${event.code}/professional/revoke" method="post" class="mt-4"><input type="hidden" name="locale" value="${locale}"><button class="rounded-xl border border-red-200 px-4 py-2 text-red-700">${el ? "Ανάκληση πρόσβασης" : "Revoke access"}</button></form></section>` : ""}<section class="mt-6 rounded-2xl bg-white p-5 shadow"><h2 class="text-2xl">${el ? "Επιλογή επαγγελματία" : "Choose a professional"}</h2>${options ? `<form action="/api/account/events/${event.code}/professional/assign" method="post" class="mt-4 space-y-3"><input type="hidden" name="locale" value="${locale}"><select name="professionalUserId" required class="w-full rounded-xl border px-4 py-3">${options}</select><button class="w-full rounded-xl bg-[#4f46e5] px-5 py-3 text-white">${el ? "Αποστολή ανάθεσης" : "Send assignment"}</button></form>` : `<p class="mt-3 rounded-xl bg-amber-50 p-4 text-sm text-amber-900">${el ? "Δεν υπάρχουν ακόμη active professional profiles." : "No active professional profiles are available yet."}</p>`}</section></main>${logoutScript(locale)}`,
    ),
  );
});
