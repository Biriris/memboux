...
function guestPlannerPage(input: {
  ...
  const { event, locale, user, groups, guests, tables, origin, freshGuestId, freshToken } = input;
  const el = locale === "el";
  const groupOptions = groups.map((group) => `<option value="${esc(group.id)}">${esc(group.name)}</option>`).join("");
  const freshGuest = guests.find((guest) => guest.id === freshGuestId);
  ...
  return page(`${event.eventName} · Guests`, body, { locale });
}
...
weddingPlanningRoutes.get("/dashboard/:code/wedding/guests", async (c) => {
  ...
  const [groups, guests, tables] = await Promise.all([
    ...
    c.env.DB.prepare(`SELECT g.id,g.group_id,gg.name group_name,g.first_name,g.last_name,g.email,g.phone,
      ...
      WHERE g.event_id=? ORDER BY COALESCE(gg.name,''),g.last_name,g.first_name`).bind(event.id).all<WeddingGuest>(),
  ...
  return c.html(guestPlannerPage({
    ...
    freshGuestId: c.req.query("guest") ?? undefined,
