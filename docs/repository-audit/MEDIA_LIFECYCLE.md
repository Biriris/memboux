...
The shared gallery lightbox supports horizontal navigation, pinch/desktop zoom, explicit close, and swipe-up dismissal for an unzoomed open image. Swipe-up dismissal is attached only to pages that render the media lightbox and does not override horizontal navigation or zoomed-image panning. Gallery video previews use a contained 4:5 tile independent of source metadata, while the lightbox preserves the full video with `object-contain`; this prevents late video metadata from expanding or overlapping brickwall cards. The verified implementations are [`src/views/media.ts`](../../src/views/media.ts), [`src/views/shared.ts`](../../src/views/shared.ts), and [`src/styles.css`](../../src/styles.css).
