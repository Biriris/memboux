...
The shared gallery lightbox supports horizontal navigation, pinch/desktop zoom, explicit close, and swipe-up dismissal for an unzoomed open image. Swipe-up dismissal is attached only to pages that render the media lightbox and does not override horizontal navigation or zoomed-image panning. Closing the lightbox through any path pauses the active video or audio element, removes its source, reloads it to release the browser media pipeline, and clears the stage. Original downloads are probed with a one-byte range request before browser navigation starts the attachment response; an authorization or missing-object response is shown as an error instead of being saved as a text file. Gallery image and video previews share the same contained 4:5 `object-cover` tile, while the lightbox preserves each full image or video with `object-contain`. Progressive cards are hidden through both the renderer/script state and an explicit CSS guard so masonry display rules cannot expose an unmeasured, full-width deferred image or video over the grid. The verified implementations are [`src/views/media.ts`](../../src/views/media.ts), [`src/views/shared.ts`](../../src/views/shared.ts), and [`src/styles.css`](../../src/styles.css).
