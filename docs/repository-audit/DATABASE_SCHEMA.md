...
The D1 schema is defined by the ordered SQL files in [`migrations/`](../../migrations/), not by TypeScript types. The current chain contains 71 files, from [`0001_initial.sql`](../../migrations/0001_initial.sql) through [`0071_media_operations_phase_two.sql`](../../migrations/0071_media_operations_phase_two.sql). Migrations `0061`, `0062`, `0064`, and `0066` have dedicated D1 compatibility coverage in [`wedding-guest-planning-migration.test.ts`](../../test/wedding-guest-planning-migration.test.ts), [`wedding-invitation-delivery-migration.test.ts`](../../test/wedding-invitation-delivery-migration.test.ts), [`event-surface-pins-migration.test.ts`](../../test/event-surface-pins-migration.test.ts), and [`event-owner-invitations-migration.test.ts`](../../test/event-owner-invitations-migration.test.ts); migrations `0063` and `0067` through `0071` are exercised through the complete migration chain in the Worker test suite.

Repository tests prove the newest migration against the D1 test runtime; they do not prove that remote D1 has applied migration `0071`.
...
| `event_access` | `preview`/`free`/`unlocked`/`expired`, selected `plan_key`, observe/enforced mode, feature flags, optional package expiry, media and custom-album limits, lifetime consumed-upload counter, upload-window duration/start/end, and an irreversible premium-activation marker. | [`0041`](../../migrations/0041_event_access_lifecycle.sql), [`0058`](../../migrations/0058_lifetime_trial_media_slots.sql), rebuilt by [`0073`](../../migrations/0073_permanent_free_event_plan.sql), extended by [`0074`](../../migrations/0074_event_upload_windows_and_no_downgrade.sql) and [`0075`](../../migrations/0075_event_album_entitlements.sql) |
...
| `event_experience_settings` | RSVP, guestbook, comments, moderation, independent individual/bulk guest-download controls, video-guestbook and image/video slideshow controls. | [`0027`](../../migrations/0027_event_engagement.sql), [`0069`](../../migrations/0069_event_media_hub.sql), [`0071`](../../migrations/0071_media_operations_phase_two.sql), [`0072`](../../migrations/0072_guest_bulk_downloads.sql) |
| `event_branding` | Event presentation name, colors, logo media reference, Memboux-signature preference, update actor and timestamp. | [`0071`](../../migrations/0071_media_operations_phase_two.sql) |
...
Migration [`0075_event_album_entitlements.sql`](../../migrations/0075_event_album_entitlements.sql) adds the custom-album entitlement to `commerce_products` and `event_access`. Its insert trigger enforces one custom album for Free, three for Moments (`event_pass`) and five for Celebration (`event_plus`) when access enforcement is active. Soft-deleted albums do not consume a slot and existing over-limit events are preserved. The main Guest Gallery and Official Album are not `event_albums` rows, but enforced Free events cannot use Official Album; paid packages include it without consuming a custom-album slot.
