...
`GET /dashboard/:code` renders event identity, verified status and the recommended next action. Exact routes below it (`/website`, `/guests`, `/media`, `/experience`, `/menu`, `/share`, `/lifecycle`, and `/team`) render focused workspace areas in [`src/routes/events.ts`](../../src/routes/events.ts). The owner journey is set up, guests, collect memories, event experience, share/print, and archive/backup; team access is a separate utility. The former `/manage` URL remains only as a compatibility alias that renders the overview and is no longer a workspace step. [`src/views/event-workspace.ts`](../../src/views/event-workspace.ts) renders the event-specific panels, while [`src/views/event-workspace-shell.ts`](../../src/views/event-workspace-shell.ts) owns the responsive workspace navigation and phase metadata.

The navigation is derived from the verified event membership role without changing the capability model: owners receive the complete journey plus team access; editors and viewers receive only overview, media, and sharing pages. Owner-only section requests are rejected server-side. The shell exposes `data-workspace-section-link` attributes, marks the selected page with `aria-current="page"`, and emits local `memboux:workspace-navigation` and `memboux:workspace-action` browser events. No analytics provider consumes those events in this repository.
