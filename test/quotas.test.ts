...
beforeEach(async()=>{
  await env.DB.batch([
    ...
    env.DB.prepare("CREATE TABLE event_invitations (event_id TEXT,accepted_at INTEGER,declined_at INTEGER,expires_at INTEGER)"),
...
describe("account quotas",()=>{
  ...
  it("checks storage, owned events and pending collaborators",async()=>{
    ...
    await env.DB.batch([
      ...
      env.DB.prepare("INSERT INTO event_invitations VALUES (?,NULL,NULL,?)").bind("event-1",now+1000),
    ]);
