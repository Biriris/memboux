...
import { MAX_FILE_SIZE, MAX_LEGACY_FILE_SIZE, MAX_UPLOAD_FILES, MAX_UPLOAD_TOTAL_SIZE, VIDEO_UPLOADS_ENABLED, VIDEO_UPLOAD_TYPES } from "../src/config";
...
describe("upload validation", () => {
  ...
  it("enforces per-file and total byte limits at exact boundaries", () => {
    expect(validateUploadFiles([file({ size: MAX_LEGACY_FILE_SIZE })])).toBeNull();
    expect(validateUploadFiles([file({ size: MAX_LEGACY_FILE_SIZE + 1 })])).toBe("file_too_large");
    expect(validateUploadFiles([file({ size: MAX_FILE_SIZE })], { resumable: true })).toBeNull();
    expect(validateUploadFiles([file({ size: MAX_FILE_SIZE + 1 })], { resumable: true })).toBe("file_too_large");
    const maximumSelection = [
      file({ size: MAX_LEGACY_FILE_SIZE }),
      file({ size: MAX_UPLOAD_TOTAL_SIZE - MAX_LEGACY_FILE_SIZE }),
    ];
  ...
  it("returns localized errors with their HTTP status", () => {
    expect(uploadValidationDetails("unsupported_type", "en")).toEqual({
      message: "Supported formats: JPEG, PNG, WebP, GIF, MP4, WebM, and MOV.", status: 415,
    });
